...
module Homebrew
  ...
  module API
    ...
    sig { params(dry_run: T::Boolean).returns(T::Hash[Symbol, T::Hash[Symbol, T.anything]]) }
    def self.generate_cask_api!(dry_run: false)
      tap = CoreCaskTap.instance
      Kernel.raise TapUnavailableError, tap.name unless tap.installed?
      ...
      Homebrew.with_no_api_env do
        ...
        File.write("api/cask_tap_migrations.json", tap_migrations_json) unless dry_run
        ...
        Homebrew::SimulateSystem.with(os: latest_macos, arch: :arm) do
          tap.cask_files.each do |path|
            ...
            html_template_name = cask_html_template(name)
        ...
        OnSystem::VALID_OS_ARCH_TAGS.each do |bottle_tag|
          ...
          File.write("api/internal/cask.#{bottle_tag}.json", JSON.generate(json_contents)) unless dry_run

          all_json_contents[bottle_tag.to_sym] = json_contents
        end
      end
    ...
    def self.cask_html_template(title)
      <<~EOS
        ---
        title: '#{title}'
        layout: cask
