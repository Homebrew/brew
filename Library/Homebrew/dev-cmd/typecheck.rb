...
module Homebrew
  module DevCmd
    class Typecheck < AbstractCommand
      ...
      sig { void }
      def trim_rubocop_rbi
        rbi_file = Dir.glob(HOMEBREW_LIBRARY_PATH/"sorbet/rbi/gems/rubocop@*.rbi").first
        ...
        parsed.value.statements.body.each do |node|
          ...
          when Prism::ConstantWriteNode # Keep essential constants.
            nodes_to_keep << node if node.name.to_s.match?(/^[A-Z_]+$/)
          else # Keep other top-level nodes (comments, etc.)
            nodes_to_keep << node
          end
        end

        # Don't rewrite the file if we didn't get rid of anything.
        return if nodes_to_keep.length >= parsed.value.statements.body.length
