...
module Cask
  module Artifact
    # Installs a native Linux package (`.deb`, `.rpm`, `.pkg.tar.*`, `.apk`)
    # using the appropriate system package manager.
    class Package < AbstractArtifact
      sig { returns(String) }
      attr_reader :path
      ...
      def package_name(command:)
        ...
        when ".pkg"
          basename = File.basename(path).sub(/\.pkg\.tar\.\w+$/, "")
          return basename.rpartition("-").first
        else
          raise CaskError, "Unsupported package format: #{File.extname(path)}"
        end
