...
class GitHubRunnerMatrix
  ...
  def active_runner_specs_hash
    ...
    active_runners.map { |runner| runner.spec.to_h }
  end
  ...
  sig { params(runner: GitHubRunner).returns(T::Array[String]) }
  def compatible_matrix_item_names(runner)
    if @dependent_matrix
      compatible_dependent_names(runner)
    else
      compatible_testing_formulae(runner).map(&:name).uniq.sort
    end
  end
end
