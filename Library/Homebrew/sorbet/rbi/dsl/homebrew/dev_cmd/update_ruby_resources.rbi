# typed: true

# DO NOT EDIT MANUALLY
# This is an autogenerated file for dynamic methods in `Homebrew::DevCmd::UpdatePerlResources`.
# Please instead update this file by running `bin/tapioca dsl Homebrew::DevCmd::UpdatePerlResources`.


class Homebrew::DevCmd::UpdateRubyResources
  sig { returns(Homebrew::DevCmd::UpdateRubyResources::Args) }
  def args; end
end

class Homebrew::DevCmd::UpdateRubyResources::Args < Homebrew::CLI::Args
  sig { returns(T::Boolean) }
  def ignore_errors?; end

  sig { returns(T::Boolean) }
  def p?; end

  sig { returns(T::Boolean) }
  def print_only?; end

  sig { returns(T::Boolean) }
  def s?; end

  sig { returns(T::Boolean) }
  def silent?; end
end
