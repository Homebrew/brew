...
class SBOM
  ...
  def generate_packages_json(runtime_dependency_declaration, compiler_declaration, bottling:)
    ...
    source_purl = if (tap_name = source.tap_name)
      "pkg:brew/#{tap_name}/#{name}@#{spec_version}"
    else
      "pkg:brew/#{name}@#{spec_version}"
    end
    ...
    if source.url.present? && (registry_pkg = Homebrew::Vulns::Identify.registry_package(source.url))
      external_refs << {
        referenceCategory: "PACKAGE-MANAGER",
