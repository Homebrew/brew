...
module Homebrew
  module TestBot
    class Formulae < TestFormulae
      ...
      sig {
 params(tap: T.nilable(Tap), git: String, dry_run: T::Boolean, fail_fast: T::Boolean, verbose: T::Boolean,
        output_paths: T::Hash[Symbol, Pathname]).void
      }
      ...
      def bottle_reinstall_formula(formula, new_formula, args:)
        ...
        @bottle_filename = T.let(
          Pathname.new(T.must(bottle_step.output).gsub(%r{.*(\./\S+#{HOMEBREW_BOTTLES_EXTNAME_REGEX}).*}om, '\1')),
          T.nilable(Pathname),
        )
        ...
        bottle_filename = T.must(@bottle_filename)
        bottle_json_filename = T.must(@bottle_json_filename)
