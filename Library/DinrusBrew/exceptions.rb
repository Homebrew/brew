...
class NoSuchKegError < RuntimeError
  ...
  def initialize(name, tap: nil)
    @name = name
    @tap = tap
    message = "Нет такого кега: #{DINRUSBREW_CELLAR}/#{name}"
...
class FormulaOrCaskUnavailableError < RuntimeError
  ...
  def initialize(name)
    ...
    @without_api = DinrusBrew::EnvConfig.no_install_from_api?
    @auto_without_api = DinrusBrew::EnvConfig.automatically_set_no_install_from_api?
  end
  ...
  def did_you_mean
    ...
    similar_formula_names = DinrusBrew.with_no_api_env_if_needed(@without_api) { Formula.fuzzy_search(name) }
...
class TapFormulaOrCaskUnavailableError < FormulaOrCaskUnavailableError
  ...
  def to_s
    s = super
    s += "\nТапните его и попытайтесь снова: brew tap #{tap}" unless tap.installed?
    s
  end
end
...
# Raised when the remote of homebrew/core does not match DINRUSBREW_CORE_GIT_REMOTE.
class TapCoreRemoteMismatchError < TapRemoteMismatchError
  def message
    <<~EOS
      Tap #{name} remote does not match DINRUSBREW_CORE_GIT_REMOTE.
      #{expected_remote} != #{actual_remote}
      Please set DINRUSBREW_CORE_GIT_REMOTE="#{actual_remote}" and run `brew update` instead.
    EOS
  end
end
...
# Raised when another DinrusBrew operation is already in progress.
class OperationInProgressError < RuntimeError
  sig { params(locked_path: Pathname).void }
  def initialize(locked_path)
    full_command = DinrusBrew.running_command_with_args.presence || "brew"
    lock_context = if (env_lock_context = DinrusBrew::EnvConfig.lock_context.presence)
      "\n#{env_lock_context}"
    end
    message = <<~EOS
...
class FormulaConflictError < RuntimeError
  ...
  def message
    ...
    message << <<~EOS
      ...
      Unlinking removes a formula's symlinks from #{DINRUSBREW_PREFIX}. You can
...
class BuildError < RuntimeError
  ...
  sig { returns(T::Array[T.untyped]) }
  def fetch_issues
    return [] if ENV["DINRUSBREW_NO_BUILD_ERROR_ISSUES"].present?
  ...
  def dump(verbose: false)
    ...
    if formula.tap && !OS.unsupported_configuration?
      if formula.tap.official?
        ...
        puts <<~EOS
          If reporting this issue please do so at (not DinrusBrew/brew or DinrusBrew/homebrew-core):
            #{Formatter.url(issues_url)}
        EOS
      else
        puts <<~EOS
          If reporting this issue please do so to (not DinrusBrew/brew or DinrusBrew/homebrew-core):
            #{formula.tap}
        EOS
      end
    else
      puts <<~EOS
        Do not report this issue to DinrusBrew/brew or DinrusBrew/homebrew-core!
      EOS
    end
    ...
    require "diagnostic"
    checks = DinrusBrew::Diagnostic::Checks.new
...
# Raised by `DinrusBrew.install`, `DinrusBrew.reinstall` and `DinrusBrew.upgrade`
...
class HomebrewCurlDownloadStrategyError < CurlDownloadStrategyError
  def initialize(url)
    super "DinrusBrew-installed `curl` is not installed for: #{url}"
  end
end
