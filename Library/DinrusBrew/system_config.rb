...
module SystemConfig
  class << self
    ...
    def homebrew_repo
      GitRepository.new(DINRUSBREW_REPOSITORY)
    end

    sig { returns(String) }
    def branch
      homebrew_repo.branch_name || "(отсутствует)"
    end

    sig { returns(String) }
    def head
      homebrew_repo.head_ref || "(отсутствует)"
    end

    sig { returns(String) }
    def last_commit
      homebrew_repo.last_committed || "никогда"
    end

    sig { returns(String) }
    def origin
      homebrew_repo.origin_url || "(отсутствует)"
    end
    ...
    def dump_tap_config(tap, out = $stdout)
      ...
      if (json_file = DinrusBrew::API::DINRUSBREW_CACHE_API/json_file_name) && json_file.exist?
    ...
    def homebrew_config(out = $stdout)
      out.puts "DINRUSBREW_VERSION: #{DINRUSBREW_VERSION}"
    ...
    def homebrew_env_config(out = $stdout)
      out.puts "DINRUSBREW_PREFIX: #{DINRUSBREW_PREFIX}"
      {
        DINRUSBREW_REPOSITORY: DinrusBrew::DEFAULT_REPOSITORY,
        DINRUSBREW_CELLAR:     DinrusBrew::DEFAULT_CELLAR,
      }.freeze.each do |key, default|
        value = Object.const_get(key)
        out.puts "#{key}: #{value}" if value.to_s != default.to_s
      end

      DinrusBrew::EnvConfig::ENVS.each do |env, hash|
        method_name = DinrusBrew::EnvConfig.env_method_name(env, hash)

        if hash[:boolean]
          out.puts "#{env}: set" if DinrusBrew::EnvConfig.send(method_name)
          next
        end

        value = DinrusBrew::EnvConfig.send(method_name)
        next unless value
      ...
      out.puts "DinrusBrew Ruby: #{describe_homebrew_ruby}"
    end
