...
module Utils
  ...
  module Bottles
    class << self
      ...
      def tag(tag = nil)
        ...
        when Tag
          ...
          @tag ||= Tag.new(
            system: DINRUSBREW_SYSTEM.downcase.to_sym,
            arch:   DINRUSBREW_PROCESSOR.downcase.to_sym,
          )
        end
      end
      ...
      def extname_tag_rebuild(filename)
        DINRUSBREW_BOTTLES_EXTNAME_REGEX.match(filename).to_a
      end
    ...
    class Tag
      ...
      def default_prefix
        if linux?
          DINRUSBREW_LINUX_DEFAULT_PREFIX
        elsif standardized_arch == :arm64
          DINRUSBREW_MACOS_ARM_DEFAULT_PREFIX
        else
          DINRUSBREW_DEFAULT_PREFIX
        end
      end
