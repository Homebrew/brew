...
# Repology API client.
module Repology
  DINRUSBREW_CORE = "homebrew"
  DINRUSBREW_CASK = "homebrew_casks"
  MAX_PAGINATION = 15
  ...
  def self.parse_api_response(limit = nil, last_package = "", repository:)
    ...
    when DINRUSBREW_CORE
      "formulae"
    when DINRUSBREW_CASK
      "casks"
    else
      "packages"
    end
  ...
  def self.latest_version(repositories)
    # The status is "unique" when the package is present only in DinrusBrew, so
    # Repology has no way of knowing if the package is up-to-date.
    is_unique = repositories.find do |repo|
      repo["status"] == "unique"
    end.present?

    return "present only in DinrusBrew" if is_unique
