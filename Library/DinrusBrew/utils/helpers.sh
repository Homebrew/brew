...
ohai() {
  ...
  if [[ -n "${DINRUSBREW_COLOR}" || (-t 1 && -z "${DINRUSBREW_NO_COLOR}") ]]
  then
...
opoo() {
  ...
  if [[ -n "${DINRUSBREW_COLOR}" || (-t 2 && -z "${DINRUSBREW_NO_COLOR}") ]]
  then
...
curl() {
  "${DINRUSBREW_LIBRARY}/DinrusBrew/shims/shared/curl" "$@"
}

git() {
  "${DINRUSBREW_LIBRARY}/DinrusBrew/shims/shared/git" "$@"
}
