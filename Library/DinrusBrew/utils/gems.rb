...
module DinrusBrew
  ...
  DINRUSBREW_BUNDLER_VERSION = "2.5.20"
  ...
  RUBY_BUNDLE_VENDOR_DIRECTORY = (DINRUSBREW_LIBRARY_PATH/"vendor/bundle/ruby").freeze
  ...
  def self.gemfile
    File.join(ENV.fetch("DINRUSBREW_LIBRARY"), "DinrusBrew", "Gemfile")
  end
  ...
  def self.setup_gem_environment!(setup_path: true)
    ...
    ENV["TMPDIR"] = ENV.fetch("DINRUSBREW_TEMP", nil)
  ...
  def self.install_bundler!
    ...
    ENV["BUNDLER_VERSION"] = DINRUSBREW_BUNDLER_VERSION # Set so it correctly finds existing installs
    install_gem_setup_path!(
      "bundler",
      version:               DINRUSBREW_BUNDLER_VERSION,
      executable:            "bundle",
  ...
  def self.install_bundler_gems!(only_warn_on_failure: false, setup_path: true, groups: [])
    ...
    if (homebrew_bundle_user_cache = ENV.fetch("DINRUSBREW_BUNDLE_USER_CACHE", nil))
