...
module Utils
  ...
  module Curl
    ...
    def curl_executable(use_homebrew_curl: false)
      return DINRUSBREW_BREWED_CURL_PATH if use_homebrew_curl

      @curl_executable ||= T.let(DINRUSBREW_SHIMS_PATH/"shared/curl", T.nilable(T.any(Pathname, String)))
    end
    ...
    def curl_args(
      *extra_args,
      connect_timeout: nil,
      max_time: nil,
      retries: DinrusBrew::EnvConfig.curl_retries.to_i,
      retry_max_time: nil,
      show_output: false,
      show_error: true,
      user_agent: nil,
      ...
      if show_output != true
        ...
        args << "--verbose" if DinrusBrew::EnvConfig.curl_verbose?
    ...
    def curl_headers(*args, wanted_headers: [], **options)
      ...
      # This is a workaround for https://github.com/DinrusBrew/brew/issues/18213
    ...
    def curl_check_http_content(url, url_type, specs: {}, user_agents: [:default], referer: nil,
      ...
      user_agents.each do |user_agent|
        loop do
          ...
          attempts += 1
          break if attempts >= DinrusBrew::EnvConfig.curl_retries.to_i
        end
      ...
      unless http_status_ok?(details[:status_code])
        ...
        # https://github.com/DinrusBrew/brew/issues/13789
        ...
        check_github_api = url_type == SharedAudits::URL_TYPE_HOMEPAGE &&
                           ...
                           repo_details &&
                           DinrusBrew::EnvConfig.github_api_token.present?
      ...
      if url.start_with?("https://") && DinrusBrew::EnvConfig.no_insecure_redirect? &&
