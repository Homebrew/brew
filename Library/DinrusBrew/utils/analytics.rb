  ...
  module Analytics
    ...
    class << self
      ...
      def deferred_curl(url, args)
        ...
        if ENV["DINRUSBREW_ANALYTICS_DEBUG"]
      ...
      sig { params(command_instance: DinrusBrew::AbstractCommand).void }
      def report_command_run(command_instance)
        ...
        tags = {
          command:,
          ci:        ENV["CI"].present?,
          devcmdrun: DinrusBrew::EnvConfig.devcmdrun?,
          developer: DinrusBrew::EnvConfig.developer?,
        }
      ...
      def report_test_bot_test(step_command_short, passed)
        return if not_this_run? || disabled?
        return if ENV["DINRUSBREW_TEST_BOT_ANALYTICS"].blank?
        ...
        tags = {
          passed:,
          arch:   DINRUSBREW_PHYSICAL_PROCESSOR,
          os:     DINRUSBREW_SYSTEM,
        }
      ...
      sig { returns(T::Boolean) }
      def disabled?
        return true if DinrusBrew::EnvConfig.no_analytics?
      ...
      sig { returns(T::Boolean) }
      def not_this_run?
        ENV["DINRUSBREW_NO_ANALYTICS_THIS_RUN"].present?
      end

      sig { returns(T::Boolean) }
      def no_message_output?
        # Used by DinrusBrew/install
        ENV["DINRUSBREW_NO_ANALYTICS_MESSAGE_OUTPUT"].present?
      end

      sig { void }
      def messages_displayed!
        DinrusBrew::Settings.write :analyticsmessage, true
        DinrusBrew::Settings.write :caskanalyticsmessage, true
        DinrusBrew::Settings.write :influxanalyticsmessage, true
      end

      sig { void }
      def enable!
        DinrusBrew::Settings.write :analyticsdisabled, false
        delete_uuid!
        messages_displayed!
      end
      ...
      sig { void }
      def disable!
        DinrusBrew::Settings.write :analyticsdisabled, true
        delete_uuid!
      end

      sig { void }
      def delete_uuid!
        DinrusBrew::Settings.delete :analyticsuuid
      end

      sig { params(args: DinrusBrew::Cmd::Info::Args, filter: T.nilable(String)).void }
      def output(args:, filter: nil)
        ...
        begin
          json = DinrusBrew::API::Analytics.fetch category, days
      ...
      sig { params(json: T::Hash[String, T.untyped], args: DinrusBrew::Cmd::Info::Args).void }
      ...
      sig { params(formula: Formula, args: DinrusBrew::Cmd::Info::Args).void }
      def output_github_packages_downloads(formula, args:)
        ...
        formula_url = "https://github.com/DinrusBrew/homebrew-core/pkgs/#{formula_url_suffix}"
        output = Utils::Curl.curl_output("--fail", formula_url)
        return unless output.success?

        formula_version_urls = output.stdout
                                     .scan(%r{/orgs/DinrusBrew/packages/#{formula_url_suffix}\d+\?tag=[^"]+})
                                     .map do |url|
          T.cast(url, String).sub("/orgs/DinrusBrew/packages/", "/DinrusBrew/homebrew-core/pkgs/")
        end
      ...
      def formula_output(formula, args:)
        return if DinrusBrew::EnvConfig.no_analytics? || DinrusBrew::EnvConfig.no_github_api?

        require "api"

        json = DinrusBrew::API::Formula.fetch formula.name
      ...
      sig { params(cask: Cask::Cask, args: DinrusBrew::Cmd::Info::Args).void }
      def cask_output(cask, args:)
        return if DinrusBrew::EnvConfig.no_analytics? || DinrusBrew::EnvConfig.no_github_api?

        require "api"

        json = DinrusBrew::API::Cask.fetch cask.token
        return if json.blank? || json["analytics"].blank?
      ...
      def default_package_tags
        cache[:default_package_tags] ||= begin
          ...
          prefix = DinrusBrew.default_prefix? ? DINRUSBREW_PREFIX.to_s : "custom-prefix"

          # Tags are always strings and must have low cardinality.
          {
            ci:             ENV["CI"].present?,
            prefix:,
            default_prefix: DinrusBrew.default_prefix?,
            developer:      DinrusBrew::EnvConfig.developer?,
            devcmdrun:      DinrusBrew::EnvConfig.devcmdrun?,
            arch:           DINRUSBREW_PHYSICAL_PROCESSOR,
            os:             DINRUSBREW_SYSTEM,
      ...
      def default_package_fields
        cache[:default_package_fields] ||= begin
          version = if (match_data = DINRUSBREW_VERSION.match(/^[\d.]+/))
            suffix = "-dev" if DINRUSBREW_VERSION.include?("-")
      ...
      def config_true?(key)
        DinrusBrew::Settings.read(key) == "true"
      end
