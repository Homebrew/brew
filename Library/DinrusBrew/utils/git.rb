...
module Utils
  ...
  module Git
    ...
    def self.ensure_installed!
      ...
      if CoreTap.instance.installed?
        begin
          ...
          raise "Refusing to install Git on a generic OS." if ENV["DINRUSBREW_TEST_GENERIC_OS"]
    ...
    def self.set_name_email!(author: true, committer: true)
      if DinrusBrew::EnvConfig.git_name
        ENV["GIT_AUTHOR_NAME"] = DinrusBrew::EnvConfig.git_name if author
        ENV["GIT_COMMITTER_NAME"] = DinrusBrew::EnvConfig.git_name if committer
      end

      if DinrusBrew::EnvConfig.git_committer_name && committer
        ENV["GIT_COMMITTER_NAME"] = DinrusBrew::EnvConfig.git_committer_name
      end

      if DinrusBrew::EnvConfig.git_email
        ENV["GIT_AUTHOR_EMAIL"] = DinrusBrew::EnvConfig.git_email if author
        ENV["GIT_COMMITTER_EMAIL"] = DinrusBrew::EnvConfig.git_email if committer
      end

      return unless committer
      return unless DinrusBrew::EnvConfig.git_committer_email

      ENV["GIT_COMMITTER_EMAIL"] = DinrusBrew::EnvConfig.git_committer_email
    end

    def self.setup_gpg!
      gnupg_bin = DINRUSBREW_PREFIX/"opt/gnupg/bin"
