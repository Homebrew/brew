...
# DINRUSBREW_PREFIX is set by extend/ENV/super.rb
# shellcheck disable=SC2154
SHFMT="${DINRUSBREW_PREFIX}/opt/shfmt/bin/shfmt"
if [[ ! -x "${SHFMT}" ]]
then
...
# DINRUSBREW_PREFIX is set by extend/ENV/super.rb
# shellcheck disable=SC2154
DIFF="${DINRUSBREW_PREFIX}/opt/diffutils/bin/diff"
DIFF_ARGS=("-d" "-C" "1")
if [[ ! -x "${DIFF}" ]]
then
  # DINRUSBREW_PATH is set by global.rb
  # shellcheck disable=SC2154
  if [[ -x "$(PATH="${DINRUSBREW_PATH}" command -v diff)" ]]
  then
    DIFF="$(PATH="${DINRUSBREW_PATH}" command -v diff)" # fall back to `diff` in PATH without coloring
  elif [[ -z "${DINRUSBREW_PATH}" && -x "$(command -v diff)" ]]
  then
    # DINRUSBREW_PATH may unset if shfmt.sh is called by vscode
