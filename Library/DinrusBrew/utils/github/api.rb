  ...
  def self.pat_blurb(scopes = ALL_SCOPES)
    ...
    <<~EOS
      Create a GitHub personal access token:
        #{Formatter.url(
          "https://github.com/settings/tokens/new?scopes=#{scopes.join(",")}&description=DinrusBrew",
        )}
      #{Utils::Shell.set_variable_in_profile("DINRUSBREW_GITHUB_API_TOKEN", "your_token_here")}
    EOS
  end
  ...
  module API
    ...
    class AuthenticationFailedError < Error
      def initialize(credentials_type, github_message)
        ...
        when :env_token
          require "utils/formatter"
          <<~EOS
            DINRUSBREW_GITHUB_API_TOKEN may be invalid or expired; check:
    ...
    def self.github_cli_token
      ...
      Utils::UID.drop_euid do
        # Avoid `Formula["gh"].opt_bin` so this method works even with `DINRUSBREW_DISABLE_LOAD_FORMULA`.
        env = {
          "PATH" => PATH.new(DINRUSBREW_PREFIX/"opt/gh/bin", ENV.fetch("PATH")),
    ...
    def self.keychain_username_password
      ...
      Utils::UID.drop_euid do
        ...
        #   https://github.com/DinrusBrew/brew/issues/6862#issuecomment-572610344
    ...
    def self.credentials
      @credentials ||= DinrusBrew::EnvConfig.github_api_token.presence
    ...
    def self.credentials_type
      if DinrusBrew::EnvConfig.github_api_token.present?
        :env_token
    ...
    CREDENTIAL_NAMES = {
      env_token:                  "DINRUSBREW_GITHUB_API_TOKEN",
    ...
    def self.open_rest(
      ...
      return block_given? ? yield({}) : {} if DinrusBrew::EnvConfig.no_github_api?
      ...
      if data
        begin
          ...
          data_tmpfile = Tempfile.new("github_api_post", DINRUSBREW_TEMP)
      ...
      headers_tmpfile = Tempfile.new("github_api_headers", DINRUSBREW_TEMP)
      begin
        if data_tmpfile
