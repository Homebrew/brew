...
module GitHub
  ...
  def self.print_pull_requests_matching(query, only = nil)
    open_or_closed_prs = search_issues(query, is: only, type: "pr", user: "DinrusBrew")
  ...
  def self.fetch_pull_requests(name, tap_remote_repo, state: nil, version: nil)
    return [] if DinrusBrew::EnvConfig.no_github_api?
    ...
    if API.credentials_type == :none
    ...
    elsif state == "open" && ENV["GITHUB_REPOSITORY_OWNER"] == "DinrusBrew"
      ...
      # The PR API is also not very scalable so limit to DinrusBrew CI.
  ...
  def self.multiple_short_commits_exist?(user, repo, commit)
    return false if DinrusBrew::EnvConfig.no_github_api?
  ...
  def self.count_repo_commits(nwo, user, from: nil, to: nil, max: nil)
    odie "Cannot count commits, DINRUSBREW_NO_GITHUB_API set!" if DinrusBrew::EnvConfig.no_github_api?
  ...
  def self.too_many_open_prs?(tap)
    ...
    return false if ENV["DINRUSBREW_TEST_BOT_AUTOBUMP"].present?

    odie "Cannot count PRs, DINRUSBREW_NO_GITHUB_API set!" if DinrusBrew::EnvConfig.no_github_api?
    ...
    begin
      ...
      # Ignore SAML access errors (https://github.com/DinrusBrew/brew/issues/18610)
