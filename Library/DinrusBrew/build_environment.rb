...
class BuildEnvironment
  ...
  KEYS = %w[
    ...
    DINRUSBREW_CC DINRUSBREW_CXX
    CFLAGS CXXFLAGS CPPFLAGS LDFLAGS SDKROOT MAKEFLAGS
    CMAKE_PREFIX_PATH CMAKE_INCLUDE_PATH CMAKE_LIBRARY_PATH CMAKE_FRAMEWORK_PATH
    MACOSX_DEPLOYMENT_TARGET PKG_CONFIG_PATH PKG_CONFIG_LIBDIR
    DINRUSBREW_DEBUG DINRUSBREW_MAKE_JOBS DINRUSBREW_VERBOSE
    all_proxy ftp_proxy http_proxy https_proxy no_proxy
    DINRUSBREW_SVN DINRUSBREW_GIT
    DINRUSBREW_SDKROOT
    MAKE GIT CPP
  ...
  def self.dump(env, out = $stdout)
    keys = self.keys(env)
    keys -= %w[CC CXX OBJC OBJCXX] if env["CC"] == env["DINRUSBREW_CC"]
