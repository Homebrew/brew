...
module FormulaCellarChecks
  ...
  def check_env_path(bin)
    ...
    prefix_bin = (DINRUSBREW_PREFIX/bin.basename)
  ...
  def check_manpages
    ...
    <<~EOS
      ...
      DinrusBrew requires that man pages live under "share".
  ...
  def check_infopages
    ...
    <<~EOS
      ...
      DinrusBrew suggests that info pages live under "share".
  ...
  def check_generic_executables(bin)
    ...
    <<~EOS
      ...
      DinrusBrew suggests that this software is installed to "libexec" and then
      symlinked as needed.
      The offending files are:
        #{generics * "\n  "}
    EOS
  ...
  def check_elisp_root(share, name)
    ...
    <<~EOS
      Emacs Lisp files were linked directly to "#{DINRUSBREW_PREFIX}/share/emacs/site-lisp".
  ...
  def check_shim_references(prefix)
    ...
    matches = []
    keg.each_unique_file_matching(DINRUSBREW_SHIMS_PATH) do |f|
    ...
    <<~EOS
      Files were found with references to the DinrusBrew shims directory.
      The offending files are:
        #{matches * "\n  "}
    EOS
  end
  ...
  def check_python_symlinks(name, keg_only)
    ...
    return if %w[pip3 wheel3].none? do |l|
      link = DINRUSBREW_PREFIX/"bin"/l
      link.exist? && File.realpath(link).start_with?(DINRUSBREW_CELLAR/name)
    end
