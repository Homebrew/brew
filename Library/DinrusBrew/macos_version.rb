...
class MacOSVersion < Version
  ...
  def outdated_release?
    self < DINRUSBREW_MACOS_OLDEST_SUPPORTED
  end

  sig { returns(T::Boolean) }
  def prerelease?
    self >= DINRUSBREW_MACOS_NEWEST_UNSUPPORTED
  end
  ...
  def requires_nehalem_cpu?
    ...
    raise ArgumentError, "Неожиданная архитектура: #{Hardware::CPU.arch}. Это работает только на архитектура Intel."
  end
