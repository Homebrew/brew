...
class Build
  ...
  def install
    ...
    with_env(new_env) do
      ...
      formula.brew(
        ...
        with_env(
          # For head builds, DINRUSBREW_FORMULA_PREFIX should include the commit,
          # which is not known until after the formula has been staged.
          DINRUSBREW_FORMULA_PREFIX:    formula.prefix,
          # https://reproducible-builds.org/docs/build-path/
          DINRUSBREW_FORMULA_BUILDPATH: formula.buildpath,
          # https://reproducible-builds.org/docs/source-date-epoch/
          SOURCE_DATE_EPOCH:          formula.source_modified_time.to_i.to_s,
          # Avoid make getting confused about timestamps.
          # https://github.com/DinrusBrew/homebrew-core/pull/87470
          TZ:                         "UTC0",
        ) do
          formula.patch
          ...
          if args.interactive?
            ohai "Вход в интерактивный режим..."
            puts <<~EOS
              Введите `exit`, чтобы вернуться и закончить установку.
              Установить в этот префикс: #{formula.prefix}
            EOS

            if args.git?
              puts <<~EOS
                Теперь это директория - репозиторий Git. Сделайте редактирование и используйте:
                  git diff | pbcopy,
                чтобы скопировать diff в буфер обмена.
              EOS
            end
  ...
  def fixopt(formula)
    ...
    raise "#{formula.opt_prefix} отсутствует или сломан\nПжлста, переустановите #{formula.full_name}. Пардон :("
  end
end

begin
  ENV.delete("DINRUSBREW_FORBID_PACKAGES_FROM_PATHS")
  args = DinrusBrew::Cmd::InstallCmd.new.args
