...
module DinrusBrew
  module DevCmd
    class UpdateSponsors < AbstractCommand
      ...
      cmd_args do
        description <<~EOS
          Update the list of GitHub Sponsors in the `DinrusBrew/brew` README.
        EOS
      ...
      def run
        ...
        GitHub.sponsorships("DinrusBrew").each do |s|
        ...
        named_sponsors << "many other users and organisations via [GitHub Sponsors](https://github.com/sponsors/DinrusBrew)"

        readme = DINRUSBREW_REPOSITORY/"README.md"
        content = readme.read
        content.gsub!(/(DinrusBrew is generously supported by) .*\Z/m, "\\1 #{named_sponsors.to_sentence}.\n")
        content << "\n#{logo_sponsors.join}\n" if logo_sponsors.presence

        File.write(readme, content)

        diff = system_command "git", args: [
          "-C", DINRUSBREW_REPOSITORY, "diff", "--exit-code", "README.md"
        ]
