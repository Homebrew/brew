    ...
    class Unbottled < AbstractCommand
      ...
      def run
        ...
        DinrusBrew::SimulateSystem.with(os:, arch:) do
          all = args.eval_all?
          if args.total?
            if !all && !DinrusBrew::EnvConfig.eval_all?
              raise UsageError, "`brew unbottled --total` needs `--eval-all` passed or `DINRUSBREW_EVAL_ALL` set!"
            end
      ...
      def formulae_all_installs_from_args(all)
        if args.named.present?
          ...
          if !args.eval_all? && !DinrusBrew::EnvConfig.eval_all?
            raise UsageError,
                  "`brew unbottled --dependents` needs `--eval-all` passed or `DINRUSBREW_EVAL_ALL` set!"
          end
          ...
          analytics = DinrusBrew::API::Analytics.fetch "install", 90

          if analytics.blank?
            raise UsageError,
                  "default sort by analytics data requires " \
                  "`DINRUSBREW_NO_GITHUB_API` and `DINRUSBREW_NO_ANALYTICS` to be unset"
          end
