...
module DinrusBrew
  module DevCmd
    class Test < AbstractCommand
      ...
      sig { override.void }
      def run
        DinrusBrew.install_bundler_gems!(groups: ["formula_test"], setup_path: false)
        ...
        args.named.to_resolved_formulae.each do |f|
          ...
          begin
            exec_args = DINRUSBREW_RUBY_EXEC_ARGS + %W[
              --
              #{DINRUSBREW_LIBRARY_PATH}/test.rb
              #{f.path}
            ...
            if Sandbox.available?
              ...
              sandbox.allow_write_path(DINRUSBREW_PREFIX/"var/cache")
              sandbox.allow_write_path(DINRUSBREW_PREFIX/"var/homebrew/locks")
              sandbox.allow_write_path(DINRUSBREW_PREFIX/"var/log")
              sandbox.allow_write_path(DINRUSBREW_PREFIX/"var/run")
