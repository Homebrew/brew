...
module DinrusBrew
  module DevCmd
    class Sh < AbstractCommand
      cmd_args do
        description <<~EOS
          Enter an interactive shell for DinrusBrew's build environment. Use years-battle-hardened
          build logic to help your `./configure && make && make install`
          and even your `gem install` succeed. Especially handy if you run DinrusBrew
      ...
      def run
        ...
        if superenv?(args.env)
          ...
          ENV["PATH"] = PATH.new(ENV.fetch("PATH")).insert(1, DINRUSBREW_PREFIX/"bin").to_s
        end
        ...
        if args.cmd.present?
          ...
          puts <<~EOS
            Your shell has been configured to use DinrusBrew's build environment;
