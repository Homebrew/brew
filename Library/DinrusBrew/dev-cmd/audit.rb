...
module DinrusBrew
  module DevCmd
    class Audit < AbstractCommand
      cmd_args do
        description <<~EOS
          Check <formula> for DinrusBrew coding style violations. This should be run before
        ...
        switch "--eval-all",
               description: "Evaluate all available formulae and casks, whether installed or not, to audit them. " \
                            "Implied if `DINRUSBREW_EVAL_ALL` is set."
        switch "--new",
               description: "Run various additional style checks to determine if a new formula or cask is eligible " \
                            "for DinrusBrew. This should be used when creating new formulae or casks and implies " \
                            "`--strict` and `--online`."
        switch "--new-formula",
               replacement: "--new",
               disable:     true,
      ...
      def run
        ...
        DinrusBrew.auditing = true
        DinrusBrew.inject_dump_stats!(FormulaAuditor, /^audit_/) if args.audit_debug?
        ...
        DinrusBrew.install_bundler_gems!(groups: gem_groups)

        ENV.activate_extensions!
        ENV.setup_build_environment

        audit_formulae, audit_casks = DinrusBrew.with_no_api_env do # audit requires full Ruby source
          if args.tap
            ...
            if !args.eval_all? && !DinrusBrew::EnvConfig.eval_all?
              # This odisabled should probably stick around indefinitely.
              odisabled "brew audit",
                        "brew audit --eval-all or DINRUSBREW_EVAL_ALL"
            end
            no_named_args = true
            [
        ...
        formula_problems = audit_formulae.sort.each_with_object({}) do |f, problems|
          ...
          errors = os_arch_combinations.flat_map do |os, arch|
            SimulateSystem.with(os:, arch:) do
              ...
              # don't unnecessarily require a full DinrusBrew/core clone.
              fa = if f.core_formula?
                DinrusBrew.with_no_api_env(&audit_proc)
              else
                audit_proc.call
              end
