...
module DinrusBrew
  ...
  bottle do
    <% if [DINRUSBREW_BOTTLE_DEFAULT_DOMAIN.to_s,
           "#{DINRUSBREW_BOTTLE_DEFAULT_DOMAIN}/bottles"].exclude?(root_url) %>
    ...
    <% end %>
      ...
      ALLOWABLE_DINRUSBREW_REPOSITORY_LINKS = T.let([
        %r{#{Regexp.escape(DINRUSBREW_LIBRARY)}/DinrusBrew/os/(mac|linux)/pkgconfig},
      ...
      cmd_args do
        ...
        flag   "--root-url=",
               description: "Use the specified <URL> as the root of the bottle's URL instead of DinrusBrew's default."
        flag   "--root-url-using=",
               description: "Use the specified download strategy class for downloading the bottle's URL instead of " \
                            "DinrusBrew's default."
      ...
      def run
        if args.merge?
          DinrusBrew.install_bundler_gems!(groups: ["ast"])
          return merge
        end

        DinrusBrew.install_bundler_gems!(groups: ["bottle"])
      ...
      def cellar_parameter_needed?(cellar)
        default_cellars = [
          DinrusBrew::DEFAULT_MACOS_CELLAR,
          DinrusBrew::DEFAULT_MACOS_ARM_CELLAR,
          DinrusBrew::DEFAULT_LINUX_CELLAR,
        ]
      ...
      def sudo_purge
        return unless ENV["DINRUSBREW_BOTTLE_SUDO_PURGE"]
      ...
      def formula_ignores(formula)
        ignores = []
        cellar_regex = Regexp.escape(DINRUSBREW_CELLAR)
        prefix_regex = Regexp.escape(DINRUSBREW_PREFIX)
        ...
        # binutils is relocatable for the same reason: https://github.com/DinrusBrew/brew/pull/11899#issuecomment-906804451.
      ...
      def bottle_formula(formula)
        ...
        prefix = DINRUSBREW_PREFIX.to_s
        cellar = DINRUSBREW_CELLAR.to_s
        ...
        keg&.lock do
          ...
          begin
            ...
            prefix_check = if DinrusBrew.default_prefix?(prefix)
            ...
            repository_reference = if DINRUSBREW_PREFIX == DINRUSBREW_REPOSITORY
              DINRUSBREW_LIBRARY
            else
              DINRUSBREW_REPOSITORY
            end.to_s
            if keg_contain?(repository_reference, keg, ignores + ALLOWABLE_DINRUSBREW_REPOSITORY_LINKS)
            ...
            if args.skip_relocation?
              ...
              relocatable = false if keg_contain?(DINRUSBREW_LIBRARY.to_s, keg, ignores, formula_and_runtime_deps_names)
        ...
        json = {
          formula.full_name => {
            "formula" => {
              ...
              "path"             => formula.path.to_s.delete_prefix("#{DINRUSBREW_REPOSITORY}/"),
      ...
      def merge
        ...
        bottles_hash.each do |formula_name, bottle_hash|
          ...
          path = DINRUSBREW_REPOSITORY/bottle_hash["formula"]["path"]
