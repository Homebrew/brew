...
module DinrusBrew
  module DevCmd
    class BumpUnversionedCasks < AbstractCommand
      ...
      sig { override.void }
      def run
        DinrusBrew.install_bundler_gems!(groups: ["bump_unversioned_casks"])

        state_file = if args.state_file.present?
          Pathname(T.must(args.state_file)).expand_path
        else
          DINRUSBREW_CACHE/"bump_unversioned_casks.json"
        end
          ...
          if sha256.present? && last_sha256 != sha256
            ...
            if version
              if cask.version == version
                ...
                begin
                  system_command! DINRUSBREW_BREW_FILE, args: bump_cask_pr_args
