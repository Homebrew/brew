...
module DinrusBrew
  module DevCmd
    class Release < AbstractCommand
      cmd_args do
        description <<~EOS
          Create a new draft DinrusBrew/brew release with the appropriate version number and release notes.
          ...
          *Note:* Requires write access to the DinrusBrew/brew repository.
        EOS
        switch "--major",
      ...
      sig { override.void }
      def run
        safe_system "git", "-C", DINRUSBREW_REPOSITORY, "fetch", "origin" if DinrusBrew::EnvConfig.no_auto_update?

        require "utils/github"

        begin
          latest_release = GitHub.get_latest_release "DinrusBrew", "brew"
        ...
        if args.major? || args.minor?
          ...
          latest_major_minor_release = begin
            GitHub.get_release "DinrusBrew", "brew", "#{latest_version.major_minor}.0"
          rescue GitHub::API::HTTPNotFoundError
            nil
          end
        ...
        if args.major? || args.minor?
          ...
          blog_post_notes = GitHub.generate_release_notes("DinrusBrew", "brew", new_version,
        ...
        release_notes = if args.major? || args.minor?
          "Release notes for this release can be found on the [DinrusBrew blog](https://brew.sh/blog/#{new_version}).\n"
        else
          ""
        end
        release_notes += GitHub.generate_release_notes("DinrusBrew", "brew", new_version,
                                                       previous_tag: latest_version)["body"]

        begin
          release = GitHub.create_or_update_release "DinrusBrew", "brew", new_version, body: release_notes, draft: true
