  ...
  module DevCmd
    class TapNew < AbstractCommand
      ...
      def run
        ...
        odie "Invalid tap name '#{tap}'" unless tap.path.to_s.match?(DINRUSBREW_TAP_PATH_REGEX)
        ...
        readme = <<~MARKDOWN
          ...
          Or, in a [`brew bundle`](https://github.com/DinrusBrew/homebrew-bundle) `Brewfile`:
          ...
          `brew help`, `man brew` or check [DinrusBrew's documentation](https://docs.brew.sh).
        MARKDOWN
        # <!-- vale on -->
        ...
        actions_main = <<~YAML
          ...
          jobs:
            test-bot:
              ...
              runs-on: ${{ matrix.os }}
              steps:
                - name: Set up DinrusBrew
                  id: set-up-homebrew
                  uses: DinrusBrew/actions/setup-homebrew@master

                - name: Cache DinrusBrew Bundler RubyGems
        ...
        pr_pull_env = {
          "DINRUSBREW_GITHUB_API_TOKEN" => "${{ github.token }}",
        }
        if args.github_packages?
          pr_pull_permissions["packages"] = "write"
          pr_pull_env["DINRUSBREW_GITHUB_PACKAGES_TOKEN"] = "${{ github.token }}"
          pr_pull_env["DINRUSBREW_GITHUB_PACKAGES_USER"] = "${{ github.repository_owner }}"
        end
        actions_publish = <<~YAML
          ...
          #{pr_pull_permissions.sort.map { |k, v| "      #{k}: #{v}" }.join("\n")}
              steps:
                ...
                - name: Set up git
                  uses: DinrusBrew/actions/git-user-config@master
          ...
          #{pr_pull_env.sort.map { |k, v| "          #{k}: #{v}" }.join("\n")}
                ...
                - name: Push commits
                  uses: DinrusBrew/actions/git-try-push@master
                  with:
                    token: ${{ github.token }}
                    branch: #{branch}
        ...
        unless args.no_git?
          cd tap.path do |path|
            ...
            if git_owner != Process.uid && git_owner == Process.euid
              # Under DinrusBrew user model, EUID is permitted to execute commands under the UID.
