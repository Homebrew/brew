    ...
    class Create < AbstractCommand
      cmd_args do
        description <<~EOS
          ...
          and open it in the editor. DinrusBrew will attempt to automatically derive the
        ...
        switch "--no-fetch",
               description: "DinrusBrew will not download <URL> to the cache and will thus not add its SHA-256 " \
      ...
      def create_cask
        ...
        cask_path.atomic_write <<~RUBY
          ...
          #                https://docs.brew.sh/Adding-Software-to-DinrusBrew#cask-stanzas
      ...
      def create_formula
        ...
        formula = DinrusBrew.with_no_api_env do
          Formula[fc.name]
        end
        ...
        puts <<~EOS
          Please run the following command before submitting:
            DINRUSBREW_NO_INSTALL_FROM_API=1 brew audit --new #{fc.name}
        EOS
        path
      end
