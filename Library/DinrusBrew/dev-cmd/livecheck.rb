    ...
    class LivecheckCmd < AbstractCommand
      cmd_args do
        description <<~EOS
          ...
          casks to check is taken from `DINRUSBREW_LIVECHECK_WATCHLIST` or
      ...
      sig { override.void }
      def run
        DinrusBrew.install_bundler_gems!(groups: ["livecheck"])

        all = args.eval_all?

        if args.debug? && args.verbose?
          puts args
          puts DinrusBrew::EnvConfig.livecheck_watchlist if DinrusBrew::EnvConfig.livecheck_watchlist.present?
        end

        formulae_and_casks_to_check = DinrusBrew.with_no_api_env do
          if args.tap
      ...
      sig { returns(String) }
      def watchlist_path
        @watchlist_path ||= T.let(File.expand_path(DinrusBrew::EnvConfig.livecheck_watchlist), T.nilable(String))
      end

      sig { returns(T::Boolean) }
      def skip_autobump?
        !(args.autobump? || DinrusBrew::EnvConfig.livecheck_autobump?)
      end
    end
  end
end
