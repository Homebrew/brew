  ...
  module DevCmd
    class UpdateTest < AbstractCommand
      ...
      cmd_args do
        ...
        EOS
        switch "--to-tag",
               description: "Set `DINRUSBREW_UPDATE_TO_TAG` to test updating between tags."
        switch "--keep-tmp",
      ...
      sig { override.void }
      def run
        # Avoid `update-report.rb` tapping DinrusBrew/homebrew-core
        ENV["DINRUSBREW_UPDATE_TEST"] = "1"

        # Avoid accidentally updating when we don't expect it.
        ENV["DINRUSBREW_NO_AUTO_UPDATE"] = "1"

        # Use default behaviours
        ENV["DINRUSBREW_AUTO_UPDATE_SECS"] = nil
        ENV["DINRUSBREW_DEVELOPER"] = nil
        ENV["DINRUSBREW_DEV_CMD_RUN"] = nil
        ENV["DINRUSBREW_MERGE"] = nil
        ENV["DINRUSBREW_NO_UPDATE_CLEANUP"] = nil
        ENV["DINRUSBREW_UPDATE_TO_TAG"] = nil

        branch = if args.to_tag?
          ENV["DINRUSBREW_UPDATE_TO_TAG"] = "1"
          "stable"
        else
          ENV["DINRUSBREW_DEV_CMD_RUN"] = "1"
          "master"
        end
        ...
        chdir "update-test" do
          ...
          # copy DinrusBrew installation
          safe_system "git", "clone", "#{DINRUSBREW_REPOSITORY}/.git", ".",
                      "--branch", "master", "--single-branch"

          # set git origin to another copy
          safe_system "git", "clone", "#{DINRUSBREW_REPOSITORY}/.git", "remote.git",
                      "--bare", "--branch", "master", "--single-branch"
          safe_system "git", "config", "remote.origin.url", "#{curdir}/remote.git"
          ENV["DINRUSBREW_BREW_GIT_REMOTE"] = "#{curdir}/remote.git"
      ...
      def git_tags
        ...
        if tags.blank?
          tags = if (DINRUSBREW_REPOSITORY/".git/shallow").exist?
