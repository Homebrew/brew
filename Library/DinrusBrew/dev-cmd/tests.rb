...
module DinrusBrew
  module DevCmd
    class Tests < AbstractCommand
      ...
      cmd_args do
        description <<~EOS
          Run DinrusBrew's unit and integration tests.
        EOS
        switch "--coverage",
      ...
      def run
        ...
        DinrusBrew.install_bundler_gems!(groups: DinrusBrew.valid_gem_groups - ["sorbet"])

        DINRUSBREW_LIBRARY_PATH.cd do
          setup_environment!
          ...
          parallel_rspec_log_path = if ENV["CI"]
            ...
            "#{DINRUSBREW_CACHE}/#{parallel_rspec_log_name}"
          end
          ...
          bundle_args = ["-I", DINRUSBREW_LIBRARY_PATH/"test"]
          bundle_args += %W[
            --seed #{seed}
            --color
            --require spec_helper
          ...
          # rubocop:disable DinrusBrew/MoveToExtendOS
          unless OS.mac?
          ...
          # rubocop:enable DinrusBrew/MoveToExtendOS
          ...
          ENV["DINRUSBREW_DEBUG"] = "1" if args.debug? # Used in spec_helper.rb to require the "debug" gem.
      ...
      def changed_test_files
        ...
        filestub_regex = %r{Library/DinrusBrew/([\w/-]+).rb}
      ...
      def setup_environment!
        ...
        allowed_test_env << "DINRUSBREW_USE_RUBY_FROM_PATH" if DinrusBrew::EnvConfig.developer?
        DinrusBrew::EnvConfig::ENVS.keys.map(&:to_s).each do |env|
          next if allowed_test_env.include?(env)

          ENV.delete(env)
        end

        # Codespaces DINRUSBREW_PREFIX and /tmp are mounted 755 which makes Ruby warn constantly.
        if (ENV["DINRUSBREW_CODESPACES"] == "true") && (DINRUSBREW_TEMP.to_s == "/tmp")
          # Need to keep this fairly short to avoid socket paths being too long in tests.
          homebrew_prefix_tmp = "/home/linuxbrew/tmp"
          ENV["DINRUSBREW_TEMP"] = homebrew_prefix_tmp
          FileUtils.mkdir_p homebrew_prefix_tmp
          system "chmod", "-R", "g-w,o-w", DINRUSBREW_PREFIX, homebrew_prefix_tmp
        end

        ENV["DINRUSBREW_TESTS"] = "1"
        ENV["DINRUSBREW_NO_AUTO_UPDATE"] = "1"
        ENV["DINRUSBREW_NO_ANALYTICS_THIS_RUN"] = "1"
        ENV["DINRUSBREW_TEST_GENERIC_OS"] = "1" if args.generic?
        ENV["DINRUSBREW_TEST_ONLINE"] = "1" if args.online?
        ENV["DINRUSBREW_SORBET_RUNTIME"] = "1"
        ENV["DINRUSBREW_NO_FORCE_BREW_WRAPPER"] = "1"

        # TODO: remove this and fix tests when possible.
        ENV["DINRUSBREW_NO_INSTALL_FROM_API"] = "1"
        ENV.delete("DINRUSBREW_INTERNAL_JSON_V3")

        ENV["USER"] ||= system_command!("id", args: ["-nu"]).stdout.chomp

        # Avoid local configuration messing with tests, e.g. git being configured
        # to use GPG to sign by default
        ENV["HOME"] = "#{DINRUSBREW_LIBRARY_PATH}/test"

        # Print verbose output when requesting debug or verbose output.
        ENV["DINRUSBREW_VERBOSE_TESTS"] = "1" if args.debug? || args.verbose?

        if args.coverage?
          ENV["DINRUSBREW_TESTS_COVERAGE"] = "1"
