...
module DinrusBrew
  module DevCmd
    class Edit < AbstractCommand
      cmd_args do
        description <<~EOS
          Open a <formula>, <cask> or <tap> in the editor set by `EDITOR` or `DINRUSBREW_EDITOR`,
          or open the DinrusBrew repository for editing if no argument is provided.
        EOS
      ...
      sig { override.void }
      def run
        ENV["COLORTERM"] = ENV.fetch("DINRUSBREW_COLORTERM", nil)
        # Recover $TMPDIR for emacsclient
        ENV["TMPDIR"] = ENV.fetch("DINRUSBREW_TMPDIR", nil)

        unless (DINRUSBREW_REPOSITORY/".git").directory?
          odie <<~EOS
        ...
        paths = if args.named.empty?
          ...
          if which_editor(silent: true) == "subl"
            ["--project", DINRUSBREW_REPOSITORY/".sublime/homebrew.sublime-project"]
          else
        ...
        if paths.any? do |path|
             ...
             !DinrusBrew::EnvConfig.no_install_from_api? &&
             !DinrusBrew::EnvConfig.no_env_hints? &&
          ...
          opoo <<~EOS
            ...
            DINRUSBREW_NO_INSTALL_FROM_API is not set.
          EOS
        end
      end
      ...
      def raise_with_message!(path, cask)
        ...
        if (tap_match = Regexp.new("#{DINRUSBREW_TAP_DIR_REGEX.source}$").match(path.to_s))
          ...
          if !CoreCaskTap.instance.installed? && DinrusBrew::API::Cask.all_casks.key?(name)
            ...
            action = "create a new cask"
              ...
              DinrusBrew::API::Formula.all_formulae.key?(name)
