...
module DinrusBrew
  module DevCmd
    class Irb < AbstractCommand
      cmd_args do
        description <<~EOS
          Enter the interactive DinrusBrew Ruby shell.
        EOS
        switch "--examples",
               description: "Show several examples."
        switch "--pry",
               env:         :pry,
               description: "Use Pry instead of IRB. Implied if `DINRUSBREW_PRY` is set."
      end
      ...
      def run
        ...
        if args.pry?
          DinrusBrew.install_bundler_gems!(groups: ["pry"])
          require "pry"
        else
          require "irb"
        end
        ...
        ohai "Interactive DinrusBrew Shell", "Example commands available with: `brew irb --examples`"
        if args.pry?
          ...
          ENV["IRBRC"] = (DINRUSBREW_LIBRARY_PATH/"brew_irbrc").to_s
      ...
      def clean_argv
        global_options = DinrusBrew::CLI::Parser
                         .global_options
