...
module DinrusBrew
  module DevCmd
    class VendorGems < AbstractCommand
      cmd_args do
        description <<~EOS
          Install and commit DinrusBrew's vendored gems.
        EOS
      ...
      sig { override.void }
      def run
        DinrusBrew.install_bundler!

        ENV["BUNDLE_WITH"] = DinrusBrew.valid_gem_groups.join(":")

        ohai "cd #{DINRUSBREW_LIBRARY_PATH}"
        DINRUSBREW_LIBRARY_PATH.cd do
          ...
          if args.non_bundler_gems?
            %w[
              ...
              (DINRUSBREW_LIBRARY_PATH/"vendor/gems").cd do
