...
module DinrusBrew
  module DevCmd
    class Prof < AbstractCommand
      cmd_args do
        description <<~EOS
          Run DinrusBrew with a Ruby profiler. For example, `brew prof readall`.
        EOS
        switch "--stackprof",
      ...
      sig { override.void }
      def run
        DinrusBrew.install_bundler_gems!(groups: ["prof"], setup_path: false)
        ...
        DinrusBrew.setup_gem_environment!

        if args.stackprof?
          with_env DINRUSBREW_STACKPROF: "1" do
            system(*DINRUSBREW_RUBY_EXEC_ARGS, brew_rb, *args.named)
          end
