...
class Requirement
  ...
  def message
    ...
    s = "#{class_name} неудовлетворительно!\n"
    if cask
      s += <<~EOS
        Нужные каски можно установить посредством:
  ...
  def satisfied_result_parent
    ...
    if parent.to_s =~ %r{^#{Regexp.escape(DINRUSBREW_CELLAR)}/([\w+-.@]+)/[^/]+/(s?bin)/?$}o
      parent = DINRUSBREW_PREFIX/"opt/#{Regexp.last_match(1)}/#{Regexp.last_match(2)}"
    end
    parent
  end
  ...
  def modify_build_environment(env: nil, cc: nil, build_bottle: false, bottle_arch: nil)
    ...
    return if ["#{DINRUSBREW_PREFIX}/bin", "#{DINRUSBREW_PREFIX}/bin"].include?(parent.to_s)
