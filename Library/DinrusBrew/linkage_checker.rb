...
class LinkageChecker
  ...
  def display_normal_output
    display_items "Системные библиотеки", @system_dylibs
    display_items "Библиотеки DinrusBrew", @brewed_dylibs
    display_items "Косвенные зависимости по компоновке", @indirect_deps
    display_items "Библиотеки со ссылкой на @rpath", @variable_dylibs
    display_items "Отсутствующие библиотеки", @broken_dylibs
    display_items "Сломанные зависимости", @broken_deps
    display_items "Необъявленные зависимости по компоновке", @undeclared_deps
    display_items "Зависимости без компоновки", @unnecessary_deps
    display_items "Нежелательные системные библиотеки", @unwanted_system_dylibs
    display_items "Файлы с отсутствующим rpath", @files_missing_rpaths
    display_items "Ссылки на @executable_path в библиотеках", @executable_path_dylibs
  end
  ...
  def display_test_output(puts_output: true, strict: false)
    display_items("Отсутствующие библиотеки", @broken_dylibs, puts_output:)
    display_items("Сломанные зависимости", @broken_deps, puts_output:)
    display_items("Нежелательные системные библиотеки", @unwanted_system_dylibs, puts_output:)
    display_items("Конфликтующие библиотеки", @version_conflict_deps, puts_output:)
    return unless strict

    display_items("Косвенные зависимости по компоновке", @indirect_deps, puts_output:)
    display_items("Необъявленные зависимости по компоновке", @undeclared_deps, puts_output:)
    display_items("Файлы с отсутствующим rpath", @files_missing_rpaths, puts_output:)
    display_items "Ссылки на @executable_path в библиотеках", @executable_path_dylibs, puts_output:
  end

  sig { params(test: T::Boolean, strict: T::Boolean).returns(T::Boolean) }
  def broken_library_linkage?(test: false, strict: false)
    raise ArgumentError, "Строгая проверка компоновки требует активации тестового режима." if strict && !test
  ...
  def dylib_to_dep(dylib)
    dylib =~ %r{#{Regexp.escape(DINRUSBREW_PREFIX)}/(opt|Cellar)/([\w+-.@]+)/}o
    Regexp.last_match(2)
  end
  ...
  def harmless_broken_link?(dylib)
    ...
    [
      ...
      "#{DINRUSBREW_PREFIX}/opt/llvm/lib/libc++.1.dylib",
    ].include?(dylib)
  end
  ...
  def resolve_formula(keg)
    ...
    opoo "Формула недоступна: #{keg.name}"
  end
end
