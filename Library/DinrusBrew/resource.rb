...
class Resource
  ...
  def stage(target = nil, debug_symbols: false, &block)
    raise ArgumentError, "Требуется целевая директория или блок" if !target && block.blank?
  ...
  def determine_url_mirrors
    ...
    # glibc-bootstrap
    if url.start_with?("https://github.com/DinrusBrew/glibc-bootstrap/releases/download")
      if (artifact_domain = DinrusBrew::EnvConfig.artifact_domain.presence)
        artifact_url = url.sub("https://github.com", artifact_domain)
        return [artifact_url] if DinrusBrew::EnvConfig.artifact_domain_no_fallback?
      ...
      if DinrusBrew::EnvConfig.bottle_domain != DINRUSBREW_BOTTLE_DEFAULT_DOMAIN
        ...
        extra_urls << "#{DinrusBrew::EnvConfig.bottle_domain}/glibc-bootstrap/#{tag}/#{filename}"
      end
    end

    # PyPI packages: PEP 503 – Simple Repository API <https://peps.python.org/pep-0503>
    if (pip_index_url = DinrusBrew::EnvConfig.pip_index_url.presence)
  ...
  class BottleManifest < Resource
    ...
    def tab
      ...
      raise Error, "Не найден tab в манифесте." if tab.blank?

      begin
        JSON.parse(tab)
      rescue JSON::ParserError
        raise Error, "Не удалось разобрать tab JSON."
      end
    end
    ...
    def manifest_annotations
      ...
      json = begin
        ...
        raise Error, "Загруженный манифест GitHub Packages повреждён или изменён (неполноценный JSON): " \
                     "\n#{cached_download}"
      end

      manifests = json["manifests"]
      raise Error, "Отсутствует раздел 'manifests'." if manifests.blank?

      manifests_annotations = manifests.filter_map { |m| m["annotations"] }
      raise Error, "Отсутствует раздел 'annotations'." if manifests_annotations.blank?
      ...
      raise Error, "Не найден манифест, соответствующий контрольной сумме бутыли." if manifest_annotations.blank?
