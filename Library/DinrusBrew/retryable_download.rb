...
module DinrusBrew
  class RetryableDownload
    ...
    def fetch(verify_download_integrity: true, timeout: nil, quiet: false)
      ...
      unless quiet
        puts "Загружено в: #{download}" unless already_downloaded
      ...
      wait = 2 ** @try
      unless quiet
        what = Utils.pluralize("попытка", tries_remaining, plural: "и", singular: "а")
        ohai "Повтор попытки загрузки через #{wait}с... (#{tries_remaining} #{what} осталось)"
      end
      sleep wait
