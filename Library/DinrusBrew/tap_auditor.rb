  ...
  class TapAuditor
    ...
    def initialize(tap, strict:)
      DinrusBrew.with_no_api_env do
        tap.clear_cache if DinrusBrew::EnvConfig.automatically_set_no_install_from_api?
        @name                      = tap.name
        @path                      = tap.path
    ...
    def audit_json_files
      json_patterns = Tap::DINRUSBREW_TAP_JSON_FILES.map { |pattern| @path/pattern }
      Pathname.glob(json_patterns).each do |file|
        JSON.parse file.read
      rescue JSON::ParserError
        problem "#{file.to_s.delete_prefix("#{@path}/")} содержит неполноценный JSON"
      end
    end
    ...
    def audit_aliases_renames_duplicates
      ...
      problem "Следующее должно быть либо алиасом, либо переименованием, но не тем и другим: #{duplicates.to_sentence}"
    end
...
-   sig { params(list_file: String, list: T.untyped).void }
    def check_formula_list(list_file, list)
      ...
      unless [Hash, Array].include? list.class
        problem <<~EOS
          #{list_file} должен содержать массив JSON
          из имён формул или объектов JSON, маппирующих имена формул в значения.
        EOS
        return
      end
