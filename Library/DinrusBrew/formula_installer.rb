...
class FormulaInstaller
  ...
  ETC_VAR_DIRS = T.let([DINRUSBREW_PREFIX/"etc", DINRUSBREW_PREFIX/"var"].freeze, T::Array[Pathname])
  ...
  def pour_bottle?(output_warning: false)
    ...
    unless bottle.compatible_locations?
      if output_warning
        ...
        opoo <<~EOS
          Строится #{formula.full_name} из исходников, как требует бутыль:
          - DINRUSBREW_CELLAR: #{bottle.cellar} (у вас #{DINRUSBREW_CELLAR})
          - DINRUSBREW_PREFIX: #{prefix} (у вас #{DINRUSBREW_PREFIX})
        EOS
      end
      return false
    end
  ...
  def check_install_sanity
    ...
    if force_bottle? && !pour_bottle?
      raise CannotInstallFormulaError, "--force-bottle передан, но у #{formula.full_name} нет бутыли!"
    end

    if DinrusBrew.default_prefix? &&
       !build_from_source? && !build_bottle? && !formula.head? && formula.tap&.core_tap? &&
       # Integration tests override homebrew-core locations
       ENV["DINRUSBREW_INTEGRATION_TEST"].nil? &&
       !pour_bottle?
      ...
      if message
        message += <<~EOS
          ...
          It is expected behaviour that DinrusBrew will be buggy and slow when building from source.
          Do not create any issues about failures building from source on DinrusBrew's GitHub repositories.
          Do not create any issues building from source even if you think this message is unrelated.
          Any opened issues will be immediately closed without response.
          Do not ask for help from DinrusBrew or its maintainers on social media.
          You may ask for help building from source in DinrusBrew's discussions but are unlikely to receive a response.
    ...
    if DinrusBrew::EnvConfig.developer?
      ...
      recursive_dep_map.each do |dep, recursive_deps|
        if [formula.name, formula.full_name].include?(dep.name)
          cyclic_dependencies << "#{formula.full_name} прямо зависит от себя самой"
        elsif recursive_deps.any? { |rdep| [formula.name, formula.full_name].include?(rdep.name) }
          cyclic_dependencies << "#{formula.full_name} зависит от себя самой через #{dep.name}"
        end
      end

      if cyclic_dependencies.present?
        raise CannotInstallFormulaError, <<~EOS
          #{formula.full_name} содержит рекурсивную зависимость от себя самой:
    ...
    recursive_deps.each do |dep|
      if (tab = Tab.for_formula(dep.to_formula)) && tab.arch.present? && tab.arch.to_s != Hardware::CPU.arch.to_s
        invalid_arch_dependencies << "#{dep} построен для #{tab.arch}"
      end
    ...
    if invalid_arch_dependencies.present?
      raise CannotInstallFormulaError, <<~EOS
        Зависимости #{formula.full_name} не построены для архитектуры ЦПБ #{Hardware::CPU.arch}:
    ...
    raise CannotInstallFormulaError,
          "Нужно выполнить `brew unpin #{pinned_unsatisfied_deps * " "}`, т.к. для установки " \
          "#{formula.full_name} требуется свежая версия закреплённых зависимостей"
  end
  ...
  def build_bottle_postinstall
    ...
    (etc_var_postinstall - @etc_var_preinstall).each do |file|
      Pathname.new(file).cp_path_sub(DINRUSBREW_PREFIX, formula.bottle_prefix)
    end
  end
  ...
  def install
    ...
    start_time = Time.now
    DinrusBrew::Install.perform_build_from_source_checks if !pour_bottle? && DevelopmentTools.installed?

    # Warn if a more recent version of this formula is available in the tap.
    begin
      if !quiet? &&
         formula.pkg_version < (v = Formulary.factory(formula.full_name, force_bottle: force_bottle?).pkg_version)
        opoo "#{formula.full_name} #{v} доступен и более свеж, чем версия #{formula.pkg_version}."
      end
    ...
    formula.deprecated_flags.each do |deprecated_option|
      ...
      opoo "#{formula.full_name}: #{old_flag} депрекирован; вместо него используется #{new_flag}!"
    end

    options = display_options(formula).join(" ")
    oh1 "Устанавливается #{Formatter.identifier(formula.full_name)} #{options}".strip if show_header?
...
on_request: installed_on_request?, options:)
      ...
      # Catch any other types of exceptions as they leave us with nothing installed.
        ...
        ignore_interrupts do
          begin
            ...
            odie <<~EOS
              Не удалось удалить кег #{formula.prefix.basename}! Сделайте это вручную:
    ...
    opoo "Ничего не установлено в #{formula.prefix}" unless formula.latest_version_installed?
    end_time = Time.now
    DinrusBrew.messages.package_installed(formula.name, end_time - start_time)
  end
  ...
  def check_conflicts
    ...
    conflicts = formula.conflicts.select do |c|
      ...
      opoo <<~EOS
        #{formula}: #{e.message}
        'conflicts_with "#{c.name}"' следует удалить из #{formula.path.basename}.
      EOS

      raise if DinrusBrew::EnvConfig.developer?

      $stderr.puts "Отчитайтесь о проблеме по тапу #{formula.tap&.full_name}".squeeze(" ")
      $stderr.puts " (нет DinrusBrew/brew или DinrusBrew/homebrew-core)!" unless formula.core_formula?
      false
    else
  ...
  def install_dependencies(deps)
    if deps.empty? && only_deps?
      puts "Все зависимости для #{formula.full_name} удовлетворительны."
    elsif !deps.empty?
      oh1 "Устанавливаются зависимости для #{formula.full_name}: " \
  ...
  def install_dependency(dep, inherited_options)
    ...
    if df.tap.present? && tab.present? && (tab_tap = tab.source["tap"].presence) &&
      ...
      odie <<~EOS
        #{df} уже установлен из #{tab_tap}!
        Вначале выполните `brew uninstall #{df}`."
      EOS
    end
    ...
    oh1 "Устанавливается зависимость #{formula.full_name}: #{Formatter.identifier(dep.name)}"
    fi.install
    fi.finish
  ...
  def caveats
    ...
    audit_installed if DinrusBrew::EnvConfig.developer?
    ...
    DinrusBrew.messages.record_caveats(formula.name, caveats)
  end
  ...
  def finish
    ...
    ohai "Завершение" if verbose?

    keg = Keg.new(formula.prefix)
    if skip_link?
      unless quiet?
        ohai "Пропускается 'link' по требованию."
        puts "Это можно выполнить вручную посредством:"
    ...
    DinrusBrew::Install.global_post_install

    if build_bottle? || skip_post_install?
      unless quiet?
        if build_bottle?
          ohai "Не выполняется 'post_install', т.к. строится бутыль"
        elsif skip_post_install?
          ohai "Пропускается 'post_install' по требованию."
        end
        puts "Это можно выполнить вручную посредством:"
    ...
    unless build_bottle?
      ...
      sbom.write(validate: DinrusBrew::EnvConfig.developer?)
    end
    ...
    if formula.name == "curl" &&
      ...
      ENV["DINRUSBREW_CURL"] = (formula.opt_bin/"curl").to_s
  ...
  def summary
    s = +""
    s << "#{DinrusBrew::EnvConfig.install_badge}  " unless DinrusBrew::EnvConfig.no_emoji?
    s << "#{formula.prefix.resolved_path}: #{formula.prefix.abv}"
    s << ", построен за #{pretty_duration build_time}" if build_time
    s.freeze
  end
  ...
  def build
    ...
    args = [
      ...
      *DINRUSBREW_RUBY_EXEC_ARGS,
      "--",
      DINRUSBREW_LIBRARY_PATH/"build.rb",
    ...
    raise "Пустая инсталляция" if !formula.prefix.directory? || Keg.new(formula.prefix).empty_installation?
  ...
  def link(keg)
    ...
    unless link_keg
      begin
        ...
        ofail "Не удалось создать #{formula.opt_prefix}"
        puts "Зависимое от #{formula.full_name}, вероятно, не построится."
        puts e
      end
      return
    end
    ...
    if cask_installed_with_formula_name
      ohai "Установлен каск #{formula.name}, пропускается link."
      return
    end

    if keg.linked?
      opoo "Этот кег уже отмечен как линкованный, тем не менее, продолжаем..."
      keg.remove_linked_keg_record
    end

    DinrusBrew::Unlink.unlink_versioned_formulae(formula, verbose: verbose?)

    link_overwrite_backup = {} # Hash: conflict file -> backup file
    backup_dir = DINRUSBREW_CACHE/"Backup"
    ...
    begin
      ...
      if formula.link_overwrite?(conflict_file) && !link_overwrite_backup.key?(conflict_file)
        backup_file = backup_dir/conflict_file.relative_path_from(DINRUSBREW_PREFIX).to_s
        backup_file.parent.mkpath
        FileUtils.mv conflict_file, backup_file
        link_overwrite_backup[conflict_file] = backup_file
        retry
      end
      ofail "Шаг `brew link` завершён неуспешно"
      puts "Формула построена, но не создана символьная ссылка в #{DINRUSBREW_PREFIX}"
      puts e
      puts
      puts "Возможные конфликтующие файлы:"
      ...
      puts e
      puts
      puts "Можно попробовать ещё раз, используя:"
      puts "  brew link #{formula.name}"
      @show_summary_heading = true
    # Handle all other possible exceptions when linking.
    rescue Exception => e # rubocop:disable Lint/RescueException
      ofail "Во время шага `brew link` произошла неожидаемая ошибка."
      puts "Формула построена, но не создана символьная ссылка в  #{DINRUSBREW_PREFIX}"
      puts e
    ...
    opoo "Эти файлы переписаны при шаге `brew link`:"
    puts link_overwrite_backup.keys
    puts
    puts "Они дублированы для бэкапа в: #{backup_dir}"
    @show_summary_heading = true
  end
  ...
  # Handle all possible exceptions when installing service files.
    ...
    puts e
    ofail "Не удалось установить файлы службы"
  ...
  # Rescue all possible exceptions when fixing linkage.
    ...
    ofail "Не удалось исправить компоновку инсталяции."
    puts "Формула построена, но могут быть проблемы сеё использованием или компоновкой с ней "
    puts "дпугих формул."
  ...
  sig { void }
  def clean
    ohai "Очистка" if verbose?
    Cleaner.new(formula).clean
  # Handle all possible exceptions when cleaning does not complete.
  rescue Exception => e # rubocop:disable Lint/RescueException
    opoo "Шаг очистки не завершён успешно."
    puts "Тем не менее, установка прошла отлично, поэтому она будет компонована в ваш префикс."

    require "utils/backtrace"
    odebug e, Utils::Backtrace.clean(e)

    DinrusBrew.failed = true
    @show_summary_heading = true
  end
  ...
  def post_install
    args = [
      ...
      *DINRUSBREW_RUBY_EXEC_ARGS,
      ...
      "--",
      DINRUSBREW_LIBRARY_PATH/"postinstall.rb"
    ]
    ...
    if Sandbox.available?
      ...
      Keg.keg_link_directories.each do |dir|
        sandbox.allow_write_path "#{DINRUSBREW_PREFIX}/#{dir}"
      end
      sandbox.run(*args)
    else
      Utils.safe_fork do
  ...
  # Handle all possible exceptions when postinstall does not complete.
    ...
    opoo "Шаг постустановки не прошёл успешно."
    puts "Можете попробовать ещё раз, используя:"
    puts "  brew postinstall #{formula.full_name}"

    require "utils/backtrace"
    odebug e, Utils::Backtrace.clean(e), always_display: DinrusBrew::EnvConfig.developer?
  ...
  def fetch_dependencies
    ...
    oh1 "Получение зависимостей для #{formula.full_name}: " \
        "#{deps.map(&:first).map { Formatter.identifier(_1) }.to_sentence}",
        truncate: false
  ...
  def fetch
    ...
    oh1 "Получение #{Formatter.identifier(formula.full_name)}".strip
    ...
    check_attestation = if pour_bottle?(output_warning: true)
      ...
      @formula = DinrusBrew::API::Formula.source_download(formula) if formula.loaded_from_api?
    ...
    if check_attestation &&
       DinrusBrew::Attestation.enabled? &&
       formula.tap&.core_tap? &&
       formula.name != "gh"
      ohai "Проверка аттестации для #{formula.name}"
      begin
        DinrusBrew::Attestation.check_core_attestation T.cast(downloadable_object, Bottle)
      rescue DinrusBrew::Attestation::GhIncompatible
        ...
        raise CannotInstallFormulaError, <<~EOS
          ...
          by performing an upgrade before retrying:
            ...
            brew upgrade gh
        EOS
      rescue DinrusBrew::Attestation::GhAuthInvalid
        # Only raise an error if we explicitly opted-in to verification.
        raise CannotInstallFormulaError, <<~EOS if DinrusBrew::EnvConfig.verify_attestations?
          The bottle for #{formula.name} could not be verified.

          This typically indicates an invalid GitHub API token.

          If you have `DINRUSBREW_GITHUB_API_TOKEN` set, check it is correct
        ...
        ENV["DINRUSBREW_NO_VERIFY_ATTESTATIONS"] = "1"
      rescue DinrusBrew::Attestation::GhAuthNeeded
        raise CannotInstallFormulaError, <<~EOS
          The bottle for #{formula.name} could not be verified.

          This typically indicates a missing GitHub API token, which you
          can resolve either by setting `DINRUSBREW_GITHUB_API_TOKEN` or
          by running:

            gh auth login
        EOS
      rescue DinrusBrew::Attestation::MissingAttestationError, DinrusBrew::Attestation::InvalidAttestationError => e
        raise CannotInstallFormulaError, <<~EOS
  ...
  def pour
    ...
    return if cellar == DINRUSBREW_CELLAR.to_s && prefix == DINRUSBREW_PREFIX.to_s

    return unless ENV["DINRUSBREW_RELOCATE_BUILD_PREFIX"]

    keg.relocate_build_prefix(keg, prefix, DINRUSBREW_PREFIX)
  end
  ...
  def forbidden_license_check
    forbidden_licenses = DinrusBrew::EnvConfig.forbidden_licenses.to_s.dup
    ...
    if invalid_licenses.present?
      opoo <<~EOS
        DINRUSBREW_FORBIDDEN_LICENSES contains invalid license identifiers: #{invalid_licenses.to_sentence}
    ...
    owner = DinrusBrew::EnvConfig.forbidden_owner
    owner_contact = if (contact = DinrusBrew::EnvConfig.forbidden_owner_contact.presence)
      "\n#{contact}"
    end
    ...
    unless ignore_deps?
      compute_dependencies.each do |(dep, _options)|
        ...
        raise CannotInstallFormulaError, <<~EOS
          ...
          its licenses were forbidden by #{owner} in `DINRUSBREW_FORBIDDEN_LICENSES`:
    ...
    raise CannotInstallFormulaError, <<~EOS
      #{formula.name}'s licenses are all forbidden by #{owner} in `DINRUSBREW_FORBIDDEN_LICENSES`:
  ...
  def forbidden_tap_check
    ...
    unless ignore_deps?
      compute_dependencies.each do |(dep, _options)|
        ...
        error_message << "has not allowed this tap in `DINRUSBREW_ALLOWED_TAPS`" unless dep_tap.allowed_by_env?
        error_message << " and\n" if !dep_tap.allowed_by_env? && dep_tap.forbidden_by_env?
        error_message << "has forbidden this tap in `DINRUSBREW_FORBIDDEN_TAPS`" if dep_tap.forbidden_by_env?
    ...
    error_message = "The installation of #{formula.full_name} has the tap #{formula_tap}\n" \
                    "but #{owner} "
    error_message << "has not allowed this tap in `DINRUSBREW_ALLOWED_TAPS`" unless formula_tap.allowed_by_env?
    error_message << " and\n" if !formula_tap.allowed_by_env? && formula_tap.forbidden_by_env?
    error_message << "has forbidden this tap in `DINRUSBREW_FORBIDDEN_TAPS`" if formula_tap.forbidden_by_env?
    error_message << ".#{owner_contact}"
  ...
  def forbidden_formula_check
    forbidden_formulae = Set.new(DinrusBrew::EnvConfig.forbidden_formulae.to_s.split)
    ...
    unless ignore_deps?
      compute_dependencies.each do |(dep, _options)|
        ...
        raise CannotInstallFormulaError, <<~EOS
          ...
          but the #{dep_name} formula was forbidden by #{owner} in `DINRUSBREW_FORBIDDEN_FORMULAE`.#{owner_contact}
        EOS
      end
    end
    ...
    raise CannotInstallFormulaError, <<~EOS
      ...
      in `DINRUSBREW_FORBIDDEN_FORMULAE`.#{owner_contact}
    EOS
  end
