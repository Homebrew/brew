...
module OS
  ...
  module Mac
    raise "Loaded OS::Mac on generic OS!" if ENV["DINRUSBREW_TEST_GENERIC_OS"]
    ...
    VERSION = ENV.fetch("DINRUSBREW_MACOS_VERSION").chomp.freeze
    ...
    def self.full_version
      @full_version ||= if (fake_macos = ENV.fetch("DINRUSBREW_FAKE_MACOS", nil)) # for Portable Ruby building
    ...
    #
    # - {https://github.com/DinrusBrew/legacy-homebrew/issues/13}
    # - {https://github.com/DinrusBrew/legacy-homebrew/issues/41}
    # - {https://github.com/DinrusBrew/legacy-homebrew/issues/48}
    def self.macports_or_fink
      ...
      # read-only in order to try out DinrusBrew, but this doesn't work as
    ...
    def self.app_with_bundle_id(*ids)
      ...
      paths.max_by { |bundle_path| DinrusBrew::BundleVersion.from_info_plist(bundle_path/"Contents/Info.plist") }
    end
