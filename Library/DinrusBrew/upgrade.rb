  ...
  module Upgrade
    def self.upgrade_formulae(
      ...
      begin
        ...
        raise CyclicDependencyError, dependency_graph.strongly_connected_components if DinrusBrew::EnvConfig.developer?
      end
    ...
    private_class_method def self.print_upgrade_message(formula, fi_options)
      ...
      oh1 "Идёт апгрейд #{Formatter.identifier(formula.full_specified_name)}"
    ...
    def self.puts_no_installed_dependents_check_disable_message_if_not_already!
      ...
      return if DinrusBrew::EnvConfig.no_installed_dependents_check?
      return if @puts_no_installed_dependents_check_disable_message_if_not_already

      puts "Отключите это поведение, установив DINRUSBREW_NO_INSTALLED_DEPENDENTS_CHECK."
    ...
    def self.check_installed_dependents(
      ...
      if DinrusBrew::EnvConfig.no_installed_dependents_check?
        unless DinrusBrew::EnvConfig.no_env_hints?
          opoo <<~EOS
            DINRUSBREW_NO_INSTALLED_DEPENDENTS_CHECK установлен: не будут проверяться просроченные
            зависимости или зависимости со сломаной компоновкой!
          EOS
        end
        return
      end
      ...
      if skipped_dependents.present?
        opoo <<~EOS
          Следующие зависимости апгрейдированных формул просрочены, но
          не подлежат апгрейду, так как не бутелированы:
            #{skipped_dependents * "\n  "}
        EOS
      end
      ...
      if pinned_dependents.present?
        ...
        opoo "Не подлежат апгрейду #{pinned_dependents.count} закреплённые #{plural}:"
      ...
      if upgradeable_dependents.blank?
        ohai "Нет просроченных зависимостей, подлежащих апгрейду!" unless dry_run
      else
        formula_plural = Utils.pluralize("formula", installed_formulae.count, plural: "e")
        upgrade_verb = dry_run ? "Подлежал бы апгрейду" : "Проходит апгрейд"
        ohai "#{upgrade_verb} #{Utils.pluralize("dependent", upgradeable_dependents.count,
                                                include_count: true)} апгрейдированной #{formula_plural}:"
      ...
      unless dry_run
        oh1 "Проверка зависимостей формул, подлежащих апгрейду..."
        Upgrade.puts_no_installed_dependents_check_disable_message_if_not_already!
      end
      ...
      if broken_dependents.blank?
        if dry_run
          ohai "Не найдено никаких сломанных зависимостей!"
          opoo "Если они сломаны при апгрейде, то их также ждёт апгрейд и переустановка."
        else
          ohai "Не найдено никаких сломанных зависимостей!"
        end
        return
      ...
      if outdated_pinned_broken_dependents.present?
        ...
        onoe "Не переустанавливается #{count} сломанных и просроченных, но закреплённых #{plural}:"
      ...
      if reinstallable_broken_dependents.blank?
        ohai "Нет сломанных зависимостей, подлежащих переустановке!"
      else
        ohai "Переустанавливается #{Utils.pluralize("dependent", reinstallable_broken_dependents.count,
                                             include_count: true)} со сломаной линковкой из исходников:"
        Upgrade.puts_no_installed_dependents_check_disable_message_if_not_already!
