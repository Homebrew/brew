...
class Keg
  ...
  class AlreadyLinkedError < RuntimeError
    def initialize(keg)
      super <<~EOS
        Компоновать #{keg.name} невозможно.
        Уже компонована другая версия: #{keg.linked_keg_record.resolved_path}
      EOS
    end
  end
  ...
  class ConflictError < LinkError
    ...
    def suggestion
      ...
      "уже существует. возможно, вам нужно её удалить:\n  rm '#{dst}'\n"
    else
      <<~EOS
        символьная ссылка, относящаяся к #{conflict.name}.Её можно откомпоновать:
    ...
    def to_s
      s = []
      s << "Не удалось сделать символьную ссылку на #{src}"
      s << "Цель #{dst}" << suggestion
      s << <<~EOS
        Чтобы принудительно это сделать и переписать все конфликтующие файлы:
          brew link --overwrite #{keg.name}

        Чирьы получить список всех файлов, которые будут удалены:
  ...
  class DirectoryNotWritableError < LinkError
    ...
    def to_s
      <<~EOS
        Не удалось сделать символьную ссылку на #{src}
        #{dst.dirname} незаписываемый.
      EOS
    end
  end
  ...
  def self.for(path)
    ...
    if (path = original_path.realpath)
      until path.root?
        return Keg.new(path) if path.parent.parent == DINRUSBREW_CELLAR.realpath

        path = path.parent.realpath # realpath() prevents root? failing
      end
    end
    raise NotAKegError, "#{original_path} не внутри кегв"
  end
  ...
  ).map { |dir| DINRUSBREW_PREFIX/dir }.sort.uniq.freeze
  end
  ...
  def self.must_be_writable_directories
    ...
    %w[
      ...
      var/log
    ].map { |dir| DINRUSBREW_PREFIX/dir } + must_exist_subdirectories + [
      DINRUSBREW_CACHE,
      DINRUSBREW_CELLAR,
      DINRUSBREW_LOCKS,
      DINRUSBREW_LOGS,
  ...
  def initialize(path)
    path = path.resolved_path if path.to_s.start_with?("#{DINRUSBREW_PREFIX}/opt/")
    raise "#{path} неполноценный кег" if path.parent.parent.realpath != DINRUSBREW_CELLAR.realpath
    raise "#{path} не директория" unless path.directory?

    @path = path
    @name = path.parent.basename.to_s
    @linked_keg_record = DINRUSBREW_LINKED_KEGS/name
    @opt_record = DINRUSBREW_PREFIX/"opt/#{name}"
    @oldname_opt_records = []
  ...
  def uninstall(raise_failures: false)
    ...
    odie <<~EOS
      Не удалось удалить кег #{name}! Сделайте это вручную:
        sudo rm -rf #{path}
    EOS
  end
  ...
  def unlink(verbose: false, dry_run: false)
    ...
    keg_directories.each do |dir|
      dir.find do |src|
        dst = DINRUSBREW_PREFIX + src.relative_path_from(path)
  ...
  def oldname_opt_records
    ...
    @oldname_opt_records = if (opt_dir = DINRUSBREW_PREFIX/"opt").directory?
  ...
  # symlinks the contents of path+relative_dir recursively into #{DINRUSBREW_PREFIX}/relative_dir
