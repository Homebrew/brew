...
class MacOSRequirement < Requirement
  ...
  def initialize(tags = [], comparator: ">=")
    @version = begin
      ...
      MacOSVersion.new(DINRUSBREW_MACOS_OLDEST_ALLOWED) if comparator == ">="
    end
  ...
  def minimum_version
    return MacOSVersion.new(DINRUSBREW_MACOS_OLDEST_ALLOWED) if @comparator == "<=" || !version_specified?
  ...
  def message(type: :formula)
    return "Для этого ПО требуется macOS." unless version_specified?

    case @comparator
    when ">="
      "Это ПО не выполняется на версиях macOS старше #{@version.pretty_name}."
    when "<="
      case type
      when :formula
        <<~EOS
