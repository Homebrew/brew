  ...
  class << self
    ...
    def locate(tool)
      ...
      (@locate ||= T.let({}, T.nilable(T::Hash[T.any(String, Symbol), T.untyped]))).fetch(tool) do |key|
        @locate[key] = if File.executable?((path = "/usr/bin/#{tool}"))
          Pathname.new path
        # DinrusBrew GCCs most frequently; much faster to check this before xcrun
        elsif (path = DINRUSBREW_PREFIX/"bin/#{tool}").executable?
          path
        end
      end
    end
    ...
    def gcc_version(cc)
      (@gcc_version ||= T.let({}, T.nilable(T::Hash[String, Version]))).fetch(cc) do
        path = DINRUSBREW_PREFIX/"opt/#{CompilerSelector.preferred_gcc}/bin"/cc
    ...
    def ca_file_handles_most_https_certificates?
      ...
      # older OS versions.
      ENV["DINRUSBREW_SYSTEM_CA_CERTIFICATES_TOO_OLD"].nil?
    end
    ...
    def ca_file_substitution_required?
      (!ca_file_handles_most_https_certificates? || ENV["DINRUSBREW_FORCE_BREWED_CA_CERTIFICATES"].present?) &&
        !(DINRUSBREW_PREFIX/"etc/ca-certificates/cert.pem").exist?
    end

    sig { returns(T::Boolean) }
    def curl_substitution_required?
      !curl_handles_most_https_certificates? && !DINRUSBREW_BREWED_CURL_PATH.exist?
    end
