...
module DinrusBrew
  ...
  module Diagnostic
    ...
    class Checks
      ...
      ############# @!group HELPERS
      # Finds files in `DINRUSBREW_PREFIX` *and* /usr/local.
      # Specify paths relative to a prefix, e.g. "include/foo.h".
      # Sets @found for your convenience.
      def find_relative_paths(*relative_paths)
        @found = [DINRUSBREW_PREFIX, "/usr/local"].uniq.reduce([]) do |found, prefix|
      ...
      def please_create_pull_requests(what = "unsupported configuration")
        <<~EOS
          ...
          It is expected behaviour that DinrusBrew will be buggy and slow.
          Do not create any issues about this on DinrusBrew's GitHub repositories.
          Do not create any issues even if you think this message is unrelated.
          Any opened issues will be immediately closed without response.
          Do not ask for help from DinrusBrew or its maintainers on social media.
          You may ask for help in DinrusBrew's discussions but are unlikely to receive a response.
      ...
      def examine_git_origin(repository_path, desired_origin)
        ...
        if current_origin.nil?
          <<~EOS
            ...
            Without a correctly configured origin, DinrusBrew won't update
          ...
          <<~EOS
            ...
            With a non-standard origin, DinrusBrew won't update properly.
      ...
      def broken_tap(tap)
        ...
        repo = GitRepository.new(DINRUSBREW_REPOSITORY)
      ...
      def check_for_stray_dylibs
        ...
        __check_stray_files "/usr/local/lib", "*.dylib", allow_list, <<~EOS
          ...
          building DinrusBrew formulae and may need to be deleted.
      ...
      def check_tmpdir_sticky_bit
        world_writable = DINRUSBREW_TEMP.stat.mode & 0777 == 0777
        return if !world_writable || DINRUSBREW_TEMP.sticky?

        <<~EOS
          #{DINRUSBREW_TEMP} is world-writable but does not have the sticky bit set.
          To set it, run the following command:
            sudo chmod +t #{DINRUSBREW_TEMP}
        EOS
      end
      alias generic_check_tmpdir_sticky_bit check_tmpdir_sticky_bit

      def check_exist_directories
        return if DINRUSBREW_PREFIX.writable?
      ...
      def check_multiple_cellars
        return if DINRUSBREW_PREFIX.to_s == DINRUSBREW_REPOSITORY.to_s
        return unless (DINRUSBREW_REPOSITORY/"Cellar").exist?
        return unless (DINRUSBREW_PREFIX/"Cellar").exist?

        <<~EOS
          You have multiple Cellars.
          You should delete #{DINRUSBREW_REPOSITORY}/Cellar:
            rm -rf #{DINRUSBREW_REPOSITORY}/Cellar
        EOS
      end
        ...
        paths.each do |p|
          ...
          when "/usr/bin"
            unless @seen_prefix_bin
              ...
              conflicts = Dir["#{DINRUSBREW_PREFIX}/bin/*"]
                          .map { |fn| File.basename fn }
                          .select { |bn| File.exist? "/usr/bin/#{bn}" }

              unless conflicts.empty?
                message = inject_file_list conflicts, <<~EOS
                  /usr/bin occurs before #{DINRUSBREW_PREFIX}/bin in your PATH.
                  This means that system-provided programs will be used instead of those
                  provided by DinrusBrew. Consider setting your PATH so that
                  #{DINRUSBREW_PREFIX}/bin occurs before /usr/bin. Here is a one-liner:
                    #{Utils::Shell.prepend_path_in_profile("#{DINRUSBREW_PREFIX}/bin")}

                  The following tools exist at both paths:
                EOS
              end
            end
          when "#{DINRUSBREW_PREFIX}/bin"
            @seen_prefix_bin = true
          when "#{DINRUSBREW_PREFIX}/sbin"
      ...
      def check_user_path_2
        ...
        <<~EOS
          DinrusBrew's "bin" was not found in your PATH.
      ...
      def check_user_path_3
        ...
        <<~EOS
          DinrusBrew's "sbin" was not found in your PATH but you have installed
          formulae that put executables in #{DINRUSBREW_PREFIX}/sbin.
          Consider setting your PATH for example like so:
            #{Utils::Shell.prepend_path_in_profile("#{DINRUSBREW_PREFIX}/sbin")}
        EOS
      end

      def check_for_symlinked_cellar
        return unless DINRUSBREW_CELLAR.exist?
        return unless DINRUSBREW_CELLAR.symlink?

        <<~EOS
          Symlinked Cellars can cause problems.
          Your DinrusBrew Cellar is a symlink: #{DINRUSBREW_CELLAR}
                          which resolves to: #{DINRUSBREW_CELLAR.realpath}

          The recommended DinrusBrew installations are either:
          (A) Have Cellar be a real directory inside of your DINRUSBREW_PREFIX
          (B) Symlink "bin/brew" into your prefix, but don't symlink "Cellar".

          Older installations of DinrusBrew may have created a symlinked Cellar, but this can
      ...
      def check_git_version
        minimum_version = ENV.fetch("DINRUSBREW_MINIMUM_GIT_VERSION")
        return unless Utils::Git.available?
        ...
        <<~EOS
          ...
          Git #{minimum_version} or newer is required for DinrusBrew.
          Please upgrade:
      ...
      def check_for_git
        ...
        <<~EOS
          ...
          DinrusBrew uses Git for several internal functions and some formulae use Git
      ...
      def check_git_newline_settings
        ...
        autocrlf = DINRUSBREW_REPOSITORY.cd { `git config --get core.autocrlf`.chomp }
      ...
      def check_brew_git_origin
        repo = GitRepository.new(DINRUSBREW_REPOSITORY)
        examine_git_origin(repo, DinrusBrew::EnvConfig.brew_git_remote)
      end
      ...
      def check_coretap_integrity
        ...
        broken_tap(core_tap) || examine_git_origin(core_tap.git_repository, DinrusBrew::EnvConfig.core_git_remote)
      end
      ...
      def check_deprecated_official_taps
        ...
        <<~EOS
          You have the following deprecated, official taps tapped:
            DinrusBrew/homebrew-#{tapped_deprecated_taps.join("\n  DinrusBrew/homebrew-")}
      ...
      def __check_linked_brew(formula)
        formula.installed_prefixes.each do |prefix|
          prefix.find do |src|
            ...
            dst = DINRUSBREW_PREFIX + src.relative_path_from(prefix)
      ...
      def check_missing_deps
        ...
        missing = Set.new
        DinrusBrew::Diagnostic.missing_deps(Formula.installed).each_value do |deps|
          missing.merge(deps)
        end
      ...
      def check_git_status
        ...
        repos = {
          "DinrusBrew/brew"          => DINRUSBREW_REPOSITORY,
          "DinrusBrew/homebrew-core" => CoreTap.instance.path,
        }
        ...
        repos.each do |name, path|
          ...
          message += <<~EOS
            ...
            Stashing returns DinrusBrew to a pristine state but can be undone
      ...
      def check_for_unlinked_but_not_keg_only
        unlinked = Formula.racks.reject do |rack|
          if (DINRUSBREW_LINKED_KEGS/rack.basename).directory?
            true
          else
            begin
      ...
      def check_homebrew_prefix
        return if DinrusBrew.default_prefix?

        <<~EOS
          Your DinrusBrew's prefix is not #{DinrusBrew::DEFAULT_PREFIX}.

          Many of DinrusBrew's bottles (binary packages) can only be used with the default prefix.
          Consider uninstalling DinrusBrew and reinstalling into the default prefix.
      ...
      def check_for_unnecessary_core_tap
        return if DinrusBrew::EnvConfig.developer?
        return if DinrusBrew::EnvConfig.no_install_from_api?
        return if DinrusBrew::EnvConfig.devcmdrun?
      ...
      def check_cask_software_versions
        add_info "DinrusBrew Version", DINRUSBREW_VERSION
      ...
      def check_cask_install_location
        locations = Dir.glob(DINRUSBREW_CELLAR.join("brew-cask", "*")).reverse
      ...
      def check_cask_staging_location
        ...
        add_info "DinrusBrew Cask Staging Location", user_tilde(path.to_s)
      ...
      def check_cask_taps
        ...
        add_info "DinrusBrew Cask Taps:", taps_info
