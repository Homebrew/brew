...
module DinrusBrew
  ...
  class UnversionedCaskChecker
    ...
    def all_versions
      ...
      Dir.mktmpdir("cask-checker", DINRUSBREW_TEMP) do |dir|
        dir = Pathname(dir)
        ...
        pkg_paths.each do |pkg_path|
          Dir.mktmpdir("cask-checker", DINRUSBREW_TEMP) do |extract_dir|
    ...
    def guess_cask_version
      if apps.empty? && pkgs.empty? && qlplugins.empty?
        opoo "Cask #{cask} не содержит никаких приложений, ql-плагинов или инсталяторов PKG."
        return
      end
