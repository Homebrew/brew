# Documentation defined in Library/DinrusBrew/cmd/update-reset.rb

# Replaces the function in Library/DinrusBrew/brew.sh to cache the Git executable to provide
...
homebrew-update-reset() {
  ...
  if [[ -z "${REPOS[*]}" ]]
  then
    REPOS+=("${DINRUSBREW_REPOSITORY}" "${DINRUSBREW_LIBRARY}"/Taps/*/*)
  fi
  ...
  do
    ...
    ohai "Resetting ${DIR}..."
    # DINRUSBREW_* variables here may all set by bin/brew or the user
    # shellcheck disable=SC2154
    if [[ "${DIR}" == "${DINRUSBREW_REPOSITORY}" &&
       (-n "${DINRUSBREW_UPDATE_TO_TAG}" ||
       (-z "${DINRUSBREW_DEVELOPER}" && -z "${DINRUSBREW_DEV_CMD_RUN}")) ]]
    then
