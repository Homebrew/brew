    ...
    class UpdateReport < AbstractCommand
      ...
      def auto_update_header
        @auto_update_header ||= begin
          ohai "Auto-updated DinrusBrew!" if args.auto_update?
          true
        end
      end
      ...
      def output_update_report
        ...
        if CoreTap.instance.installed? && CoreTap.instance.linuxbrew_core? &&
           ENV["DINRUSBREW_LINUXBREW_CORE_MIGRATION"].blank?
          ohai "Re-running `brew update` for linuxbrew-core migration"

          if DinrusBrew::EnvConfig.core_git_remote != DINRUSBREW_CORE_DEFAULT_GIT_REMOTE
            opoo <<~EOS
              DINRUSBREW_CORE_GIT_REMOTE was set: #{DinrusBrew::EnvConfig.core_git_remote}.
              It has been unset for the migration.
              You may need to change this from a linuxbrew-core mirror to a homebrew-core one.

            EOS
          end
          ENV.delete("DINRUSBREW_CORE_GIT_REMOTE")

          if DinrusBrew::EnvConfig.bottle_domain != DINRUSBREW_BOTTLE_DEFAULT_DOMAIN
            opoo <<~EOS
              DINRUSBREW_BOTTLE_DOMAIN was set: #{DinrusBrew::EnvConfig.bottle_domain}.
              It has been unset for the migration.
              You may need to change this from a Linuxbrew package mirror to a DinrusBrew one.

            EOS
          end
          ENV.delete("DINRUSBREW_BOTTLE_DOMAIN")

          ENV["DINRUSBREW_LINUXBREW_CORE_MIGRATION"] = "1"
          FileUtils.rm_f DINRUSBREW_LOCKS/"update"

          update_args = []
          update_args << "--auto-update" if args.auto_update?
          update_args << "--force" if args.force?
          exec DINRUSBREW_BREW_FILE, "update", *update_args
        end

        if ENV["DINRUSBREW_ADDITIONAL_GOOGLE_ANALYTICS_ID"].present?
          opoo "DINRUSBREW_ADDITIONAL_GOOGLE_ANALYTICS_ID is now a no-op so can be unset."
          puts "All DinrusBrew Google Analytics code and data was destroyed."
        end

        if ENV["DINRUSBREW_NO_GOOGLE_ANALYTICS"].present?
          opoo "DINRUSBREW_NO_GOOGLE_ANALYTICS is now a no-op so can be unset."
          puts "All DinrusBrew Google Analytics code and data was destroyed."
        end
        ...
        initial_revision = ENV["DINRUSBREW_UPDATE_BEFORE"].to_s
        current_revision = ENV["DINRUSBREW_UPDATE_AFTER"].to_s
        ...
        if initial_revision != current_revision
          ...
          new_tag = Utils.popen_read(
            "git", "-C", DINRUSBREW_REPOSITORY, "tag", "--list", "--sort=-version:refname", "*.*"
          ).lines.first.chomp

          Settings.write "latesttag", new_tag if new_tag != old_tag

          if new_tag == old_tag
            ohai "Updated DinrusBrew from #{shorten_revision(initial_revision)} " \
                 "to #{shorten_revision(current_revision)}."
          elsif old_tag.blank?
            ohai "Updated DinrusBrew from #{shorten_revision(initial_revision)} " \
                 "to #{new_tag} (#{shorten_revision(current_revision)})."
          else
            ohai "Updated DinrusBrew from #{old_tag} (#{shorten_revision(initial_revision)}) " \
                 "to #{new_tag} (#{shorten_revision(current_revision)})."
          end
        end

        # Check if we can parse the JSON and do any Ruby-side follow-up.
        unless DinrusBrew::EnvConfig.no_install_from_api?
          DinrusBrew::API::Formula.write_names_and_aliases
          DinrusBrew::API::Cask.write_names
        end

        DinrusBrew.failed = true if ENV["DINRUSBREW_UPDATE_FAILED"]
        return if DinrusBrew::EnvConfig.disable_load_formula?
        ...
        Tap.installed.each do |tap|
          ...
          next if (tap.core_tap? || tap.core_cask_tap?) && !DinrusBrew::EnvConfig.no_install_from_api?

          if ENV["DINRUSBREW_MIGRATE_LINUXBREW_FORMULAE"].present? && tap.core_tap? &&
        ...
        unless DinrusBrew::EnvConfig.no_install_from_api?
          api_cache = DinrusBrew::API::DINRUSBREW_CACHE_API
        ...
        if updated
          if hub.empty?
            ...
            if ENV.fetch("DINRUSBREW_UPDATE_REPORT_ONLY_INSTALLED", false)
              opoo "DINRUSBREW_UPDATE_REPORT_ONLY_INSTALLED is now the default behaviour, " \
        ...
        elsif !args.auto_update? && !ENV["DINRUSBREW_UPDATE_FAILED"] && !ENV["DINRUSBREW_MIGRATE_LINUXBREW_FORMULAE"]
        ...
        failed_fetch_dirs = ENV["DINRUSBREW_MISSING_REMOTE_REF_DIRS"]&.split("\n")
        ...
        if old_tag.blank? || new_major_version > old_major_version || new_minor_version > old_minor_version
          puts <<~EOS
            The #{new_major_version}.#{new_minor_version}.0 release notes are available on the DinrusBrew Blog:
        ...
        puts <<~EOS
          The #{new_tag} changelog can be found at:
            #{Formatter.url("https://github.com/DinrusBrew/brew/releases/tag/#{new_tag}")}
        EOS
      end
      ...
      def shorten_revision(revision)
        Utils.popen_read("git", "-C", DINRUSBREW_REPOSITORY, "rev-parse", "--short", revision).chomp
      end

      def tap_or_untap_core_taps_if_necessary
        return if ENV["DINRUSBREW_UPDATE_TEST"]
        ...
        if DinrusBrew::EnvConfig.no_install_from_api?
          ...
          ENV["DINRUSBREW_UPDATE_BEFORE_DINRUSBREW_DINRUSBREW_CORE"] = revision
          ENV["DINRUSBREW_UPDATE_AFTER_DINRUSBREW_DINRUSBREW_CORE"] = revision
        else
          return if DinrusBrew::EnvConfig.developer? || ENV["DINRUSBREW_DEV_CMD_RUN"]
          return if ENV["DINRUSBREW_GITHUB_HOSTED_RUNNER"] || ENV["GITHUB_ACTIONS_DINRUSBREW_SELF_HOSTED"]
          return if (DINRUSBREW_PREFIX/".homebrewdocker").exist?
      ...
      def link_completions_manpages_and_docs(repository = DINRUSBREW_REPOSITORY)
        command = "brew update"
      ...
      def analytics_message
        ...
        if Utils::Analytics.disabled? && !Utils::Analytics.influx_message_displayed?
          ohai "DinrusBrew's analytics have entirely moved to our InfluxDB instance in the EU."
          ...
          ENV["DINRUSBREW_NO_ANALYTICS_THIS_RUN"] = "1"
          # Use the shell's audible bell.
          print "\a"

          # Use an extra newline and bold to avoid this being missed.
          ohai "DinrusBrew collects anonymous analytics."
          puts <<~EOS
      ...
      def donation_message
        ...
        ohai "DinrusBrew is run entirely by unpaid volunteers. Please consider donating:"
        puts "  #{Formatter.url("https://github.com/DinrusBrew/brew#donations")}\n\n"
      ...
      def install_from_api_message
        ...
        no_install_from_api_set = DinrusBrew::EnvConfig.no_install_from_api? &&
                                  !DinrusBrew::EnvConfig.automatically_set_no_install_from_api?
        return unless no_install_from_api_set

        ohai "You have DINRUSBREW_NO_INSTALL_FROM_API set"
        puts "DinrusBrew >=4.1.0 is dramatically faster and less error-prone when installing"
        puts "from the JSON API. Please consider unsetting DINRUSBREW_NO_INSTALL_FROM_API."
...
class Reporter
  ...
  def initialize(tap, api_names_txt: nil, api_names_before_txt: nil, api_dir_prefix: nil)
    ...
    if installed_from_api?(api_names_txt, api_names_before_txt, api_dir_prefix)
      ...
      initial_revision_var = "DINRUSBREW_UPDATE_BEFORE#{tap.repository_var_suffix}"
      @initial_revision = ENV[initial_revision_var].to_s
      raise ReporterRevisionUnsetError, initial_revision_var if @initial_revision.empty?

      current_revision_var = "DINRUSBREW_UPDATE_AFTER#{tap.repository_var_suffix}"
  ...
  def migrate_tap_migration
    (report[:D] + report[:DC]).each do |full_name|
      ...
      if report[:DC].include? full_name
        next unless (DINRUSBREW_PREFIX/"Caskroom"/new_name).exist?

        new_tap = Tap.fetch(new_tap_name)
        new_tap.ensure_installed!
        ohai "#{name} has been moved to DinrusBrew.", <<~EOS
          To uninstall the cask, run:
            brew uninstall --cask --force #{name}
        EOS
        next if (DINRUSBREW_CELLAR/new_name.split("/").last).directory?

        ohai "Installing #{new_name}..."
        system DINRUSBREW_BREW_FILE, "install", new_full_name
        begin
          unless Formulary.factory(new_full_name).keg_only?
            system DINRUSBREW_BREW_FILE, "link", new_full_name, "--overwrite"
          end
      ...
      next unless (dir = DINRUSBREW_CELLAR/name).exist? # skip if formula is not installed.
      ...
      if new_tap_name.start_with?("homebrew/cask")
        if new_tap.installed? && (DINRUSBREW_PREFIX/"Caskroom").directory?
          ohai "#{name} has been moved to DinrusBrew Cask."
          ohai "brew unlink #{name}"
          system DINRUSBREW_BREW_FILE, "unlink", name
          ohai "brew cleanup"
          system DINRUSBREW_BREW_FILE, "cleanup"
          ohai "brew install --cask #{new_name}"
          system DINRUSBREW_BREW_FILE, "install", "--cask", new_name
          ohai <<~EOS
            #{name} has been moved to DinrusBrew Cask.
            The existing keg has been unlinked.
            Please uninstall the formula when convenient by running:
              brew uninstall --force #{name}
          EOS
        else
          ohai "#{name} has been moved to DinrusBrew Cask.", <<~EOS
  ...
  def migrate_formula_rename(force:, verbose:)
    Formula.installed.each do |formula|
      ...
      oldnames_to_migrate = formula.oldnames.select do |oldname|
        oldname_rack = DINRUSBREW_CELLAR/oldname
...
class ReporterHub
  ...
  def dump(auto_update: false)
    unless DinrusBrew::EnvConfig.no_update_report_new?
  ...
  def dump_new_cask_report
    return if DinrusBrew::SimulateSystem.simulating_or_running_on_linux?
  ...
  def installed?(formula)
    (DINRUSBREW_CELLAR/formula.split("/").last).directory?
  end
