...
module DinrusBrew
  module Cmd
    class Info < AbstractCommand
      ...
      cmd_args do
        description <<~EOS
          Display brief statistics for your DinrusBrew installation.
          If a <formula> or <cask> is provided, show summary of information about it.
        EOS
        switch "--analytics",
               description: "List global DinrusBrew analytics data or, if specified, installation and " \
                            "build error data for <formula> (provided neither `$DINRUSBREW_NO_ANALYTICS` " \
                            "nor `$DINRUSBREW_NO_GITHUB_API` are set)."
        flag   "--days=",
        ...
        switch "--eval-all",
               ...
               description: "Evaluate all available formulae and casks, whether installed or not, to print their " \
                            "JSON. Implied if `$DINRUSBREW_EVAL_ALL` is set."
        switch "--variations",
               depends_on:  "--json",
      ...
      def print_statistics
        ...
        puts "#{Utils.pluralize("keg", count, include_count: true)}, #{DINRUSBREW_CELLAR.dup.abv}"
      end
