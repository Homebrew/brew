# Documentation defined in Library/DinrusBrew/cmd/vendor-install.rb

# DINRUSBREW_ARTIFACT_DOMAIN, DINRUSBREW_ARTIFACT_DOMAIN_NO_FALLBACK, DINRUSBREW_BOTTLE_DOMAIN, DINRUSBREW_CACHE,
# DINRUSBREW_CURLRC, DINRUSBREW_DEVELOPER, DINRUSBREW_DEBUG, DINRUSBREW_VERBOSE are from the user environment
# DINRUSBREW_PORTABLE_RUBY_VERSION is set by utils/ruby.sh
# DINRUSBREW_LIBRARY, DINRUSBREW_PREFIX are set by bin/brew
# DINRUSBREW_CURL, DINRUSBREW_GITHUB_PACKAGES_AUTH, DINRUSBREW_LINUX, DINRUSBREW_LINUX_MINIMUM_GLIBC_VERSION, DINRUSBREW_MACOS,
# DINRUSBREW_PHYSICAL_PROCESSOR, DINRUSBREW_PROCESSOR, DINRUSBREW_USER_AGENT_CURL are set by brew.sh
...
# Built from https://github.com/DinrusBrew/homebrew-portable-ruby.
set_ruby_variables() {
  ...
  if [[ -n "${DINRUSBREW_MACOS}" && "${VENDOR_PHYSICAL_PROCESSOR}" == "arm64" && "${DINRUSBREW_PREFIX}" == "/usr/local" ]]
  then
  ...
  ruby_PLATFORMINFO="${DINRUSBREW_LIBRARY}/DinrusBrew/vendor/portable-ruby-${ruby_PROCESSOR}-${ruby_OS}"
  ...
  then
    ruby_FILENAME="portable-ruby-${DINRUSBREW_PORTABLE_RUBY_VERSION}.${ruby_TAG}.bottle.tar.gz"
    ruby_URLs=()
    if [[ -n "${DINRUSBREW_ARTIFACT_DOMAIN}" ]]
    then
      ruby_URLs+=("${DINRUSBREW_ARTIFACT_DOMAIN}/v2/homebrew/portable-ruby/portable-ruby/blobs/sha256:${ruby_SHA}")
      if [[ -n "${DINRUSBREW_ARTIFACT_DOMAIN_NO_FALLBACK}" ]]
      then
        ruby_URL="${ruby_URLs[0]}"
        return
      fi
    fi
    if [[ -n "${DINRUSBREW_BOTTLE_DOMAIN}" ]]
    then
      ruby_URLs+=("${DINRUSBREW_BOTTLE_DOMAIN}/bottles-portable-ruby/${ruby_FILENAME}")
    fi
    ruby_URLs+=(
      "https://ghcr.io/v2/homebrew/portable-ruby/portable-ruby/blobs/sha256:${ruby_SHA}"
      "https://github.com/DinrusBrew/homebrew-portable-ruby/releases/download/${DINRUSBREW_PORTABLE_RUBY_VERSION}/${ruby_FILENAME}"
    )
    ruby_URL="${ruby_URLs[0]}"
  fi
}
...
check_linux_glibc_version() {
  if [[ -z "${DINRUSBREW_LINUX}" || -z "${DINRUSBREW_LINUX_MINIMUM_GLIBC_VERSION}" ]]
  then
    return 0
  fi
  ...
  local minimum_required_major="${DINRUSBREW_LINUX_MINIMUM_GLIBC_VERSION%.*}"
  local minimum_required_minor="${DINRUSBREW_LINUX_MINIMUM_GLIBC_VERSION#*.}"
  ...
  then
    ...
    then
      odie "Vendored tools require system Glibc ${DINRUSBREW_LINUX_MINIMUM_GLIBC_VERSION} or later (yours is ${glibc_version})."
    fi
  else
...
fetch() {
  ...
  # DINRUSBREW_CURLRC isn't misspelt here
  # shellcheck disable=SC2153
  if [[ -z "${DINRUSBREW_CURLRC}" ]]
  then
    curl_args[${#curl_args[*]}]="-q"
  elif [[ "${DINRUSBREW_CURLRC}" == /* ]]
  then
    curl_args+=("-q" "--config" "${DINRUSBREW_CURLRC}")
  fi
  ...
  curl_args+=(
    ...
    --header "Authorization: ${DINRUSBREW_GITHUB_PACKAGES_AUTH}"
  )

  if [[ -n "${DINRUSBREW_QUIET}" ]]
  then
    curl_args[${#curl_args[*]}]="--silent"
  elif [[ -z "${DINRUSBREW_VERBOSE}" ]]
  then
  ...
  mkdir -p "${DINRUSBREW_CACHE}"
  [[ -n "${DINRUSBREW_QUIET}" ]] || ohai "Downloading ${VENDOR_URL}" >&2
  if [[ -f "${CACHED_LOCATION}" ]]
  then
    [[ -n "${DINRUSBREW_QUIET}" ]] || echo "Already downloaded: ${CACHED_LOCATION}" >&2
  else
    for url in "${VENDOR_URLs[@]}"
    do
      [[ -n "${DINRUSBREW_QUIET}" || -n "${first_try}" ]] || ohai "Downloading ${url}" >&2
      first_try=''
      if [[ -f "${temporary_path}" ]]
      then
        # DINRUSBREW_CURL is set by brew.sh (and isn't misspelt here)
        # shellcheck disable=SC2153
        "${DINRUSBREW_CURL}" "${curl_args[@]}" -C - "${url}" -o "${temporary_path}"
        if [[ $? -eq 33 ]]
        then
          [[ -n "${DINRUSBREW_QUIET}" ]] || echo "Trying a full download" >&2
          rm -f "${temporary_path}"
          "${DINRUSBREW_CURL}" "${curl_args[@]}" "${url}" -o "${temporary_path}"
        fi
      else
        "${DINRUSBREW_CURL}" "${curl_args[@]}" "${url}" -o "${temporary_path}"
      fi
...
GitHub (used for DinrusBrew updates and binary packages).
EOS
    fi
...
install() {
  ...
  safe_cd "${VENDOR_DIR}"
  [[ -n "${DINRUSBREW_QUIET}" ]] || ohai "Pouring ${VENDOR_FILENAME}" >&2
  tar "${tar_args}" "${CACHED_LOCATION}"

  if [[ "${VENDOR_PROCESSOR}" != "${DINRUSBREW_PROCESSOR}" ]] ||
     [[ "${VENDOR_PHYSICAL_PROCESSOR}" != "${DINRUSBREW_PHYSICAL_PROCESSOR}" ]]
  then
    return 0
  fi
...
homebrew-vendor-install() {
  ...
  then
    VENDOR_PROCESSOR="${DINRUSBREW_PROCESSOR}"
  fi
  ...
  then
    odie "No DinrusBrew ${VENDOR_NAME} ${VENDOR_VERSION} available for ${DINRUSBREW_PROCESSOR} processors!"
  fi
  ...
  CACHED_LOCATION="${DINRUSBREW_CACHE}/${VENDOR_FILENAME}"
