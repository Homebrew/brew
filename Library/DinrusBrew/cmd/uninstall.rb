    ...
    class UninstallCmd < AbstractCommand
      ...
      def run
        ...
        if ENV["DINRUSBREW_AUTOREMOVE"].present?
          opoo "DINRUSBREW_AUTOREMOVE is now a no-op as it is the default behaviour. " \
               "Set DINRUSBREW_NO_AUTOREMOVE=1 to disable it."
        end
        Cleanup.autoremove unless DinrusBrew::EnvConfig.no_autoremove?
      end
    end
  end
end
