    ...
    class InstallCmd < AbstractCommand
      cmd_args do
        description <<~EOS
          ...
          Unless `$DINRUSBREW_NO_INSTALLED_DEPENDENTS_CHECK` is set, `brew upgrade` or `brew reinstall` will be run for
          outdated dependents and dependents with broken linkage, respectively.

          Unless `$DINRUSBREW_NO_INSTALL_CLEANUP` is set, `brew cleanup` will then be run for
          the installed formulae or, every 30 days, for all formulae.

          Unless `$DINRUSBREW_NO_INSTALL_UPGRADE` is set, `brew install` <formula> will upgrade <formula> if it
        ...
        [
          ...
          [:flag, "--env=", {
            description: "Disabled other than for internal DinrusBrew use.",
            hidden:      true,
          }],
          [:switch, "--ignore-dependencies", {
            description: "An unsupported DinrusBrew development option to skip installing any dependencies of any " \
                         "kind. If the dependencies are not already present, the formula will have issues. If " \
                         "you're not developing DinrusBrew, consider adjusting your PATH rather than using this " \
                         "option.",
          }],
          ...
          [:flag, "--cc=", {
            description: "Attempt to compile using the specified <compiler>, which should be the name of the " \
                         ...
                         "accept compilers that are provided by DinrusBrew or bundled with macOS. Please do not " \
          ...
          [:switch, "-i", "--interactive", {
            description: "Download and patch <formula>, then open a shell. This allows the user to " \
                         ...
                         "package into a DinrusBrew package.",
          }],
          [:switch, "-g", "--git", {
      ...
      def run
        ...
        if args.ignore_dependencies?
          opoo <<~EOS
            #{Tty.bold}`--ignore-dependencies` is an unsupported DinrusBrew developer option!#{Tty.reset}
        ...
        if casks.any?
          ...
          if !DinrusBrew::EnvConfig.no_install_upgrade? && installed_casks.any?
        ...
        formulae = DinrusBrew::Attestation.sort_formulae_for_install(formulae) if DinrusBrew::Attestation.enabled?
        ...
        if build_flags.present? && !DinrusBrew::EnvConfig.developer?
        ...
        DinrusBrew.messages.display_messages(display_times: args.display_times?)
        ...
        $stderr.puts Utils::Backtrace.clean(e) if DinrusBrew::EnvConfig.developer?
        ofail e.message
