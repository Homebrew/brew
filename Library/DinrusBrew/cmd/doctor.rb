    ...
    class Doctor < AbstractCommand
      cmd_args do
        description <<~EOS
          ...
          Please note that these warnings are just used to help the DinrusBrew maintainers
          with debugging if you file an issue. If everything you use DinrusBrew for
      ...
      sig { override.void }
      def run
        DinrusBrew.inject_dump_stats!(Diagnostic::Checks, /^check_*/) if args.audit_debug?
        ...
        methods.each do |method|
          ...
          if first_warning
            $stderr.puts <<~EOS
              #{Tty.bold}Please note that these warnings are just used to help the DinrusBrew maintainers
              with debugging if you file an issue. If everything you use DinrusBrew for is
        ...
        puts "Your system is ready to brew." if !DinrusBrew.failed? && !args.quiet?
      end
    end
  end
end
