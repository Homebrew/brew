...
module DinrusBrew
  module Cmd
    class Log < AbstractCommand
      ...
      cmd_args do
        description <<~EOS
          Show the `git log` for <formula> or <cask>, or show the log for the DinrusBrew repository
      ...
      def git_log(cd_dir, path = nil, tap = nil)
        cd cd_dir do
          ...
          elsif cd_dir == DINRUSBREW_REPOSITORY
            name = "DinrusBrew/brew"
            git_cd = "$(brew --repo)"
          else
            name, git_cd = cd_dir
          end
