  ...
  module Cmd
    class ReadallCmd < AbstractCommand
      cmd_args do
        ...
        switch "--syntax",
               description: "Syntax-check all of DinrusBrew's Ruby files (if no <tap> is passed)."
        switch "--eval-all",
      ...
      def run
        DinrusBrew.with_no_api_env do
          if args.syntax? && args.no_named?
            scan_files = "#{DINRUSBREW_LIBRARY_PATH}/**/*.rb"
            ruby_files = Dir.glob(scan_files).grep_v(%r{/(vendor)/})

            DinrusBrew.failed = true unless Readall.valid_ruby_syntax?(ruby_files)
          end
          ...
          taps = if args.no_named?
            if !args.eval_all? && !DinrusBrew::EnvConfig.eval_all?
              raise UsageError, "`brew readall` needs a tap or `--eval-all` passed or `$DINRUSBREW_EVAL_ALL` set!"
            end
          ...
          taps.each do |tap|
            DinrusBrew.failed = true unless Readall.valid_tap?(tap, **options)
          end
        end
      end
    end
