  ...
  module Cmd
    class SearchCmd < AbstractCommand
      ...
      cmd_args do
        ...
        switch "--eval-all",
               ...
               description: "Evaluate all available formulae and casks, whether installed or not, to search their " \
                            "descriptions. Implied if `$DINRUSBREW_EVAL_ALL` is set."
        switch "--pull-request",
      ...
      def run
        ...
        if args.desc?
          if !args.eval_all? && !DinrusBrew::EnvConfig.eval_all? && DinrusBrew::EnvConfig.no_install_from_api?
            raise UsageError, "`brew search --desc` needs `--eval-all` passed or `$DINRUSBREW_EVAL_ALL` set!"
          end
