    ...
    class Shellenv < AbstractCommand
      ...
      cmd_args do
        description <<~EOS
          ...
          Print export statements. When run in a shell, this installation of DinrusBrew will be added to your `PATH`, `MANPATH`, and `INFOPATH`.

          The variables `$DINRUSBREW_PREFIX`, `$DINRUSBREW_CELLAR` and `$DINRUSBREW_REPOSITORY` are also exported to avoid
          querying them multiple times.
          To help guarantee idempotence, this command produces no output when DinrusBrew's `bin` and `sbin` directories
