    ...
    class List < AbstractCommand
      ...
      def run
        if args.full_name? &&
          ...
          if !args.cask? && DINRUSBREW_CELLAR.exist? && DINRUSBREW_CELLAR.children.any?
            ohai "Formulae" if $stdout.tty? && !args.formula?
            safe_system "ls", *ls_args, DINRUSBREW_CELLAR
      ...
      def filtered_list
        names = if args.no_named?
          ...
          racks.select do |rack|
            DinrusBrew.failed = true unless rack.exist?
            rack.exist?
          end
        end
        if args.pinned?
          ...
          names.sort.each do |d|
            keg_pin = (DINRUSBREW_PINNED_KEGS/d.basename.to_s)
      ...
      def list_casks
        casks = if args.no_named?
          ...
          filtered_args = args.named.dup.delete_if do |n|
            DinrusBrew.failed = true unless Cask::Caskroom.path.join(n).exist?
            !Cask::Caskroom.path.join(n).exist?
          end
          # NamedAargs subclasses array
          T.cast(filtered_args, DinrusBrew::CLI::NamedArgs).to_formulae_and_casks(only: :cask)
        end
