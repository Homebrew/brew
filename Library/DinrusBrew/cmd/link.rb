...
module DinrusBrew
  module Cmd
    class Link < AbstractCommand
      cmd_args do
        description <<~EOS
          Symlink all of <formula>'s installed files into DinrusBrew's prefix.
      ...
      def run
        ...
        kegs.freeze.each do |keg|
          ...
          if keg_only
            if DINRUSBREW_PREFIX.to_s == DINRUSBREW_DEFAULT_PREFIX && formula.present? &&
            ...
            if !args.force? && (formula.blank? || !formula.keg_only_reason.versioned_formula?)
              opoo "#{keg.name} только кег и должен компоноваться через `--force`."
          ...
          keg.lock do
            ...
            begin
              ...
              puts "#{n} символьных ссылок создано."
            end

            puts_keg_only_path_message(keg) if keg_only && !DinrusBrew::EnvConfig.developer?
          end
        end
      end
      ...
      def puts_keg_only_path_message(keg)
        ...
        opt = DINRUSBREW_PREFIX/"opt/#{keg.name}"
