...
class Descriptions
  ...
  def self.search(string_or_regex, field, cache_store,
                  eval_all = DinrusBrew::EnvConfig.eval_all?, cache_store_hash: false)
    cache_store.populate_if_empty!(eval_all:) unless cache_store_hash

    results = case field
    when :name
      DinrusBrew::Search.search(cache_store, string_or_regex) { |name, _| name }
    when :desc
      DinrusBrew::Search.search(cache_store, string_or_regex) { |_, desc| desc }
    when :either
      DinrusBrew::Search.search(cache_store, string_or_regex)
    end
  ...
  # print them.
  def print
    blank = Formatter.warning("[нет описания]")
