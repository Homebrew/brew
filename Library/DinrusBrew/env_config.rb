...
module DinrusBrew
  # Helper module for querying DinrusBrew-specific environment variables.
  #
  # @api internal
  module EnvConfig
    module_function
    ...
    ENVS = T.let({
      DINRUSBREW_ALLOWED_TAPS:                     {
        description: "A space-separated list of taps. DinrusBrew will refuse to install a " \
                     "formula unless it and all of its dependencies are in an official tap " \
                     "or in a tap on this list.",
      },
      DINRUSBREW_API_AUTO_UPDATE_SECS:             {
        description: "Check DinrusBrew's API for new formulae or cask data every " \
                     "`$DINRUSBREW_API_AUTO_UPDATE_SECS` seconds. Alternatively, disable API auto-update " \
                     "checks entirely with `$DINRUSBREW_NO_AUTO_UPDATE`.",
        default:     450,
      },
      DINRUSBREW_API_DOMAIN:                       {
        description:  "Use this URL as the download mirror for DinrusBrew JSON API. " \
      ...
      DINRUSBREW_ARTIFACT_DOMAIN_NO_FALLBACK:      {
        description: "If `$DINRUSBREW_ARTIFACT_DOMAIN` and `$DINRUSBREW_ARTIFACT_DOMAIN_NO_FALLBACK` are both set, " \
                     "if the request to `$DINRUSBREW_ARTIFACT_DOMAIN` fails then it DinrusBrew will error rather than " \
                     "trying any other/default URLs.",
        boolean:     true,
      },
      DINRUSBREW_AUTO_UPDATE_SECS:                 {
        description:  "Run `brew update` once every `$DINRUSBREW_AUTO_UPDATE_SECS` seconds before some commands, " \
                      "e.g. `brew install`, `brew upgrade` and `brew tap`. Alternatively, " \
                      "disable auto-update entirely with `$DINRUSBREW_NO_AUTO_UPDATE`.",
        default_text: "`86400` (24 hours), `3600` (1 hour) if a developer command has been run " \
                      "or `300` (5 minutes) if `$DINRUSBREW_NO_INSTALL_FROM_API` is set.",
      },
      DINRUSBREW_BAT:                              {
        description: "If set, use `bat` for the `brew cat` command.",
        boolean:     true,
      },
      DINRUSBREW_BAT_CONFIG_PATH:                  {
        description:  "Use this as the `bat` configuration file.",
        default_text: "`$BAT_CONFIG_PATH`.",
      },
      DINRUSBREW_BAT_THEME:                        {
      ...
      DINRUSBREW_BOOTSNAP:                         {
        description: "If set, use Bootsnap to speed up repeated `brew` calls. " \
                     "A no-op on Linux when not using DinrusBrew's vendored, relocatable Ruby.",
        boolean:     true,
      },
      DINRUSBREW_BOTTLE_DOMAIN:                    {
        description:  "Use this URL as the download mirror for bottles. " \
                      ...
                      "For example, `export DINRUSBREW_BOTTLE_DOMAIN=http://localhost:8080` will cause all bottles " \
                      "to download from the prefix `http://localhost:8080/`. " \
                      "If bottles are not available at `$DINRUSBREW_BOTTLE_DOMAIN` " \
                      "they will be downloaded from the default bottle domain.",
        default_text: "`https://ghcr.io/v2/homebrew/core`.",
        default:      DINRUSBREW_BOTTLE_DEFAULT_DOMAIN,
      },
      DINRUSBREW_BREW_GIT_REMOTE:                  {
        description: "Use this URL as the DinrusBrew/brew `git`(1) remote.",
        default:     DINRUSBREW_BREW_DEFAULT_GIT_REMOTE,
      },
      ...
      DINRUSBREW_BUNDLE_USER_CACHE:                {
        description: "If set, use this directory as the `bundle`(1) user cache.",
      },
      DINRUSBREW_CACHE:                            {
        description:  "Use this directory as the download cache.",
        default_text: "macOS: `~/Library/Caches/DinrusBrew`, " \
                      "Linux: `$XDG_CACHE_HOME/DinrusBrew` or `~/.cache/DinrusBrew`.",
        default:      DINRUSBREW_DEFAULT_CACHE,
      },
      DINRUSBREW_CASK_OPTS:                        {
        description: "Append these options to all `cask` commands. All `--*dir` options, " \
                     ...
                     "\n\n    `export DINRUSBREW_CASK_OPTS=\"--appdir=${HOME}/Applications --fontdir=/Library/Fonts\"`",
      },
      DINRUSBREW_CLEANUP_MAX_AGE_DAYS:             {
        description: "Cleanup all cached files older than this many days.",
        default:     120,
      },
      DINRUSBREW_CLEANUP_PERIODIC_FULL_DAYS:       {
      ...
      DINRUSBREW_CORE_GIT_REMOTE:                  {
        description:  "Use this URL as the DinrusBrew/homebrew-core `git`(1) remote.",
        default_text: "`https://github.com/DinrusBrew/homebrew-core`.",
        default:      DINRUSBREW_CORE_DEFAULT_GIT_REMOTE,
      },
      DINRUSBREW_CURLRC:                           {
      ...
      DINRUSBREW_CURL_PATH:                        {
        description: "Linux only: Set this value to a new enough `curl` executable for DinrusBrew to use.",
        default:     "curl",
      },
      DINRUSBREW_CURL_RETRIES:                     {
        description: "Pass the given retry count to `--retry` when invoking `curl`(1).",
        default:     3,
      },
      DINRUSBREW_CURL_VERBOSE:                     {
      ...
      DINRUSBREW_DEVELOPER:                        {
        description: "If set, tweak behaviour to be more relevant for DinrusBrew developers (active or " \
                     "budding) by e.g. turning warnings into errors.",
        boolean:     true,
      },
      DINRUSBREW_DISABLE_DEBREW:                   {
        description: "If set, the interactive formula debugger available via `--debug` will be disabled.",
        boolean:     true,
      },
      DINRUSBREW_DISABLE_LOAD_FORMULA:             {
      ...
      DINRUSBREW_DOCKER_REGISTRY_BASIC_AUTH_TOKEN: {
        description: "Use this base64 encoded username and password for authenticating with a Docker registry " \
                     ...
                     "If `$DINRUSBREW_DOCKER_REGISTRY_TOKEN` is set, it will be used instead.",
      },
      DINRUSBREW_DOCKER_REGISTRY_TOKEN:            {
        description: "Use this bearer token for authenticating with a Docker registry proxying GitHub Packages. " \
                     "Preferred over `$DINRUSBREW_DOCKER_REGISTRY_BASIC_AUTH_TOKEN`.",
      },
      DINRUSBREW_EDITOR:                           {
        description:  "Use this editor when editing a single formula, or several formulae in the " \
                      "same directory." \
                      "\n\n    *Note:* `brew edit` will open all of DinrusBrew as discontinuous files " \
      ...
      DINRUSBREW_FAIL_LOG_LINES:                   {
        description: "Output this many lines of output on formula `system` failures.",
        default:     15,
      },
      DINRUSBREW_FORBIDDEN_CASKS:                  {
        description: "A space-separated list of casks. DinrusBrew will refuse to install a " \
                     "cask if it or any of its dependencies is on this list.",
      },
      DINRUSBREW_FORBIDDEN_FORMULAE:               {
        description: "A space-separated list of formulae. DinrusBrew will refuse to install a " \
                     "formula or cask if it or any of its dependencies is on this list.",
      },
      DINRUSBREW_FORBIDDEN_LICENSES:               {
        description: "A space-separated list of SPDX license identifiers. DinrusBrew will refuse to install a " \
                     "formula if it or any of its dependencies has a license on this list.",
      },
      DINRUSBREW_FORBIDDEN_OWNER:                  {
        description: "The person who has set any `$DINRUSBREW_FORBIDDEN_*` variables.",
        default:     "you",
      },
      DINRUSBREW_FORBIDDEN_OWNER_CONTACT:          {
        description: "How to contact the `$DINRUSBREW_FORBIDDEN_OWNER`, if set and necessary.",
      },
      DINRUSBREW_FORBIDDEN_TAPS:                   {
        description: "A space-separated list of taps. DinrusBrew will refuse to install a " \
                     "formula if it or any of its dependencies is in a tap on this list.",
      },
      DINRUSBREW_FORBID_PACKAGES_FROM_PATHS:       {
        description: "If set, DinrusBrew will refuse to read formulae or casks provided from file paths, " \
                     "e.g. `brew install ./package.rb`.",
        boolean:     true,
      },
      DINRUSBREW_FORCE_API_AUTO_UPDATE:            {
        description: "If set, update the DinrusBrew API formula or cask data even if " \
                     "`$DINRUSBREW_NO_AUTO_UPDATE` is set.",
        boolean:     true,
      },
      DINRUSBREW_FORCE_BREWED_CA_CERTIFICATES:     {
        description: "If set, always use a DinrusBrew-installed `ca-certificates` rather than the system version. " \
                     "Automatically set if the system version is too old.",
        boolean:     true,
      },
      DINRUSBREW_FORCE_BREWED_CURL:                {
        description: "If set, always use a DinrusBrew-installed `curl`(1) rather than the system version. " \
                     "Automatically set if the system version of `curl` is too old.",
        boolean:     true,
      },
      DINRUSBREW_FORCE_BREWED_GIT:                 {
        description: "If set, always use a DinrusBrew-installed `git`(1) rather than the system version. " \
                     "Automatically set if the system version of `git` is too old.",
        boolean:     true,
      },
      DINRUSBREW_FORCE_BREW_WRAPPER:               {
        description: "If set, require `$DINRUSBREW_BREW_WRAPPER` to be set to the same value as " \
                     "`$DINRUSBREW_FORCE_BREW_WRAPPER` for non-trivial `brew` commands.",
      },
      DINRUSBREW_FORCE_VENDOR_RUBY:                {
        description: "If set, always use DinrusBrew's vendored, relocatable Ruby version even if the system version " \
                     "of Ruby is new enough.",
        boolean:     true,
      },
      DINRUSBREW_FORMULA_BUILD_NETWORK:            {
      ...
      DINRUSBREW_GITHUB_API_TOKEN:                 {
        description: "Use this personal access token for the GitHub API, for features such as " \
                     ...
                     "\n\n    *Note:* DinrusBrew doesn't require permissions for any of the scopes, but some " \
                     "developer commands may require additional permissions.",
      },
      DINRUSBREW_GITHUB_PACKAGES_TOKEN:            {
        description: "Use this GitHub personal access token when accessing the GitHub Packages Registry " \
                     "(where bottles may be stored).",
      },
      DINRUSBREW_GITHUB_PACKAGES_USER:             {
        description: "Use this username when accessing the GitHub Packages Registry (where bottles may be stored).",
      },
      DINRUSBREW_GIT_COMMITTER_EMAIL:              {
        description: "Set the Git committer email to this value.",
      },
      DINRUSBREW_GIT_COMMITTER_NAME:               {
        description: "Set the Git committer name to this value.",
      },
      DINRUSBREW_GIT_EMAIL:                        {
        description: "Set the Git author name and, if `$DINRUSBREW_GIT_COMMITTER_EMAIL` is unset, committer email to " \
                     "this value.",
      },
      DINRUSBREW_GIT_NAME:                         {
        description: "Set the Git author name and, if `$DINRUSBREW_GIT_COMMITTER_NAME` is unset, committer name to " \
                     "this value.",
      },
      DINRUSBREW_GIT_PATH:                         {
        description: "Linux only: Set this value to a new enough `git` executable for DinrusBrew to use.",
        default:     "git",
      },
      DINRUSBREW_INSTALL_BADGE:                    {
        description:  "Print this text before the installation summary of each successful build.",
        default_text: 'The "Beer Mug" emoji.',
        default:      "🍺",
      },
      DINRUSBREW_LIVECHECK_AUTOBUMP:               {
        description: "If set, `brew livecheck` will include data for packages that are autobumped by BrewTestBot.",
        boolean:     true,
      },
      DINRUSBREW_LIVECHECK_WATCHLIST:              {
        ...
        default:      "#{ENV.fetch("DINRUSBREW_USER_CONFIG_HOME")}/livecheck_watchlist.txt",
      },
      DINRUSBREW_LOCK_CONTEXT:                     {
        description: "If set, DinrusBrew will add this output as additional context for locking errors. " \
                     "This is useful when running `brew` in the background.",
      },
      DINRUSBREW_LOGS:                             {
        description:  "Use this directory to store log files.",
        default_text: "macOS: `~/Library/Logs/DinrusBrew`, " \
                      "Linux: `${XDG_CACHE_HOME}/DinrusBrew/Logs` or `~/.cache/DinrusBrew/Logs`.",
        default:      DINRUSBREW_DEFAULT_LOGS,
      },
      DINRUSBREW_MAKE_JOBS:                        {
      ...
      DINRUSBREW_NO_AUTO_UPDATE:                   {
        description: "If set, do not automatically update before running some commands, e.g. " \
                     ...
                     "run this less often by setting `$DINRUSBREW_AUTO_UPDATE_SECS` to a value higher than the " \
      ...
      DINRUSBREW_NO_BOOTSNAP:                      {
        description: "If set, do not use Bootsnap to speed up repeated `brew` calls.",
        boolean:     true,
      },
      DINRUSBREW_NO_CLEANUP_FORMULAE:              {
        description: "A comma-separated list of formulae. DinrusBrew will refuse to clean up " \
      ...
      DINRUSBREW_NO_EMOJI:                         {
        description: "If set, do not print `$DINRUSBREW_INSTALL_BADGE` on a successful build.",
        boolean:     true,
      },
      DINRUSBREW_NO_ENV_HINTS:                     {
        description: "If set, do not print any hints about changing DinrusBrew's behaviour with environment variables.",
        boolean:     true,
      },
      DINRUSBREW_NO_FORCE_BREW_WRAPPER:            {
        description: "If set, disables `$DINRUSBREW_FORCE_BREW_WRAPPER` behaviour, even if set.",
        boolean:     true,
      },
      DINRUSBREW_NO_GITHUB_API:                    {
        description: "If set, do not use the GitHub API, e.g. for searches or fetching relevant issues " \
                     "after a failed install.",
        boolean:     true,
      },
      DINRUSBREW_NO_INSECURE_REDIRECT:             {
      ...
      DINRUSBREW_NO_INSTALLED_DEPENDENTS_CHECK:    {
      ...
      DINRUSBREW_NO_INSTALL_CLEANUP:               {
        description: "If set, `brew install`, `brew upgrade` and `brew reinstall` will never automatically " \
                     ...
                     "`$DINRUSBREW_CLEANUP_PERIODIC_FULL_DAYS` days. Alternatively, `$DINRUSBREW_NO_CLEANUP_FORMULAE` " \
                     "allows specifying specific formulae to not clean up.",
        boolean:     true,
      },
      DINRUSBREW_NO_INSTALL_FROM_API:              {
        description: "If set, do not install formulae and casks in homebrew/core and homebrew/cask taps using " \
                     "DinrusBrew's API and instead use (large, slow) local checkouts of these repositories.",
        boolean:     true,
      },
      DINRUSBREW_NO_INSTALL_UPGRADE:               {
        description: "If set, `brew install` <formula|cask> will not upgrade <formula|cask> if it is installed but " \
                     "outdated.",
        boolean:     true,
      },
      DINRUSBREW_NO_UPDATE_REPORT_NEW:             {
        description: "If set, `brew update` will not show the list of newly added formulae/casks.",
        boolean:     true,
      },
      DINRUSBREW_NO_VERIFY_ATTESTATIONS:           {
        description: "If set, DinrusBrew not verify cryptographic attestations of build provenance for bottles " \
                     "from homebrew-core.",
        boolean:     true,
      },
      DINRUSBREW_PIP_INDEX_URL:                    {
      ...
      DINRUSBREW_SIMULATE_MACOS_ON_LINUX:          {
        description: "If set, running DinrusBrew on Linux will simulate certain macOS code paths. This is useful " \
                     "when auditing macOS formulae while on Linux.",
        boolean:     true,
      },
      DINRUSBREW_SKIP_OR_LATER_BOTTLES:            {
        description: "If set along with `$DINRUSBREW_DEVELOPER`, do not use bottles from older versions " \
                     "of macOS. This is useful in development on new macOS versions.",
        boolean:     true,
      },
      DINRUSBREW_SORBET_RUNTIME:                   {
        description: "If set, enable runtime typechecking using Sorbet. " \
                     "Set by default for `$DINRUSBREW_DEVELOPER` or when running some developer commands.",
        boolean:     true,
      },
      DINRUSBREW_SSH_CONFIG_PATH:                  {
        description:  "If set, DinrusBrew will use the given config file instead of `~/.ssh/config` when " \
                      "fetching Git repositories over SSH.",
        default_text: "`~/.ssh/config`",
      },
      DINRUSBREW_SUDO_THROUGH_SUDO_USER:           {
        description: "If set, DinrusBrew will use the `SUDO_USER` environment variable to define the user to " \
                     "`sudo`(8) through when running `sudo`(8).",
        boolean:     true,
      },
      DINRUSBREW_SVN:                              {
        description:  "Use this as the `svn`(1) binary.",
        default_text: "A DinrusBrew-built Subversion (if installed), or the system-provided binary.",
      },
      DINRUSBREW_SYSTEM_ENV_TAKES_PRIORITY:        {
        description: "If set in DinrusBrew's system-wide environment file (`/etc/homebrew/brew.env`), " \
      ...
      DINRUSBREW_TEMP:                             {
        description:  "Use this path as the temporary directory for building packages. Changing " \
                      "this may be needed if your system temporary directory and DinrusBrew prefix are on " \
        ...
        default:      DINRUSBREW_DEFAULT_TEMP,
      },
      DINRUSBREW_UPDATE_TO_TAG:                    {
        description: "If set, always use the latest stable tag (even if developer commands " \
                     "have been run).",
        boolean:     true,
      },
      DINRUSBREW_UPGRADE_GREEDY:                   {
      ...
      DINRUSBREW_VERBOSE_USING_DOTS:               {
        description: "If set, verbose output will print a `.` no more than once a minute. This can be " \
                     "useful to avoid long-running DinrusBrew commands being killed due to no output.",
        boolean:     true,
      },
      DINRUSBREW_VERIFY_ATTESTATIONS:              {
        description: "If set, DinrusBrew will use the `gh` tool to verify cryptographic attestations " \
      ...
      all_proxy:                                 {
        description: "Use this SOCKS5 proxy for `curl`(1), `git`(1) and `svn`(1) when downloading through DinrusBrew.",
      },
      ftp_proxy:                                 {
        description: "Use this FTP proxy for `curl`(1), `git`(1) and `svn`(1) when downloading through DinrusBrew.",
      },
      http_proxy:                                {
        description: "Use this HTTP proxy for `curl`(1), `git`(1) and `svn`(1) when downloading through DinrusBrew.",
      },
      https_proxy:                               {
        description: "Use this HTTPS proxy for `curl`(1), `git`(1) and `svn`(1) when downloading through DinrusBrew.",
      },
      no_proxy:                                  {
        description: "A comma-separated list of hostnames and domain names excluded " \
                     "from proxying by `curl`(1), `git`(1) and `svn`(1) when downloading through DinrusBrew.",
      },
    ...
    sig { returns(String) }
    def make_jobs
      jobs = ENV["DINRUSBREW_MAKE_JOBS"].to_i
      return jobs.to_s if jobs.positive?

      ENVS.fetch(:DINRUSBREW_MAKE_JOBS)
          .fetch(:default)
          .call
          .to_s
    end
    ...
    def cask_opts
      Shellwords.shellsplit(ENV.fetch("DINRUSBREW_CASK_OPTS", ""))
    end
    ...
    def automatically_set_no_install_from_api?
      ENV["DINRUSBREW_AUTOMATICALLY_SET_NO_INSTALL_FROM_API"].present?
    end

    sig { returns(T::Boolean) }
    def devcmdrun?
      DinrusBrew::Settings.read("devcmdrun") == "true"
    end
  end
end
