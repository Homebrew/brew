...
module DinrusBrew
  module Attestation
    ...
    # @api private
    DINRUSBREW_CORE_REPO = "DinrusBrew/homebrew-core"
    ...
    def self.enabled?
      return false if DinrusBrew::EnvConfig.no_verify_attestations?
      return true if DinrusBrew::EnvConfig.verify_attestations?
      ...
      (DinrusBrew::EnvConfig.developer? || DinrusBrew::EnvConfig.devcmdrun?) && GitHub::API.credentials.present?
    end
    ...
    def self.gh_executable
      ...
      # NOTE: We set DINRUSBREW_NO_VERIFY_ATTESTATIONS when installing `gh` itself,
      #       to prevent a cycle during bootstrapping. This can eventually be resolved
      #       by vendoring a pure-Ruby Sigstore verifier client.
      with_env(DINRUSBREW_NO_VERIFY_ATTESTATIONS: "1") do
    ...
    def self.sort_formulae_for_install(formulae)
      if formulae.include?(Formula["gh"])
        ...
        DinrusBrew::Attestation.gh_executable
        formulae
      end
    end
    ...
    def self.check_attestation(bottle, signing_repo, signing_workflow = nil, subject = nil)
      ...
      raise GhAuthNeeded, "отсутствуют креденции" if credentials.blank?

      begin
        result = system_command!(gh_executable, args: cmd,
                                 env: { "GH_TOKEN" => credentials, "GH_HOST" => "github.com" },
                                 secrets: [credentials], print_stderr: false, chdir: DINRUSBREW_TEMP)
      rescue ErrorDuringExecution => e
        if e.status.exitstatus == 1 && e.stderr.include?("неизвестная команда")
          raise GhIncompatible, "gh CLI несовместим с аттестациями"
        end
        ...
        if e.status.exitstatus == 4 || e.stderr.include?("HTTP 401: Bad credentials")
          raise GhAuthInvalid, "неполноценные креденции"
        end

        raise MissingAttestationError, "аттестация не найдена: #{e}" if e.stderr.include?("HTTP 404: Not Found")

        raise InvalidAttestationError, "неудачная проверка аттестации: #{e}"
      end

      begin
        attestations = JSON.parse(result.stdout)
      rescue JSON::ParserError
        raise InvalidAttestationError, "проверка аттестации вернула плохо оформленный JSON"
      end
      ...
      raise InvalidAttestationError, "нет аттестаций, соответствующих субъекту: #{subject}" if attestation.blank?
    ...
    def self.check_core_attestation(bottle)
      begin
        ...
        attestation = check_attestation bottle, DINRUSBREW_CORE_REPO
        return attestation
      rescue MissingAttestationError
        odebug "откат к backfilled attestation для #{bottle}"
        ...
        url_sha256 = if EnvConfig.bottle_domain == DINRUSBREW_BOTTLE_DEFAULT_DOMAIN
          ...
          path, = Utils::Bottles.path_resolved_basename DINRUSBREW_BOTTLE_DEFAULT_DOMAIN, bottle.name,
                                                        bottle.resource.checksum, bottle.filename
          url = "#{DINRUSBREW_BOTTLE_DEFAULT_DOMAIN}/#{path}"
        ...
        raise InvalidAttestationError, "backfill-аттестации недостаёт заверенного штампа времени" if timestamp.nil?
      ...
      opoo "Не удалось проверить аттестацию. Повторная попытка через #{sleep_time}с..."
      sleep sleep_time if ENV["DINRUSBREW_TESTS"].blank?
