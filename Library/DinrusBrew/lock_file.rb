...
# A lock file to prevent multiple DinrusBrew processes from modifying the same path.
class LockFile
  ...
  def initialize(type, locked_path)
    ...
    @path = DINRUSBREW_LOCKS/"#{lock_name}.#{type}.lock"
    @lockfile = nil
  end
...
class FormulaLock < LockFile
  ...
  def initialize(rack_name)
    super(:formula, DINRUSBREW_CELLAR/rack_name)
  end
end
...
class CaskLock < LockFile
  ...
  def initialize(cask_token)
    super(:cask, DINRUSBREW_PREFIX/"Caskroom/#{cask_token}")
  end
end
