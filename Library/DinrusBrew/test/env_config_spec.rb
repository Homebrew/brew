...
RSpec.describe DinrusBrew::EnvConfig do
  ...
  describe ".env_method_name" do
    it "generates method names" do
      expect(env_config.env_method_name("DINRUSBREW_FOO", {})).to eql("foo")
    end

    it "generates boolean method names" do
      expect(env_config.env_method_name("DINRUSBREW_BAR", boolean: true)).to eql("bar?")
    end
  end

  describe ".artifact_domain" do
    it "returns value if set" do
      ENV["DINRUSBREW_ARTIFACT_DOMAIN"] = "https://brew.sh"
      expect(env_config.artifact_domain).to eql("https://brew.sh")
    end

    it "returns nil if empty" do
      ENV["DINRUSBREW_ARTIFACT_DOMAIN"] = ""
  ...
  describe ".cleanup_periodic_full_days" do
    it "returns value if set" do
      ENV["DINRUSBREW_CLEANUP_PERIODIC_FULL_DAYS"] = "360"
      expect(env_config.cleanup_periodic_full_days).to eql("360")
    end

    it "returns default if unset" do
      ENV["DINRUSBREW_CLEANUP_PERIODIC_FULL_DAYS"] = nil
