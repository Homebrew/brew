...
RSpec.describe DinrusBrew::Cmd::Desc do
  ...
  it "errors when searching without --eval-all", :integration_test do
    ...
    expect { brew "desc", "--search", "testball" }
      .to output(/`brew desc --search` needs `--eval-all` passed or `\$DINRUSBREW_EVAL_ALL` set!/).to_stderr
      .and be_a_failure
  end
  ...
  it "successfully searches without --eval-all, with API", :integration_test do
    ...
    expect { brew "desc", "--search", "testball", "DINRUSBREW_NO_INSTALL_FROM_API" => nil }
      .to be_a_success
  end
end
