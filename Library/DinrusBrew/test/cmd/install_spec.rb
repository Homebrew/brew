...
RSpec.describe DinrusBrew::Cmd::InstallCmd do
  ...
  it "installs formulae", :integration_test do
    ...
    expect { brew "install", "testball1" }
      .to output(%r{#{DINRUSBREW_CELLAR}/testball1/0\.1}o).to_stdout
      .and not_to_output.to_stderr
      .and be_a_success
    expect(DINRUSBREW_CELLAR/"testball1/0.1/foo/test").not_to be_a_file
  end
  ...
  it "installs formulae with options", :integration_test do
    ...
    expect { brew "install", "testball1", "--with-foo" }
      ...
      .and not_to_output.to_stderr
      .and be_a_success
    expect(DINRUSBREW_CELLAR/"testball1/0.1/foo/test").to be_a_file
  end
  ...
  it "can install keg-only Formulae", :integration_test do
    ...
    expect { brew "install", "testball1" }
      .to output(%r{#{DINRUSBREW_CELLAR}/testball1/1\.0}o).to_stdout
      .and not_to_output.to_stderr
      .and be_a_success
    expect(DINRUSBREW_CELLAR/"testball1/1.0/foo/test").not_to be_a_file
  end

  it "can install HEAD Formulae", :integration_test do
    repo_path = DINRUSBREW_CACHE.join("repo")
    ...
    expect { brew "install", "testball1", "--HEAD", "--ignore-dependencies" }
      .to output(%r{#{DINRUSBREW_CELLAR}/testball1/HEAD-d5eb689}o).to_stdout
      .and output(/Cloning into/).to_stderr
      .and be_a_success
    expect(DINRUSBREW_CELLAR/"testball1/HEAD-d5eb689/foo/test").not_to be_a_file
  end
  ...
  it "installs formulae with debug symbols", :integration_test do
    ...
    expect { brew "install", "testball1", "--debug-symbols", "--build-from-source" }
      ...
      .and not_to_output.to_stderr
      .and be_a_success
    expect(DINRUSBREW_CELLAR/"testball1/0.1/bin/test").to be_a_file
    expect(DINRUSBREW_CELLAR/"testball1/0.1/bin/test.dSYM/Contents/Resources/DWARF/test").to be_a_file if OS.mac?
    expect(DINRUSBREW_CACHE/"Sources/testball1").to be_a_directory
  end
end
