...
RSpec.describe DinrusBrew::Cmd::Prefix do
  it_behaves_like "parseable arguments"

  it "prints DinrusBrew's prefix", :integration_test do
    expect { brew_sh "--prefix" }
      .to output("#{ENV.fetch("DINRUSBREW_PREFIX")}\n").to_stdout
  ...
  it "prints the prefix for a Formula", :integration_test, :needs_homebrew_core do
    expect { brew_sh "--prefix", "wget" }
      .to output("#{ENV.fetch("DINRUSBREW_PREFIX")}/opt/wget\n").to_stdout
      .and not_to_output.to_stderr
      .and be_a_success
  end
