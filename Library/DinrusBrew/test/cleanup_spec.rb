...
RSpec.describe DinrusBrew::Cleanup do
  subject(:cleanup) { described_class.new }

  let(:ds_store) { Pathname.new("#{DINRUSBREW_CELLAR}/.DS_Store") }
  let(:lock_file) { Pathname.new("#{DINRUSBREW_LOCKS}/foo") }
  ...
  describe "::prune?" do
    subject(:path) { DINRUSBREW_CACHE/"foo" }
  ...
  describe "::prune_prefix_symlinks_and_directories" do
    ...
    context "when nested directories exist with only broken symlinks" do
      let(:dir) { DINRUSBREW_PREFIX/"lib/foo" }
  ...
  describe "::cleanup_logs" do
    let(:path) { (DINRUSBREW_LOGS/"delete_me") }
  ...
  describe "::cleanup_cache" do
    it "cleans up incomplete downloads" do
      incomplete = (DINRUSBREW_CACHE/"something.incomplete")
      incomplete.mkpath
    ...
    it "cleans up 'cargo_cache'" do
      cargo_cache = (DINRUSBREW_CACHE/"cargo_cache")
      cargo_cache.mkpath
    ...
    it "cleans up 'go_cache'" do
      go_cache = (DINRUSBREW_CACHE/"go_cache")
      go_cache.mkpath
    ...
    it "cleans up 'glide_home'" do
      glide_home = (DINRUSBREW_CACHE/"glide_home")
      glide_home.mkpath
    ...
    it "cleans up 'java_cache'" do
      java_cache = (DINRUSBREW_CACHE/"java_cache")
      java_cache.mkpath
    ...
    it "cleans up 'npm_cache'" do
      npm_cache = (DINRUSBREW_CACHE/"npm_cache")
      npm_cache.mkpath
    ...
    it "cleans up 'gclient_cache'" do
      gclient_cache = (DINRUSBREW_CACHE/"gclient_cache")
    ...
    it "cleans up all files and directories" do
      git = (DINRUSBREW_CACHE/"gist--git")
      gist = (DINRUSBREW_CACHE/"gist")
      svn = (DINRUSBREW_CACHE/"gist--svn")
    ...
    context "when cleaning old files in DINRUSBREW_CACHE" do
      let(:bottle) { (DINRUSBREW_CACHE/"testball--0.0.1.tag.bottle.tar.gz") }
      let(:testball) { (DINRUSBREW_CACHE/"testball--0.0.1") }
      let(:testball_resource) { (DINRUSBREW_CACHE/"testball--rsrc--0.0.1.txt") }

      before do
        FileUtils.touch bottle
        FileUtils.touch testball
        FileUtils.touch testball_resource
        (DINRUSBREW_CELLAR/"testball"/"0.0.1").mkpath
        # Create the latest version of testball so the older version is eligible for cleanup.
        (DINRUSBREW_CELLAR/"testball"/"0.1/bin").mkpath
  ...
  describe "::cleanup_python_site_packages" do
    context "when cleaning up Python modules" do
      let(:foo_module) { (DINRUSBREW_PREFIX/"lib/python3.99/site-packages/foo") }
    ...
    it "cleans up stale `*.pyc` files in the top-level `__pycache__`" do
      pycache = DINRUSBREW_PREFIX/"lib/python3.99/site-packages/__pycache__"
