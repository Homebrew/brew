  ...
  it "fails for formulae not in PROVIDED_BY_MACOS_FORMULAE list" do
    expect_offense(<<~RUBY)
      class Baz < Formula
        ...
        ^^^^^^^^^^^^^^^^^^^^^^^^^^^ FormulaAudit/ProvidedByMacos: Formulae that are `keg_only :provided_by_macos` should be added to the `PROVIDED_BY_MACOS_FORMULAE` list (in the DinrusBrew/brew repo)
      end
    RUBY
  end
