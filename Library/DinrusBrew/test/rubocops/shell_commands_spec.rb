...
RSpec.describe RuboCop::Cop::DinrusBrew::ShellCommands do
    ...
    it "reports and corrects an offense when `system` arguments should be separated" do
      expect_offense(<<~RUBY)
        class Foo < Formula
          def install
            system "foo bar"
                   ^^^^^^^^^ DinrusBrew/ShellCommands: Separate `system` commands into `"foo", "bar"`
          end
        end
      RUBY
    ...
    it "reports and corrects an offense when `system` arguments involving interpolation should be separated" do
      expect_offense(<<~RUBY)
        class Foo < Formula
          def install
            system "\#{bin}/foo bar"
                   ^^^^^^^^^^^^^^^^ DinrusBrew/ShellCommands: Separate `system` commands into `"\#{bin}/foo", "bar"`
          end
        end
      RUBY
        ...
        class Foo < Formula
          def install
            Utils.popen_read("foo bar")
                             ^^^^^^^^^ DinrusBrew/ShellCommands: Separate `Utils.popen_read` commands into `"foo", "bar"`
          end
        end
      RUBY
        ...
        class Foo < Formula
          def install
            Utils.safe_popen_read("foo bar")
                                  ^^^^^^^^^ DinrusBrew/ShellCommands: Separate `Utils.safe_popen_read` commands into `"foo", "bar"`
          end
        end
      RUBY
        ...
        class Foo < Formula
          def install
            Utils.popen_write("foo bar")
                              ^^^^^^^^^ DinrusBrew/ShellCommands: Separate `Utils.popen_write` commands into `"foo", "bar"`
          end
        end
      RUBY
        ...
        class Foo < Formula
          def install
            Utils.safe_popen_write("foo bar")
                                   ^^^^^^^^^ DinrusBrew/ShellCommands: Separate `Utils.safe_popen_write` commands into `"foo", "bar"`
          end
        end
      RUBY
        ...
        class Foo < Formula
          def install
            Utils.popen_read("\#{bin}/foo bar")
                             ^^^^^^^^^^^^^^^^ DinrusBrew/ShellCommands: Separate `Utils.popen_read` commands into `"\#{bin}/foo", "bar"`
          end
        end
      RUBY
