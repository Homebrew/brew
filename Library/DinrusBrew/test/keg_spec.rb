...
RSpec.describe Keg do
  ...
  def setup_test_keg(name, version)
    path = DINRUSBREW_CELLAR/name/version
    (path/"bin").mkpath
  ...
  let(:dst) { DINRUSBREW_PREFIX/"bin"/"helloworld" }
  ...
  before do
    ...
    (DINRUSBREW_PREFIX/"lib").mkpath
  end

  after do
    kegs.each(&:unlink)
    rmtree DINRUSBREW_PREFIX/"lib"
  end
  ...
  specify "#oldname_opt_records" do
    ...
    oldname_opt_record = DINRUSBREW_PREFIX/"opt/oldfoo"
    oldname_opt_record.make_relative_symlink(DINRUSBREW_CELLAR/"foo/1.0")
    expect(keg.oldname_opt_records).to eq([oldname_opt_record])
  end

  specify "#remove_oldname_opt_records" do
    oldname_opt_record = DINRUSBREW_PREFIX/"opt/oldfoo"
    oldname_opt_record.make_relative_symlink(DINRUSBREW_CELLAR/"foo/2.0")
  ...
  describe "#link" do
    it "links a Keg" do
      ...
      (DINRUSBREW_PREFIX/"bin").children.each do |c|
    ...
    context "with dry run set to true" do
      ...
      it "only prints what would be done" do
        expect do
          ...
          #{DINRUSBREW_PREFIX}/bin/goodbye_cruel_world
          #{DINRUSBREW_PREFIX}/bin/helloworld
          #{DINRUSBREW_PREFIX}/bin/hiworld
        EOF
    ...
    specify "pkgconfig directory is created" do
      link = DINRUSBREW_PREFIX/"lib"/"pkgconfig"
    ...
    specify "cmake directory is created" do
      link = DINRUSBREW_PREFIX/"lib"/"cmake"
  ...
  describe "#unlink" do
    ...
    it "prunes empty top-level directories" do
      mkpath DINRUSBREW_PREFIX/"lib/foo/bar"
      mkpath keg/"lib/foo/bar"
      touch keg/"lib/foo/bar/file1"

      keg.unlink

      expect(DINRUSBREW_PREFIX/"lib/foo").not_to be_a_directory
    end

    it "ignores .DS_Store when pruning empty directories" do
      mkpath DINRUSBREW_PREFIX/"lib/foo/bar"
      touch DINRUSBREW_PREFIX/"lib/foo/.DS_Store"
      mkpath keg/"lib/foo/bar"
      touch keg/"lib/foo/bar/file1"
      ...
      expect(DINRUSBREW_PREFIX/"lib/foo").not_to be_a_directory
      expect(DINRUSBREW_PREFIX/"lib/foo/.DS_Store").not_to exist
    end
    ...
    it "doesn't remove links to symlinks" do
      a = DINRUSBREW_CELLAR/"a"/"1.0"
      b = DINRUSBREW_CELLAR/"b"/"1.0"
    ...
    it "removes broken symlinks that conflict with directories" do
      ...
      link = DINRUSBREW_PREFIX/"lib"/"foo"
      link.parent.mkpath
  ...
  describe "#optlink" do
    it "creates an opt link" do
      oldname_opt_record = DINRUSBREW_PREFIX/"opt/oldfoo"
      oldname_opt_record.make_relative_symlink(DINRUSBREW_CELLAR/"foo/1.0")
      keg_record = DINRUSBREW_CELLAR/"foo"/"2.0"
