...
RSpec.describe DinrusBrew::FormulaAuditor do
  include FileUtils
  ...
  let(:origin_tap_path) { DINRUSBREW_TAP_DIRECTORY/"homebrew/homebrew-foo" }
  let(:origin_formula_path) { origin_tap_path/formula_subpath }
  let(:tap_path) { DINRUSBREW_TAP_DIRECTORY/"homebrew/homebrew-bar" }
    ...
    specify "Invalid command" do
      fa = formula_auditor "foo", <<~RUBY
        class Foo < Formula
          ...
          service do
            run [DINRUSBREW_PREFIX/"bin/something"]
          end
        end
      RUBY
  ...
  describe "#audit_github_repository" do
    specify "#audit_github_repository when DINRUSBREW_NO_GITHUB_API is set" do
      ENV["DINRUSBREW_NO_GITHUB_API"] = "1"
  ...
  describe "#audit_github_repository_archived" do
    specify "#audit_github_repository_archived when DINRUSBREW_NO_GITHUB_API is set" do
      fa = formula_auditor "foo", <<~RUBY, strict: true, online: true
        class Foo < Formula
          homepage "https://github.com/example/example"
          url "https://brew.sh/foo-1.0.tgz"
