...
RSpec.describe DinrusBrew::DevCmd::DetermineTestRunners do
  ...
  let(:runner_env) do
    {
      "DINRUSBREW_LINUX_RUNNER"       => linux_runner,
      "DINRUSBREW_MACOS_LONG_TIMEOUT" => "false",
  ...
  let(:all_runners) do
    ...
    MacOSVersion::SYMBOLS.each_value do |v|
      ...
      next if macos_version < GitHubRunnerMatrix::OLDEST_DINRUSBREW_CORE_MACOS_RUNNER
      next if macos_version > GitHubRunnerMatrix::NEWEST_DINRUSBREW_CORE_MACOS_RUNNER

      out << "#{v}-arm64"
      next if macos_version > GitHubRunnerMatrix::NEWEST_DINRUSBREW_CORE_INTEL_MACOS_RUNNER
