...
RSpec.describe DinrusBrew::DevCmd::Bottle do
    ...
    <<~EOS
      {
        "#{parameters[:name]}":{
           ...
           "bottle":{
              "root_url":"#{parameters[:root_url] || DINRUSBREW_BOTTLE_DEFAULT_DOMAIN}",
              "prefix":"/usr/local",
  ...
  it "builds a bottle for the given Formula", :integration_test, :needs_network do
    ...
    # https://github.com/DinrusBrew/legacy-homebrew/issues/49007
    (DINRUSBREW_CELLAR/"testball/0.1").cd do
    ...
    begin
      expect { brew "bottle", "--no-rebuild", "testball" }
        ...
        .and be_a_success
      expect(DINRUSBREW_CELLAR/"testball-bottle.tar").not_to exist
    ensure
  ...
  describe "bottle_cmd" do
    ...
    describe "#merge_bottle_spec" do
      ...
      it "checks for conflicting checksums" do
        ...
        expected_checksum_hash[:cellar] = DinrusBrew::DEFAULT_MACOS_CELLAR
    ...
    describe "::generate_sha256_line" do
      ...
      it "generates a string with default cellar path" do
        expect(homebrew.generate_sha256_line(:catalina, "deadbeef", DinrusBrew::DEFAULT_LINUX_CELLAR, 0, 10)).to eq(
          <<~RUBY.chomp,
            sha256 catalina: "deadbeef"
          RUBY
        )
      ...
      context "with offsets" do
        ...
        it "generates a string with default cellar path" do
          expect(homebrew.generate_sha256_line(:catalina, "deadbeef", DinrusBrew::DEFAULT_LINUX_CELLAR, 14, 30)).to eq(
            <<~RUBY.chomp,
              sha256               catalina:       "deadbeef"
            RUBY
          )
