...
RSpec.describe Utils::Analytics do
  describe "::default_package_tags" do
    ...
    it "returns OS_VERSION and prefix when DINRUSBREW_PREFIX is a custom prefix on intel" do
      expect(DinrusBrew).to receive(:default_prefix?).and_return(false).at_least(:once)
    ...
    it "returns OS_VERSION, ARM and prefix when DINRUSBREW_PREFIX is a custom prefix on arm" do
      expect(DinrusBrew).to receive(:default_prefix?).and_return(false).at_least(:once)
      expect(described_class.default_package_tags).to have_key(:arch)
      expect(described_class.default_package_tags[:arch]).to eq DINRUSBREW_PHYSICAL_PROCESSOR
      expect(described_class.default_package_tags).to have_key(:prefix)
      expect(described_class.default_package_tags[:prefix]).to eq "custom-prefix"
    end

    it "returns OS_VERSION, Rosetta and prefix when DINRUSBREW_PREFIX is a custom prefix on Rosetta", :needs_macos do
      expect(DinrusBrew).to receive(:default_prefix?).and_return(false).at_least(:once)
      expect(described_class.default_package_tags).to have_key(:prefix)
      expect(described_class.default_package_tags[:prefix]).to eq "custom-prefix"
    end

    it "does not include prefix when DINRUSBREW_PREFIX is the default prefix" do
      expect(DinrusBrew).to receive(:default_prefix?).and_return(true).at_least(:once)
      expect(described_class.default_package_tags).to have_key(:prefix)
      expect(described_class.default_package_tags[:prefix]).to eq DINRUSBREW_PREFIX.to_s
    end
    ...
    it "includes developer when ENV['DINRUSBREW_DEVELOPER'] is set" do
      expect(DinrusBrew::EnvConfig).to receive(:developer?).and_return(true)
  ...
  describe "::report_package_event" do
    ...
    context "when ENV vars is set" do
      it "returns nil when DINRUSBREW_NO_ANALYTICS is true" do
        ENV["DINRUSBREW_NO_ANALYTICS"] = "true"
        expect(described_class).not_to receive(:report_influx)
        described_class.report_package_event(:install, package_name:, tap_name:,
          on_request:, options:)
      end

      it "returns nil when DINRUSBREW_NO_ANALYTICS_THIS_RUN is true" do
        ENV["DINRUSBREW_NO_ANALYTICS_THIS_RUN"] = "true"
        expect(described_class).not_to receive(:report_influx)
        described_class.report_package_event(:install, package_name:, tap_name:,
          on_request:, options:)
      end

      it "returns nil when DINRUSBREW_ANALYTICS_DEBUG is true" do
        ENV.delete("DINRUSBREW_NO_ANALYTICS_THIS_RUN")
        ENV.delete("DINRUSBREW_NO_ANALYTICS")
        ENV["DINRUSBREW_ANALYTICS_DEBUG"] = "true"
  ...
  describe "::report_command_run" do
    ...
    let(:command_instance) do
      ...
      DinrusBrew::DevCmd::Audit.new(["--tap=homebrew/core"])
    end
  ...
  describe "::report_test_bot_test" do
    ...
    it "passes to the influxdb method" do
      ...
      ENV.delete("DINRUSBREW_NO_ANALYTICS")
      ENV["DINRUSBREW_ANALYTICS_DEBUG"] = "true"
      ENV["DINRUSBREW_TEST_BOT_ANALYTICS"] = "true"
      expect(described_class).to receive(:report_influx).with(
        :test_bot_test,
