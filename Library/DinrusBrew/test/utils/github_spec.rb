...
RSpec.describe GitHub do
  describe "::search_query_string" do
    it "builds a query with the given hash parameters formatted as key:value" do
      query = described_class.search_query_string(user: "DinrusBrew", repo: "brew")
    ...
    it "adds a variable number of top-level string parameters to the query when provided" do
      query = described_class.search_query_string("value1", "value2", user: "DinrusBrew")
    ...
    it "turns array values into multiple key:value parameters" do
      query = described_class.search_query_string(user: ["DinrusBrew", "caskroom"])
  ...
  describe "::search_issues", :needs_network do
    it "queries GitHub issues with the passed parameters" do
      results = described_class.search_issues("brew search",
                                              repo:   "DinrusBrew/legacy-homebrew",
                                              author: "MikeMcQuaid",
                                              is:     "issue",
                                              no:     "milestone")
  ...
  describe "::approved_reviews", :needs_network do
    it "can get reviews for a pull request" do
      reviews = described_class.approved_reviews("DinrusBrew", "homebrew-core", 1, commit: "deadbeef")
  ...
  describe "::public_member_usernames", :needs_network do
    it "gets the usernames of all publicly visible members of the organisation" do
      response = described_class.public_member_usernames("DinrusBrew")
  ...
  describe "::get_artifact_urls", :needs_network do
    it "fails to find a nonexistent workflow" do
      expect do
        described_class.get_artifact_urls(
          described_class.get_workflow_run("DinrusBrew", "homebrew-core", "1"),
        )
    ...
    it "fails to find artifacts that don't exist" do
      expect do
        described_class.get_artifact_urls(
          described_class.get_workflow_run("DinrusBrew", "homebrew-core", "191680",
    ...
    it "gets artifact URLs" do
      ...
      expect(urls).to eq(["https://api.github.com/repos/DinrusBrew/homebrew-core/actions/artifacts/1969725476/zip"])
    end

    it "supports pattern matching" do
      urls = described_class.get_artifact_urls(
        described_class.get_workflow_run("DinrusBrew", "brew", "17068",
                                         workflow_id: "pkg-installer.yml", artifact_pattern: "DinrusBrew-*.pkg"),
      )
      expect(urls).to eq(["https://api.github.com/repos/DinrusBrew/brew/actions/artifacts/1405050842/zip"])
    end
  end
  ...
  describe "::pull_request_commits", :needs_network do
    ...
    it "gets commit hashes for a pull request" do
      expect(described_class.pull_request_commits("DinrusBrew", "legacy-homebrew", 50678)).to eq(hashes)
    end

    it "gets commit hashes for a paginated pull request API response" do
      expect(described_class.pull_request_commits("DinrusBrew", "legacy-homebrew", 50678, per_page: 1)).to eq(hashes)
    end
  end
