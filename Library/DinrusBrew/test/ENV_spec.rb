  ...
  describe Superenv do
    ...
    describe "#cxx11" do
      it "supports gcc-11" do
        env["DINRUSBREW_CC"] = "gcc-11"
        env.cxx11
        expect(env["DINRUSBREW_CCCFG"]).to include("x")
        expect(env["DINRUSBREW_CCCFG"]).not_to include("g")
      end

      it "supports clang" do
        env["DINRUSBREW_CC"] = "clang"
        env.cxx11
        expect(env["DINRUSBREW_CCCFG"]).to include("x")
        expect(env["DINRUSBREW_CCCFG"]).to include("g")
      end
    end
    ...
    describe "#set_debug_symbols" do
      it "sets the debug symbols flag" do
        ...
        expect(env["DINRUSBREW_CCCFG"]).to include("D")
      end
    end
  end
end
