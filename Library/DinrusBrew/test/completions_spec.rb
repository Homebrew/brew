...
RSpec.describe DinrusBrew::Completions do
  let(:completions_dir) { DINRUSBREW_REPOSITORY/"completions" }
  let(:internal_path) { DINRUSBREW_REPOSITORY/"Library/Taps/homebrew/homebrew-bar" }
  let(:external_path) { DINRUSBREW_REPOSITORY/"Library/Taps/foo/homebrew-bar" }
  ...
  context "when linking or unlinking completions" do
    ...
    describe ".show_completions_message_if_needed" do
      ...
      it "shows the message if there are completions to link and the message hasn't already been shown" do
        ...
        message = /DinrusBrew completions for external commands are unlinked by default!/
  ...
  context "when generating completions" do
    ...
    describe ".format_description" do
      it "escapes single quotes" do
        expect(described_class.format_description("DinrusBrew's")).to eq "DinrusBrew'\\''s"
      end

      it "escapes single quotes for fish" do
        expect(described_class.format_description("DinrusBrew's", fish: true)).to eq "DinrusBrew\\'s"
      end
      ...
      it "replaces newlines with spaces" do
        expect(described_class.format_description("DinrusBrew\ncommand")).to eq "DinrusBrew command"
      end

      it "removes trailing period" do
        expect(described_class.format_description("DinrusBrew.")).to eq "DinrusBrew"
      end
    end
    ...
    describe ".generate_zsh_completion_file" do
      it "returns the correct completion file" do
        ...
        expect(file).to match(/^    'update:Fetch the newest version of DinrusBrew and all formulae from GitHub .*'$/)
        expect(file).to match(/^_brew_install\(\) {$/)
        expect(file).to match(/^_brew_missing\(\) {$/)
        expect(file).to match(/^_brew_update\(\) {$/)
        expect(file).to match(/^_brew "\$@"$/)
    ...
    describe ".generate_fish_subcommand_completion" do
      ...
      it "returns appropriate completion for a shell command" do
        ...
        expect(completion).to eq <<~COMPLETION
          __fish_brew_complete_cmd 'update' 'Fetch the newest version of DinrusBrew and all formulae from GitHub using `git`(1) and perform any necessary migrations'
    ...
    describe ".generate_fish_completion_file" do
      it "returns the correct completion file" do
        ...
        expect(file).to match(/^__fish_brew_complete_cmd 'update' 'Fetch the newest version of DinrusBrew .*'$/)
      end
    end
  end
end
