...
if ENV["DINRUSBREW_TESTS_COVERAGE"]
  require "simplecov"
...
$LOAD_PATH.unshift(File.expand_path("#{ENV.fetch("DINRUSBREW_LIBRARY")}/DinrusBrew/test/support/lib"))

require_relative "support/extend/cachable"

require_relative "../global"

require "debug" if ENV["DINRUSBREW_DEBUG"]
...
RSpec.configure do |config|
  ...
  config.before(:each, :needs_network) do
    skip "Requires network connection." unless ENV["DINRUSBREW_TEST_ONLINE"]
  end

  config.before(:each, :needs_homebrew_core) do
    core_tap_path = "#{ENV.fetch("DINRUSBREW_LIBRARY")}/Taps/homebrew/homebrew-core"
  ...
  config.before(:each, :needs_svn) do
    svn_shim = DINRUSBREW_SHIMS_PATH/"shared/svn"
  ...
  config.before(:each, :needs_homebrew_curl) do
    ENV["DINRUSBREW_CURL"] = DINRUSBREW_BREWED_CURL_PATH
  ...
  config.around do |example|
    DinrusBrew.raise_deprecation_exceptions = true
    ...
    @__homebrew_failed = DinrusBrew.failed?
    ...
    begin
      if example.metadata.keys.exclude?(:focus) && !ENV.key?("DINRUSBREW_VERBOSE_TESTS")
      ...
      FileUtils.rm_rf [
        ...
        *Keg.must_exist_subdirectories,
        DINRUSBREW_LINKED_KEGS,
        DINRUSBREW_PINNED_KEGS,
        DINRUSBREW_PREFIX/"Caskroom",
        DINRUSBREW_PREFIX/"Frameworks",
        DINRUSBREW_LIBRARY/"Taps/homebrew/homebrew-cask",
        DINRUSBREW_LIBRARY/"Taps/homebrew/homebrew-bar",
        DINRUSBREW_LIBRARY/"Taps/homebrew/homebrew-bundle",
        DINRUSBREW_LIBRARY/"Taps/homebrew/homebrew-foo",
        DINRUSBREW_LIBRARY/"Taps/homebrew/homebrew-services",
        DINRUSBREW_LIBRARY/"Taps/homebrew/homebrew-shallow",
        DINRUSBREW_LIBRARY/"PinnedTaps",
        DINRUSBREW_REPOSITORY/".git",
        ...
        *Pathname.glob("#{DINRUSBREW_CELLAR}/*/"),
      ]
      ...
      DinrusBrew.failed = @__homebrew_failed
    end
  end
end
