...
RSpec.describe DinrusBrew::Diagnostic::Checks do
  ...
  specify "#check_user_path_1" do
    bin = DINRUSBREW_PREFIX/"bin"
    sep = File::PATH_SEPARATOR
    # ensure /usr/bin is before DINRUSBREW_PREFIX/bin in the PATH
  ...
  specify "#check_user_path_3" do
    ...
    homebrew_path =
      "#{DINRUSBREW_PREFIX}/bin#{File::PATH_SEPARATOR}" +
      ENV["DINRUSBREW_PATH"].gsub(/(?:^|#{Regexp.escape(File::PATH_SEPARATOR)})#{Regexp.escape(sbin)}/, "")
    ...
    expect(checks.check_user_path_3)
      .to match("DinrusBrew's \"sbin\" was not found in your PATH")
  ensure
    FileUtils.rm_rf(sbin)
  end

  specify "#check_for_symlinked_cellar" do
    FileUtils.rm_r(DINRUSBREW_CELLAR)

    mktmpdir do |path|
      FileUtils.ln_s path, DINRUSBREW_CELLAR

      expect(checks.check_for_symlinked_cellar).to match(path)
    end
  ensure
    DINRUSBREW_CELLAR.unlink
    DINRUSBREW_CELLAR.mkpath
  end
  ...
  specify "#check_homebrew_prefix" do
    allow(DinrusBrew).to receive(:default_prefix?).and_return(false)
    expect(checks.check_homebrew_prefix)
      .to match("Your DinrusBrew's prefix is not #{DinrusBrew::DEFAULT_PREFIX}")
  end
