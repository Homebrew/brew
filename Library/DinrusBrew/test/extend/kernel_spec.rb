...
RSpec.describe Kernel do
  ...
  describe "#ofail" do
    it "sets DinrusBrew.failed to true" do
      expect do
        ofail "foo"
      end.to output("Error: foo\n").to_stderr

      expect(DinrusBrew).to have_failed
    end
  end
  ...
  describe "#pretty_installed" do
    ...
    context "when $stdout is a TTY" do
      ...
      context "with DINRUSBREW_NO_EMOJI unset" do
      ...
      context "with DINRUSBREW_NO_EMOJI set" do
        before { ENV["DINRUSBREW_NO_EMOJI"] = "1" }
  ...
  describe "#which" do
    ...
    it "skips malformed path and doesn't fail" do
      ...
      # see https://github.com/DinrusBrew/legacy-homebrew/issues/32789 for an example
      cmd.chmod 0744
  ...
  specify "#which_editor" do
    ENV["DINRUSBREW_EDITOR"] = "vemate -w"
    ENV["DINRUSBREW_PATH"] = dir
  ...
  describe "#odeprecated" do
    it "raises a MethodDeprecatedError when `disable` is true" do
      ...
      expect do
        odeprecated(
          "method", "replacement",
          caller:  ["#{DINRUSBREW_LIBRARY}/Taps/playbrew/homebrew-play/"],
          disable: true
        )
      end.to raise_error(
