...
RSpec.shared_context "custom internal commands" do # rubocop:disable RSpec/ContextWording
  let(:cmds) do
    [
      # internal commands
      Commands::DINRUSBREW_CMD_PATH/"rbcmd.rb",
      Commands::DINRUSBREW_CMD_PATH/"shcmd.sh",

      # internal developer-commands
      Commands::DINRUSBREW_DEV_CMD_PATH/"rbdevcmd.rb",
      Commands::DINRUSBREW_DEV_CMD_PATH/"shdevcmd.sh",
    ]
  end
...
RSpec.describe Commands do
  ...
  describe "::path" do
    specify "returns the path for an internal command" do
      expect(described_class.path("rbcmd")).to eq(DINRUSBREW_LIBRARY_PATH/"cmd/rbcmd.rb")
      expect(described_class.path("shcmd")).to eq(DINRUSBREW_LIBRARY_PATH/"cmd/shcmd.sh")
      expect(described_class.path("idontexist1234")).to be_nil
    end

    specify "returns the path for an internal developer-command" do
      expect(described_class.path("rbdevcmd")).to eq(DINRUSBREW_LIBRARY_PATH/"dev-cmd/rbdevcmd.rb")
      expect(described_class.path("shdevcmd")).to eq(DINRUSBREW_LIBRARY_PATH/"dev-cmd/shdevcmd.sh")
    end
  end
end
