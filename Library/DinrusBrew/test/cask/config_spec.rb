...
RSpec.describe Cask::Config, :cask do
  ...
  describe "#appdir" do
    it "returns the default value if no DINRUSBREW_CASK_OPTS is unset" do
      expect(config.appdir).to eq(Pathname(TEST_TMPDIR).join("cask-appdir"))
    end

    specify "environment overwrites default" do
      ENV["DINRUSBREW_CASK_OPTS"] = "--appdir=/path/to/apps"
  ...
  describe "#env" do
    it "returns directories specified with the DINRUSBREW_CASK_OPTS variable" do
      ENV["DINRUSBREW_CASK_OPTS"] = "--appdir=/path/to/apps"
