  ...
  matcher :be_patched_with_homebrew_prefix do
    match do |formula|
      formula.brew do
        ...
        s = File.read("libexec/NOOP")
        expect(s).not_to include("NOOP"), "libexec/NOOP was not patched as expected"
        expect(s).not_to include("@@DINRUSBREW_PREFIX@@"), "libexec/NOOP was not patched as expected"
        expect(s).to include(DINRUSBREW_PREFIX.to_s), "libexec/NOOP was not patched as expected"
      end
    end
  end
