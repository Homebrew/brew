...
RSpec.describe "Bash" do
  ...
  describe "brew" do
    subject { DINRUSBREW_LIBRARY_PATH.parent.parent/"bin/brew" }
  ...
  describe "every `.sh` file" do
    it "has valid Bash syntax" do
      Pathname.glob("#{DINRUSBREW_LIBRARY_PATH}/**/*.sh").each do |path|
        relative_path = path.relative_path_from(DINRUSBREW_LIBRARY_PATH)
  ...
  describe "Bash completion" do
    subject { DINRUSBREW_LIBRARY_PATH.parent.parent/"completions/bash/brew" }
  ...
  describe "every shim script" do
    it "has valid Bash syntax" do
      ...
      (DINRUSBREW_LIBRARY_PATH/"shims").find do |path|
