...
RSpec.describe DinrusBrew::Search do
  ...
  describe "#search_descriptions" do
    let(:args) { DinrusBrew::Cmd::Desc.new(["min_arg_placeholder"]).args }
    ...
    context "with api" do
      ...
      before do
        allow(DinrusBrew::API::Formula).to receive(:all_formulae).and_return(api_formulae)
        allow(DinrusBrew::API::Cask).to receive(:all_casks).and_return(api_casks)
      end
