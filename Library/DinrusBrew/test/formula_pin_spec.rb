...
RSpec.describe FormulaPin do
  ...
  let(:name) { "double" }
  let(:formula) { instance_double(Formula, name:, rack: DINRUSBREW_CELLAR/name) }
  ...
  specify "#pin and #unpin" do
    ...
    expect(DINRUSBREW_PINNED_KEGS/name).to be_a_directory
    expect(DINRUSBREW_PINNED_KEGS.children.count).to eq(1)

    formula_pin.unpin
    expect(formula_pin).not_to be_pinned
    expect(DINRUSBREW_PINNED_KEGS).not_to be_a_directory
  end
end
