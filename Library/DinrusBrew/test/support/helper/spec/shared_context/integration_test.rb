...
RSpec.shared_context "integration test" do # rubocop:disable RSpec/ContextWording
  ...
  around do |example|
    ENV["DINRUSBREW_INTEGRATION_TEST"] = "1"
    (DINRUSBREW_PREFIX/"bin").mkpath
    FileUtils.touch DINRUSBREW_PREFIX/"bin/brew"

    example.run
  ensure
    FileUtils.rm_rf DINRUSBREW_PREFIX/"bin"
    ENV.delete("DINRUSBREW_INTEGRATION_TEST")
  end
  ...
  def brew(*args)
    ...
    # Avoid warnings when DINRUSBREW_PREFIX/bin is not in PATH.
    # Also include our extra commands directory.
    path = [
      env["PATH"],
      (DINRUSBREW_LIBRARY_PATH/"test/support/helper/cmd").realpath.to_s,
      (DINRUSBREW_PREFIX/"bin").realpath.to_s,
      ENV.fetch("PATH"),
    ].compact.join(File::PATH_SEPARATOR)

    env.merge!(
      "PATH"                        => path,
      "DINRUSBREW_PATH"               => path,
      "DINRUSBREW_BREW_FILE"          => DINRUSBREW_PREFIX/"bin/brew",
      "DINRUSBREW_INTEGRATION_TEST"   => command_id,
      "DINRUSBREW_TEST_TMPDIR"        => TEST_TMPDIR,
      "DINRUSBREW_DEV_CMD_RUN"        => "true",
      "DINRUSBREW_USE_RUBY_FROM_PATH" => ENV.fetch("DINRUSBREW_USE_RUBY_FROM_PATH", nil),
      "GEM_HOME"                    => nil,
    )
    ...
    @ruby_args ||= begin
      ...
      ruby_args << "-r#{DINRUSBREW_LIBRARY_PATH}/test/support/helper/integration_mocks"
      ruby_args << "-e" << "$0 = ARGV.shift; load($0)"
      ruby_args << (DINRUSBREW_LIBRARY_PATH/"brew.rb").resolved_path.to_s
    end
  ...
  def brew_sh(*args)
    ...
    Bundler.with_unbundled_env do
      stdout, stderr, status = Open3.capture3(env, "#{ENV.fetch("DINRUSBREW_PREFIX")}/bin/brew", *args)
      $stdout.print stdout
      $stderr.print stderr
      status
    end
  ...
  def setup_test_tap
    path = DINRUSBREW_TAP_DIRECTORY/"homebrew/homebrew-foo"
    path.mkpath
    path.cd do
      system "git", "init"
  ...
  def setup_remote_tap(name)
    Tap.fetch(name).tap do |tap|
      ...
      # Check to see if the original DinrusBrew process has taps we can use.
      system_tap_path = Pathname("#{ENV.fetch("DINRUSBREW_LIBRARY")}/Taps/#{full_name}")
