...
RSpec.describe Tap do
  ...
  subject(:homebrew_foo_tap) { described_class.fetch("DinrusBrew", "foo") }

  let(:path) { DINRUSBREW_TAP_DIRECTORY/"homebrew/homebrew-foo" }
  ...
  def setup_git_repo
    path.cd do
      system "git", "init"
      system "git", "remote", "add", "origin", "https://github.com/DinrusBrew/homebrew-foo"
      system "git", "add", "--all"
  ...
  specify "::fetch" do
    expect(described_class.fetch("DinrusBrew", "core")).to be_a(CoreTap)
    expect(described_class.fetch("DinrusBrew", "homebrew")).to be_a(CoreTap)
    tap = described_class.fetch("DinrusBrew", "foo")
  ...
  describe "::from_path" do
    let(:tap) { described_class.fetch("DinrusBrew", "core") }
    let(:path) { tap.path }
  ...
  describe "::allowed_taps" do
    before { allow(DinrusBrew::EnvConfig).to receive(:allowed_taps).and_return("homebrew/allowed") }
  ...
  describe "::forbidden_taps" do
    before { allow(DinrusBrew::EnvConfig).to receive(:forbidden_taps).and_return("homebrew/forbidden") }
  ...
  specify "attributes" do
    expect(homebrew_foo_tap.user).to eq("DinrusBrew")
  ...
  specify "#issues_url" do
    ...
    path = DINRUSBREW_TAP_DIRECTORY/"someone/homebrew-foo"
    path.mkpath
    cd path do
      system "git", "init"
    ...
    expect(homebrew_foo_tap.issues_url).to eq("https://github.com/DinrusBrew/homebrew-foo/issues")

    (DINRUSBREW_TAP_DIRECTORY/"someone/homebrew-no-git").mkpath
  ...
  describe "#remote" do
    it "returns the remote URL", :needs_network do
      ...
      expect(homebrew_foo_tap.remote).to eq("https://github.com/DinrusBrew/homebrew-foo")
      expect(homebrew_foo_tap).not_to have_custom_remote

      services_tap = described_class.fetch("DinrusBrew", "services")
      services_tap.path.mkpath
      services_tap.path.cd do
        system "git", "init"
        system "git", "remote", "add", "origin", "https://github.com/DinrusBrew/homebrew-services"
      end
  ...
  describe "#remote_repo" do
    it "returns the remote https repository" do
      ...
      expect(homebrew_foo_tap.remote_repository).to eq("DinrusBrew/homebrew-foo")

      services_tap = described_class.fetch("DinrusBrew", "services")
      services_tap.path.mkpath
      services_tap.path.cd do
        system "git", "init"
        system "git", "remote", "add", "origin", "https://github.com/DinrusBrew/homebrew-bar"
      end
      expect(services_tap.remote_repository).to eq("DinrusBrew/homebrew-bar")
    end
    ...
    it "returns the remote ssh repository" do
      ...
      services_tap.path.cd do
        system "git", "init"
        system "git", "remote", "add", "origin", "git@github.com:DinrusBrew/homebrew-bar"
      end
      expect(services_tap.remote_repository).to eq("DinrusBrew/homebrew-bar")
    end
  ...
  describe "#custom_remote?" do
    subject(:tap) { described_class.fetch("DinrusBrew", "services") }
    ...
    context "when using the default remote" do
      let(:remote) { "https://github.com/DinrusBrew/homebrew-services" }

      it(:custom_remote?) { expect(tap.custom_remote?).to be false }
    end

    context "when using a non-default remote" do
      let(:remote) { "git@github.com:DinrusBrew/homebrew-services" }
  ...
  describe "#install" do
    it "raises an error when the Tap is already tapped" do
      setup_git_repo
      already_tapped_tap = described_class.fetch("DinrusBrew", "foo")
    ...
    it "raises an error when the remote for DinrusBrew/core doesn't match DINRUSBREW_CORE_GIT_REMOTE" do
      core_tap = described_class.fetch("DinrusBrew", "core")
      wrong_remote = "#{DinrusBrew::EnvConfig.core_git_remote}-oops"
      expect do
    ...
    it "raises an error when run `brew tap --custom-remote` without a custom remote (not installed)" do
      not_tapped_tap = described_class.fetch("DinrusBrew", "bar")
    ...
    specify "Git error" do
      ...
      expect(DINRUSBREW_TAP_DIRECTORY/"user").not_to exist
    end
  end

  describe "#uninstall" do
    it "raises an error if the Tap is not available" do
      tap = described_class.fetch("DinrusBrew", "bar")
  ...
  specify "#install and #uninstall" do
    ...
    expect(DINRUSBREW_PREFIX/"share/man/man1/brew-tap-cmd.1").to be_a_file
    expect(DINRUSBREW_PREFIX/"etc/bash_completion.d/brew-tap-cmd").to be_a_file
    expect(DINRUSBREW_PREFIX/"share/zsh/site-functions/_brew-tap-cmd").to be_a_file
    expect(DINRUSBREW_PREFIX/"share/fish/vendor_completions.d/brew-tap-cmd.fish").to be_a_file
    tap.uninstall

    expect(tap).not_to be_installed
    expect(DINRUSBREW_PREFIX/"share/man/man1/brew-tap-cmd.1").not_to exist
    expect(DINRUSBREW_PREFIX/"share/man/man1").not_to exist
    expect(DINRUSBREW_PREFIX/"etc/bash_completion.d/brew-tap-cmd").not_to exist
    expect(DINRUSBREW_PREFIX/"share/zsh/site-functions/_brew-tap-cmd").not_to exist
    expect(DINRUSBREW_PREFIX/"share/fish/vendor_completions.d/brew-tap-cmd.fish").not_to exist
  ensure
    FileUtils.rm_r(DINRUSBREW_PREFIX/"etc") if (DINRUSBREW_PREFIX/"etc").exist?
    FileUtils.rm_r(DINRUSBREW_PREFIX/"share") if (DINRUSBREW_PREFIX/"share").exist?
  end
  ...
  specify "#link_completions_and_manpages when completions are enabled for non-official tap" do
    ...
    tap.install clone_target: homebrew_foo_tap.path/".git"
    (DINRUSBREW_PREFIX/"share/man/man1/brew-tap-cmd.1").delete
    (DINRUSBREW_PREFIX/"etc/bash_completion.d/brew-tap-cmd").delete
    (DINRUSBREW_PREFIX/"share/zsh/site-functions/_brew-tap-cmd").delete
    (DINRUSBREW_PREFIX/"share/fish/vendor_completions.d/brew-tap-cmd.fish").delete
  ...
  specify "#link_completions_and_manpages when completions are disabled for non-official tap" do
    ...
    tap.link_completions_and_manpages
    expect(DINRUSBREW_PREFIX/"share/man/man1/brew-tap-cmd.1").to be_a_file
    expect(DINRUSBREW_PREFIX/"etc/bash_completion.d/brew-tap-cmd").not_to be_a_file
    expect(DINRUSBREW_PREFIX/"share/zsh/site-functions/_brew-tap-cmd").not_to be_a_file
    expect(DINRUSBREW_PREFIX/"share/fish/vendor_completions.d/brew-tap-cmd.fish").not_to be_a_file
    tap.uninstall
  ensure
    FileUtils.rm_r(DINRUSBREW_PREFIX/"etc") if (DINRUSBREW_PREFIX/"etc").exist?
    FileUtils.rm_r(DINRUSBREW_PREFIX/"share") if (DINRUSBREW_PREFIX/"share").exist?
  ...
  specify "#link_completions_and_manpages when completions are enabled for official tap" do
    ...
    setup_git_repo
    setup_completion link: false
    tap = described_class.fetch("DinrusBrew", "baz")
    tap.install clone_target: homebrew_foo_tap.path/".git"
    (DINRUSBREW_PREFIX/"share/man/man1/brew-tap-cmd.1").delete
    (DINRUSBREW_PREFIX/"etc/bash_completion.d/brew-tap-cmd").delete
    (DINRUSBREW_PREFIX/"share/zsh/site-functions/_brew-tap-cmd").delete
  ...
  describe ".each" do
    ...
    context "when the core tap is not installed" do
      ...
      it "includes the core tap with the api" do
        ENV.delete("DINRUSBREW_NO_INSTALL_FROM_API")
  ...
  describe CoreTap do
    ...
    specify "attributes" do
      expect(core_tap.user).to eq("DinrusBrew")
    ...
    specify "files" do
      path = DINRUSBREW_TAP_DIRECTORY/"homebrew/homebrew-core"
  ...
  describe "#repository_var_suffix" do
    it "converts the repo directory to an environment variable suffix" do
      expect(CoreTap.instance.repository_var_suffix).to eq "_DINRUSBREW_DINRUSBREW_CORE"
    end

    it "converts non-alphanumeric characters to underscores" do
      expect(described_class.fetch("my",
                                   "tap-with-dashes").repository_var_suffix).to eq "_MY_DINRUSBREW_TAP_WITH_DASHES"
      expect(described_class.fetch("my",
                                   "tap-with-@-symbol").repository_var_suffix).to eq "_MY_DINRUSBREW_TAP_WITH___SYMBOL"
    end
  end
