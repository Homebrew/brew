...
module DinrusBrew
  ...
  module Completions
    ...
    COMPLETIONS_DIR = T.let((DINRUSBREW_REPOSITORY/"completions").freeze, Pathname)
    TEMPLATE_DIR = T.let((DINRUSBREW_LIBRARY_PATH/"completions").freeze, Pathname)
    ...
    BASH_NAMED_ARGS_COMPLETION_FUNCTION_MAPPING = T.let({
      ...
      diagnostic_check:  '__brewcomp "${__DINRUSBREW_DOCTOR_CHECKS=$(brew doctor --list-checks)}"',
    ...
    def self.show_completions_message_if_needed
      ...
      ohai "DinrusBrew completions for external commands are unlinked by default!"
      puts <<~EOS
    ...
    def self.update_shell_completions!
      ...
      puts "Записываются комплеции в #{COMPLETIONS_DIR}"
    ...
    def self.generate_zsh_completion_file(commands)
      variables = Variables.new(
        aliases:                      Commands::DINRUSBREW_INTERNAL_COMMAND_ALIASES.filter_map do |alias_cmd, command|
        ...
        builtin_command_descriptions: commands.filter_map do |command|
          next if Commands::DINRUSBREW_INTERNAL_COMMAND_ALIASES.key? command
