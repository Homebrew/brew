  ...
  class SimulateSystem
    class << self
      ...
      def with(os: T.unsafe(nil), arch: T.unsafe(nil), &_block)
        raise ArgumentError, "Надо указать хотя бы одно из `os` или `arch`." if !os && !arch
      ...
      def os=(new_os)
        ...
        raise "Неизвестная ОС: #{new_os}" unless os_options.include?(new_os)
      ...
      def arch=(new_arch)
        raise "Новая архитектура должна быть :arm или :intel" unless OnSystem::ARCH_OPTIONS.include?(new_arch)
