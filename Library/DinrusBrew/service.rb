...
module DinrusBrew
  ...
  class Service
    ...
    def name(macos: nil, linux: nil)
      raise TypeError, "В имени службы#name ожидается хотя бы одна String" if [macos, linux].none?(String)
    ...
    def keep_alive(value = nil)
      ...
      when Hash
        unless (value.keys - KEEP_ALIVE_KEYS).empty?
          raise TypeError, "Служба#keep_alive позволяет только #{KEEP_ALIVE_KEYS}"
        end
    ...
    def std_service_path_env
      "#{DINRUSBREW_PREFIX}/bin:#{DINRUSBREW_PREFIX}/sbin:/usr/bin:/bin:/usr/sbin:/sbin"
    end
    ...
    def to_systemd_unit
      ...
      <<~SYSTEMD
        [Unit]
        Description=DinrusBrew generated unit for #{@formula.name}
    ...
    def to_systemd_timer
      ...
      <<~SYSTEMD
        [Unit]
        Description=DinrusBrew generated timer for #{@formula.name}
      ...
      sockets_var = if @sockets.present?
        @sockets.transform_values { |info| "#{info[:type]}://#{info[:host]}:#{info[:port]}" }
                .then do |sockets_hash|
                  # TODO: Remove this code when all users are running on versions of DinrusBrew
    ...
    def self.from_hash(api_hash)
      ...
      hash[:run] =
        ...
        when Hash
          ...
          raise ArgumentError, "Неожидаемая команда run: #{api_hash["run"]}"
        end
    ...
    def self.replace_placeholders(string)
      string.gsub(DINRUSBREW_PREFIX_PLACEHOLDER, DINRUSBREW_PREFIX)
