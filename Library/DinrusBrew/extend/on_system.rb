...
module OnSystem
  ...
  def self.arch_condition_met?(arch)
    raise ArgumentError, "Неполнценное архитектурное условие: #{arch.inspect}" if ARCH_OPTIONS.exclude?(arch)
  ...
  def self.os_condition_met?(os_name, or_condition = nil)
    return DinrusBrew::SimulateSystem.send(:"simulating_or_running_on_#{os_name}?") if BASE_OS_OPTIONS.include?(os_name)

    raise ArgumentError, "Неполноценное устловие ОС: #{os_name.inspect}" unless MacOSVersion::SYMBOLS.key?(os_name)

    if or_condition.present? && [:or_newer, :or_older].exclude?(or_condition)
      raise ArgumentError, "Ненполноценное условие ОС `or_*`: #{or_condition.inspect}"
    end

    return false if DinrusBrew::SimulateSystem.simulating_or_running_on_linux?

    base_os = MacOSVersion.from_symbol(os_name)
    current_os = if DinrusBrew::SimulateSystem.current_os == :macos
      # Assume the oldest macOS version when simulating a generic macOS version
      # Version::NULL is always treated as less than any other version.
      Version::NULL
    else
      MacOSVersion.from_symbol(DinrusBrew::SimulateSystem.current_os)
    end
  ...
  def self.setup_base_os_methods(base)
    ...
    base.define_method(:on_system) do |linux, macos:, &block|
      ...
      raise ArgumentError, "Первым аргументом к `on_system` должен быть `:linux`" if linux != :linux
    ...
    MacOSVersion::SYMBOLS.each_key do |os_name|
      base.define_method(:"on_#{os_name}") do |or_condition = nil, &block|
        ...
        @on_system_block_min_os = if or_condition == :or_older
          @called_in_on_system_block ? @on_system_block_min_os : MacOSVersion.new(DINRUSBREW_MACOS_OLDEST_ALLOWED)
        else
  ...
  def self.included(_base)
    raise "Не включайте `OnSystem` прямо. Замените его на`OnSystem::MacOSAndLinux` или `OnSystem::MacOSOnly`"
  end
