...
# Contains shorthand DinrusBrew utility methods like `ohai`, `opoo`, `odisabled`.
...
module Kernel
  ...
  def opoo(message)
    ...
    Tty.with($stderr) do |stderr|
      stderr.puts Formatter.warning(message, label: "Предупреждение")
    end
  end
  ...
  def onoe(message)
    ...
    Tty.with($stderr) do |stderr|
      stderr.puts Formatter.error(message, label: "Ошибка")
    end
  end
  ...
  def odeprecated(method, replacement = nil,
    ...
    replacement_message = if replacement
      "Используйте вместо него #{replacement}."
    else
      "Замены нет."
    end
    ...
    unless disable_on.nil?
      if disable_on > Time.now
        will_be_disabled_message = " и будет отключен на дату #{disable_on.strftime("%Y-%m-%d")}"
      else
        disable = true
      end
    end
    ...
    return if backtrace.any? do |line|
      next true if line.include?(DINRUSBREW_CACHE.to_s)
      next true if line.include?("/.brew/")
      next true if line.include?("/.metadata/")

      next false unless line.match?(DINRUSBREW_TAP_PATH_REGEX)
    ...
    backtrace.each do |line|
      next unless (match = line.match(DINRUSBREW_TAP_PATH_REGEX))

      require "tap"

      tap = Tap.fetch(match[:user], match[:repository])
      tap_message = "\nPlease report this issue to the #{tap.full_name} tap"
      tap_message += " (not DinrusBrew/brew or DinrusBrew/homebrew-core)" unless tap.official?
    ...
    disable = true if disable_for_developers && DinrusBrew::EnvConfig.developer?
    if disable || DinrusBrew.raise_deprecation_exceptions?
      require "utils/github/actions"
      GitHub::Actions.puts_annotation_if_env_set(:error, message, file:, line:)
      exception = MethodDeprecatedError.new(message)
      exception.set_backtrace(backtrace)
      raise exception
    elsif !DinrusBrew.auditing?
      opoo message
    end
  end
  ...
  def pretty_installed(formula)
    if !$stdout.tty?
      formula.to_s
    elsif DinrusBrew::EnvConfig.no_emoji?
      Formatter.success("#{Tty.bold}#{formula} (установлена)#{Tty.reset}")
    else
  ...
  def pretty_outdated(formula)
    if !$stdout.tty?
      ...
      Formatter.error("#{Tty.bold}#{formula} (устарела)#{Tty.reset}")
    else
  ...
  def pretty_uninstalled(formula)
    if !$stdout.tty?
      ...
      Formatter.error("#{Tty.bold}#{formula} (деинсталирована)#{Tty.reset}")
    else
  ...
  def interactive_shell(formula = nil)
    unless formula.nil?
      ENV["DINRUSBREW_DEBUG_PREFIX"] = formula.prefix
      ENV["DINRUSBREW_DEBUG_INSTALL"] = formula.full_name
    end

    if Utils::Shell.preferred == :zsh && (home = Dir.home).start_with?(DINRUSBREW_TEMP.resolved_path.to_s)
    ...
    raise "Прервано из-за ненулевого статуса выхода (#{$CHILD_STATUS.exitstatus})" if $CHILD_STATUS.exited?
  ...
  def safe_system(cmd, *args, **options)
    ...
    return if DinrusBrew.system(cmd, *args, **options)
  ...
  def quiet_system(cmd, *args)
    ...
    DinrusBrew._system(cmd, *args) do
  ...
  def which(cmd, path = ENV.fetch("PATH"))
    PATH.new(path).each do |p|
      begin
        ...
        # See https://github.com/DinrusBrew/legacy-homebrew/issues/32789
        next
      end
  ...
  def which_editor(silent: false)
    editor = DinrusBrew::EnvConfig.editor
    ...
    unless silent
      opoo <<~EOS
        Используется #{editor}, так как в среде редактор не установлен.
        Рекомендуем установить переменную EDITOR или DINRUSBREW_EDITOR
        с указанием на предпочитаемый вами редактор текста.
      EOS
    end
  ...
  def exec_editor(*args)
    puts "Редактируется #{args.join "\n"}"
    with_homebrew_path { safe_system(*which_editor.shellsplit, *args) }
  end

  def exec_browser(*args)
    browser = DinrusBrew::EnvConfig.browser
    browser ||= OS::PATH_OPEN if defined?(OS::PATH_OPEN)
    return unless browser

    ENV["DISPLAY"] = DinrusBrew::EnvConfig.display

    with_env(DBUS_SESSION_BUS_ADDRESS: ENV.fetch("DINRUSBREW_DBUS_SESSION_BUS_ADDRESS", nil)) do
  ...
  def ignore_interrupts
    IGNORE_INTERRUPTS_MUTEX.synchronize do
      ...
      old_sigint_handler = trap(:INT) do
        ...
        $stderr.print "\n"
        $stderr.puts "Минуточку, проводится зачистка..."
      end
  ...
  def ensure_formula_installed!(formula_or_name, reason: "", latest: false,
    ...
    reason = " для #{reason}" if reason.present?

    unless formula.any_version_installed?
      ohai "Устанавливается `#{formula.name}`#{reason}..."
      safe_system DINRUSBREW_BREW_FILE, "install", "--formula", formula.full_name
    end

    if latest && !formula.latest_version_installed?
      ohai "Проводится апгрейд `#{formula.name}`#{reason}..."
      safe_system DINRUSBREW_BREW_FILE, "upgrade", "--formula", formula.full_name
    end
  ...
  def ensure_executable!(name, formula_name = nil, reason: "", latest: false)
    ...
    executable = [
      ...
      DINRUSBREW_PREFIX/"opt/#{formula_name}/bin/#{name}",
      DINRUSBREW_PREFIX/"bin/#{name}",
    ].compact.first
  ...
  def truncate_text_to_approximate_size(str, max_bytes, options = {})
    ...
    raise "opts[:front_weight] должен быть в диапазоне между 0.0 и 1.0" if front_weight < 0.0 || front_weight > 1.0
