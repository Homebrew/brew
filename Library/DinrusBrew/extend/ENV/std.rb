...
module Stdenv
  ...
  def setup_build_environment(formula: nil, cc: nil, build_bottle: false, bottle_arch: nil, testing_formula: false,
    ...
    self["DINRUSBREW_ENV"] = "std"

    ORIGINAL_PATHS.reverse_each { |p| prepend_path "PATH", p }
    prepend_path "PATH", DINRUSBREW_SHIMS_PATH/"shared"
    ...
    if DINRUSBREW_PREFIX.to_s != "/usr/local"
      # /usr/local is already an -isystem and -L directory so we skip it
      self["CPPFLAGS"] = "-isystem#{DINRUSBREW_PREFIX}/include"
      self["LDFLAGS"] = "-L#{DINRUSBREW_PREFIX}/lib"
      # CMake ignores the variables above
      self["CMAKE_PREFIX_PATH"] = DINRUSBREW_PREFIX.to_s
    end

    frameworks = DINRUSBREW_PREFIX.join("Frameworks")
  ...
  def determine_pkg_config_libdir
    PATH.new(
      DINRUSBREW_PREFIX/"lib/pkgconfig",
      DINRUSBREW_PREFIX/"share/pkgconfig",
