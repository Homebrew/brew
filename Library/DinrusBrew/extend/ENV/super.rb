...
module Superenv
  ...
  # The location of DinrusBrew's shims.
  #
  # @api public
  sig { returns(Pathname) }
  def self.shims_path
    DINRUSBREW_SHIMS_PATH/"super"
  end
  ...
  def setup_build_environment(formula: nil, cc: nil, build_bottle: false, bottle_arch: nil, testing_formula: false,
    ...
    self["DINRUSBREW_ENV"] = "super"
    self["MAKEFLAGS"] ||= "-j#{determine_make_jobs}"
    self["RUSTFLAGS"] = Hardware.rustflags_target_cpu(effective_arch)
    self["PATH"] = determine_path
    self["PKG_CONFIG_PATH"] = determine_pkg_config_path
    self["PKG_CONFIG_LIBDIR"] = determine_pkg_config_libdir || ""
    self["DINRUSBREW_CCCFG"] = determine_cccfg
    self["DINRUSBREW_OPTIMIZATION_LEVEL"] = "Os"
    self["DINRUSBREW_BREW_FILE"] = DINRUSBREW_BREW_FILE.to_s
    self["DINRUSBREW_PREFIX"] = DINRUSBREW_PREFIX.to_s
    self["DINRUSBREW_CELLAR"] = DINRUSBREW_CELLAR.to_s
    self["DINRUSBREW_OPT"] = "#{DINRUSBREW_PREFIX}/opt"
    self["DINRUSBREW_TEMP"] = DINRUSBREW_TEMP.to_s
    self["DINRUSBREW_OPTFLAGS"] = determine_optflags
    self["DINRUSBREW_ARCHFLAGS"] = ""
    self["DINRUSBREW_MAKE_JOBS"] = determine_make_jobs.to_s
    self["CMAKE_PREFIX_PATH"] = determine_cmake_prefix_path
    self["CMAKE_FRAMEWORK_PATH"] = determine_cmake_frameworks_path
    self["CMAKE_INCLUDE_PATH"] = determine_cmake_include_path
    self["CMAKE_LIBRARY_PATH"] = determine_cmake_library_path
    self["ACLOCAL_PATH"] = determine_aclocal_path
    self["M4"] = "#{DINRUSBREW_PREFIX}/opt/m4/bin/m4" if deps.any? { |d| d.name == "libtool" }
    self["DINRUSBREW_ISYSTEM_PATHS"] = determine_isystem_paths
    self["DINRUSBREW_INCLUDE_PATHS"] = determine_include_paths
    self["DINRUSBREW_LIBRARY_PATHS"] = determine_library_paths
    self["DINRUSBREW_DEPENDENCIES"] = determine_dependencies
    self["DINRUSBREW_FORMULA_PREFIX"] = @formula.prefix unless @formula.nil?
    ...
    # The DINRUSBREW_CCCFG ENV variable is used by the ENV/cc tool to control
  ...
  def cc=(val)
    self["DINRUSBREW_CC"] = super
  end

  sig { params(val: T.any(String, Pathname)).returns(String) }
  def cxx=(val)
    self["DINRUSBREW_CXX"] = super
  end
  ...
  def determine_aclocal_path
    PATH.new(
      ...
      DINRUSBREW_PREFIX/"share/aclocal",
    ).existing
  end
  ...
  def determine_library_paths
    ...
    if compiler.match?(GNU_GCC_REGEXP)
      ...
      # so that the linker will find those libs before any that may be linked in $DINRUSBREW_PREFIX/lib.
      # https://github.com/DinrusBrew/brew/pull/11459#issuecomment-851075936
      begin
  ...
  def determine_make_jobs
    DinrusBrew::EnvConfig.make_jobs
  end
  ...
  def deparallelize(&block)
    ...
    old_make_jobs = delete("DINRUSBREW_MAKE_JOBS")
    self["DINRUSBREW_MAKE_JOBS"] = "1"
    if block
      begin
        yield
      ensure
        self["MAKEFLAGS"] = old_makeflags
        self["DINRUSBREW_MAKE_JOBS"] = old_make_jobs
      end
    end
  ...
  def O0(&block)
    if block
      with_env(DINRUSBREW_OPTIMIZATION_LEVEL: "O0", &block)
    else
      self["DINRUSBREW_OPTIMIZATION_LEVEL"] = "O0"
    end
  end
  ...
  def O1(&block)
    if block
      with_env(DINRUSBREW_OPTIMIZATION_LEVEL: "O1", &block)
    else
      self["DINRUSBREW_OPTIMIZATION_LEVEL"] = "O1"
    end
  end
  ...
  def O3(&block)
    if block
      with_env(DINRUSBREW_OPTIMIZATION_LEVEL: "O3", &block)
    else
      self["DINRUSBREW_OPTIMIZATION_LEVEL"] = "O3"
    end
  end
