...
# DinrusBrew extends Ruby's `ENV` to make our code more readable.
...
module SharedEnvExtension
  ...
  def compiler
    @compiler ||= if (cc = @cc)
      ...
      compiler = fetch_compiler(cc, "DINRUSBREW_CC")
  ...
  def fortran
    ...
    # despite it often being the DinrusBrew-provided one set up in the first call.
    ...
    if fc
      ...
      if (gfortran = which("gfortran", (DINRUSBREW_PREFIX/"bin").to_s))
        ohai "Using DinrusBrew-provided Fortran compiler"
  ...
  def warn_about_non_apple_gcc(name)
    begin
      ...
      raise <<~EOS
        DinrusBrew GCC requested, but formula #{e.name} not found!
      EOS
    end
    ...
    raise <<~EOS
      The requested DinrusBrew GCC was not installed. You must:
  ...
  sig { returns(Integer) }
  def make_jobs
    DinrusBrew::EnvConfig.make_jobs.to_i
  end
