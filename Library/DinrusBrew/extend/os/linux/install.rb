...
module DinrusBrew
  module Install
    ...
    def self.check_cpu
      ...
      message = "Sorry, DinrusBrew does not support your computer's CPU architecture!"
    ...
    def self.symlink_ld_so
      brew_ld_so = DINRUSBREW_PREFIX/"lib/ld.so"

      ld_so = DINRUSBREW_PREFIX/"opt/glibc/bin/ld.so"
      ...
      FileUtils.mkdir_p DINRUSBREW_PREFIX/"lib"
      FileUtils.ln_sf ld_so, brew_ld_so
    end
    private_class_method :symlink_ld_so

    def self.setup_preferred_gcc_libs
      gcc_opt_prefix = DINRUSBREW_PREFIX/"opt/#{OS::LINUX_PREFERRED_GCC_RUNTIME_FORMULA}"
      glibc_installed = (DINRUSBREW_PREFIX/"opt/glibc/bin/ld.so").readable?

      return unless gcc_opt_prefix.readable?

      if glibc_installed
        ld_so_conf_d = DINRUSBREW_PREFIX/"etc/ld.so.conf.d"
        ...
        ld_gcc_conf_content = <<~EOS
          # This file is generated by DinrusBrew. Do not modify.
        ...
        if !ld_gcc_conf.exist? || (ld_gcc_conf.read != ld_gcc_conf_content)
          ...
          FileUtils.rm_f DINRUSBREW_PREFIX/"etc/ld.so.cache"
          system DINRUSBREW_PREFIX/"opt/glibc/sbin/ldconfig"
        end
      else
        odie "#{DINRUSBREW_PREFIX}/lib does not exist!" unless (DINRUSBREW_PREFIX/"lib").readable?
      end

      GCC_RUNTIME_LIBS.each do |library|
        gcc_library_symlink = DINRUSBREW_PREFIX/"lib/#{library}"
