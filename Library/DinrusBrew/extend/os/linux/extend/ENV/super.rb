...
module Superenv
  ...
  def self.shims_path
    DINRUSBREW_SHIMS_PATH/"linux/super"
  end
  ...
  def setup_build_environment(formula: nil, cc: nil, build_bottle: false, bottle_arch: nil, testing_formula: false,
    ...
    self["DINRUSBREW_OPTIMIZATION_LEVEL"] = "O2"
    self["DINRUSBREW_DYNAMIC_LINKER"] = determine_dynamic_linker_path
    self["DINRUSBREW_RPATH_PATHS"] = determine_rpath_paths(@formula)
    m4_path_deps = ["libtool", "bison"]
    self["M4"] = "#{DINRUSBREW_PREFIX}/opt/m4/bin/m4" if deps.any? { m4_path_deps.include?(_1.name) }
  end
  ...
  def determine_rpath_paths(formula)
    PATH.new(
      *formula&.lib,
      "#{DINRUSBREW_PREFIX}/opt/gcc/lib/gcc/current",
      PATH.new(run_time_deps.map { |dep| dep.opt_lib.to_s }).existing,
      "#{DINRUSBREW_PREFIX}/lib",
    )
  end

  sig { returns(T.nilable(String)) }
  def determine_dynamic_linker_path
    path = "#{DINRUSBREW_PREFIX}/lib/ld.so"
