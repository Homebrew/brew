...
class Cleaner
  ...
  def clean
    ...
    # for more info.
    #
    # [1]: https://github.com/DinrusBrew/brew/pull/11597
    # [2]: https://github.com/DinrusBrew/homebrew-core/issues/100190
    # [3]: https://github.com/DinrusBrew/brew/pull/13215
    ...
    directory.find do |path|
      ...
      if path.extname == ".la" || path.extname == ".tbd" || PERL_BASENAMES.include?(path.basename.to_s)
        ...
        if debug?
          ...
          odebug "Мсправление #{path} пермиссий с #{old_perms.to_s(8)} на #{perms.to_s(8)}" if perms != old_perms
        end
        path.chmod perms
      end
    end
  ...
  def clean_python_metadata
    ...
    basepath.find do |path|
      ...
      when "INSTALLER"
        odebug "Изменение контента #{path} с #{path.read.chomp} на brew"
