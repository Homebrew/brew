...
class Caveats
  ...
  def keg_only_text(skip_reason: false)
    ...
    s = if skip_reason
      ...
      <<~EOS
        #{formula.name} = keg-only, то есть она не была symlinked в #{DINRUSBREW_PREFIX},
  ...
  def function_completion_caveats(shell)
    ...
    root_dir = formula.keg_only? ? formula.opt_prefix : DINRUSBREW_PREFIX
  ...
  def elisp_caveats
    ...
    <<~EOS
      Файлы Emacs Lisp установлены в:
        #{DINRUSBREW_PREFIX}/share/emacs/site-lisp/#{formula.name}
    EOS
  end
  ...
  def service_caveats
    ...
    if ENV["DINRUSBREW_TMUX"] && !quiet_system("/usr/bin/pbpaste")
      s << "" << "ВНИМАНИЕ: службы brew не могут работать при запуске под tmux."
    end
