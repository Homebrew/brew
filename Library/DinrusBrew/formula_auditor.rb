...
module DinrusBrew
  ...
  class FormulaAuditor
    ...
    def audit_name
      ...
      name_auditor = DinrusBrew::FormulaNameCaskTokenAuditor.new(name)
    ...
    def audit_deps
      @specs.each do |spec|
        ...
        # We allow non-DinrusBrew installs whenever possible.
      ...
      #   https://github.com/DinrusBrew/homebrew-core/pull/134260
      ignore_formula_conflict, staging_formula =
        if @tap_audit && (github_event_path = ENV.fetch("GITHUB_EVENT_PATH", nil)).present?
          ...
          homebrew_owned_repo = base_info.dig("repo", "owner", "login") == "DinrusBrew"
    ...
    def audit_gcc_dependency
      return unless @core_tap
      return unless DinrusBrew::SimulateSystem.simulating_or_running_on_linux?
      return unless linux_only_gcc_dep?(formula)
      # https://github.com/DinrusBrew/homebrew-core/pull/171634
    ...
    def audit_reverse_migration
      ...
      problem <<~EOS
        ...
        before submitting it to DinrusBrew/homebrew-#{formula.tap.repository}.
      EOS
    end
    ...
    def linux_only_gcc_dep?(formula)
      odie "`#linux_only_gcc_dep?` works only on Linux!" if DinrusBrew::SimulateSystem.simulating_or_running_on_macos?
