homebrew-items() {
  ...
  # DINRUSBREW_MACOS is set by brew.sh
  # shellcheck disable=SC2154
  if [[ -n "${DINRUSBREW_MACOS}" ]]
  then
    sed_extended_regex_flag="-E"
    find_args=("-E" "${DINRUSBREW_REPOSITORY}/Library/Taps")
  else
    sed_extended_regex_flag="-r"
    find_args=("${DINRUSBREW_REPOSITORY}/Library/Taps" "-regextype" "posix-extended")
  fi

  # DINRUSBREW_REPOSITORY is set by brew.sh
  # shellcheck disable=SC2154
  [[ -d "${DINRUSBREW_REPOSITORY}/Library/Taps" ]] || return
  items="$(
    find "${find_args[@]}" \
      -type d \( \
