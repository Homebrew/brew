...
module DinrusBrew
  module Reinstall
    def self.reinstall_formula(
      ...
      oh1 "Переустанавливается #{Formatter.identifier(formula.full_name)} #{options.to_a.join " "}"
    ...
    # Any other exceptions we want to restore the previous keg and report the error.
      ...
      begin
        ...
        odie <<~EOS
          Не удалось удалить страховочный кег #{backup_path(keg).parent.basename}! Сделайте это вручную:
    ...
    def self.backup(keg)
      ...
      begin
        ...
        odie <<~EOS
          Не удалось переименовать кег #{keg.name}! Проверьте/исправьте нго пермиссии:
