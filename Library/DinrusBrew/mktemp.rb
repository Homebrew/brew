...
class Mktemp
  ...
  def run(chdir: true, &_block)
    ...
    @tmpdir = if retain_in_cache?
      tmp_dir = DINRUSBREW_CACHE/"Sources/#{prefix_name}"
      chmod_rm_rf(tmp_dir) # clear out previous staging directory
      tmp_dir.mkpath
      tmp_dir
    else
      Pathname.new(Dir.mktmpdir("#{prefix_name}-", DINRUSBREW_TEMP))
    end
    ...
    # contains it.
    group_id = if DINRUSBREW_ORIGINAL_BREW_FILE.grpowned?
      DINRUSBREW_ORIGINAL_BREW_FILE.stat.gid
    else
      Process.gid
    end
    begin
      ...
      opoo "Неудачная установка группы \"#{group_name || group_id}\" на #{@tmpdir}"
    end
    ...
    if retain? && @tmpdir.present? && !@quiet
      message = retain_in_cache? ? "Исходники для отладки доступны с:" : "Временные файлы удержаны в:"
