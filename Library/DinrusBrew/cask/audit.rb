...
module Cask
  ...
  class Audit
    ...
    def run!
      ...
      add_error "исключение при аудировании #{cask}: #{e.message}"
      self
    end
    ...
    sig {
      params(
        ...
        location:    T.nilable(DinrusBrew::SourceLocation),
    ...
    def result
      Formatter.error("провалено") if errors?
    end
    ...
    def audit_untrusted_pkg
      ...
      tap = @cask.tap
      return if tap.nil?
      return if tap.user != "DinrusBrew"

      return if cask.artifacts.none? { |k| k.is_a?(Artifact::Pkg) && k.stanza_options.key?(:allow_untrusted) }

      add_error "allow_untrusted is not permitted in official DinrusBrew Cask taps"
    end
    ...
    sig { void }
    def audit_token
      token_auditor = DinrusBrew::FormulaNameCaskTokenAuditor.new(cask.token)
    ...
    def audit_token_conflicts
      ...
      DinrusBrew.with_no_api_env do
        return unless core_formula_names.include?(cask.token)

        add_error(
          "possible duplicate, cask token conflicts with DinrusBrew core formula: #{Formatter.url(core_formula_url)}",
          strict_only: true,
        )
      end
    end
    ...
    def extract_artifacts
      ...
      @tmpdir ||= Pathname(Dir.mktmpdir("cask-audit", DINRUSBREW_TEMP))
    ...
    def audit_rosetta
      return if (url = cask.url).nil?
      return unless online?
      return if DinrusBrew::SimulateSystem.current_arch != :arm
    ...
    def audit_livecheck_version
      ...
      referenced_cask, = DinrusBrew::Livecheck.resolve_livecheck_reference(cask)

      # Respect skip conditions for a referenced cask
      if referenced_cask
        skip_info = DinrusBrew::Livecheck::SkipConditions.referenced_skip_information(
          referenced_cask,
          DinrusBrew::Livecheck.package_or_resource_name(cask),
        )
      end

      # Respect cask skip conditions (e.g. deprecated, disabled, latest, unversioned)
      skip_info ||= DinrusBrew::Livecheck::SkipConditions.skip_information(cask)
      return :skip if skip_info.present?

      latest_version = DinrusBrew::Livecheck.latest_version(
        cask,
    ...
    def audit_min_os
      ...
      return if min_os.nil? || min_os <= DINRUSBREW_MACOS_OLDEST_ALLOWED
    ...
    def livecheck_min_os
      ...
      content = DinrusBrew::Livecheck::Strategy.page_content(cask.livecheck.url)[:content]
      return if content.blank?

      begin
        items = DinrusBrew::Livecheck::Strategy::Sparkle.sort_items(
          DinrusBrew::Livecheck::Strategy::Sparkle.filter_items(
            DinrusBrew::Livecheck::Strategy::Sparkle.items_from_content(content),
          ),
        )
      rescue
        return
