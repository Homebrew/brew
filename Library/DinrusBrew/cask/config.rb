...
module Cask
  ...
  class Config
    ...
    sig { params(args: DinrusBrew::CLI::Args).returns(T.attached_class) }
    ...
    def env
      @env ||= self.class.canonicalize(
        DinrusBrew::EnvConfig.cask_opts
    ...
    def binarydir
      @binarydir ||= T.let(DINRUSBREW_PREFIX/"bin", T.nilable(Pathname))
    end

    sig { returns(Pathname) }
    def manpagedir
      @manpagedir ||= T.let(DINRUSBREW_PREFIX/"share/man", T.nilable(Pathname))
    end
