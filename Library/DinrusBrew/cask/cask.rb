  ...
  class Cask
    ...
    def self.all(eval_all: false)
      if !eval_all && !DinrusBrew::EnvConfig.eval_all?
        raise ArgumentError, "Cask::Cask#all не может использоваться без `--eval-all` или DINRUSBREW_EVAL_ALL"
      end
    ...
    def to_hash_with_variations(hash_method: :to_h)
      ...
      when :to_h
        if loaded_from_api? && !DinrusBrew::EnvConfig.no_install_from_api?
          return api_to_local_hash(DinrusBrew::API::Cask.all_casks[token].dup)
        end
    ...
    sig { returns(T.nilable(DinrusBrew::BundleVersion)) }
    def bundle_version
      @bundle_version ||= if (bundle = artifacts.find { |a| a.is_a?(Artifact::App) }&.target) &&
        ...
        DinrusBrew::BundleVersion.from_info_plist(plist)
      end
    end
