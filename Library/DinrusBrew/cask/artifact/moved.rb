    ...
    class Moved < Relocated
      ...
      def summarize_installed
        if target.exist?
          ...
          Formatter.error(printable_target, label: "Отсутствует #{self.class.english_name}")
        end
      end
      ...
      def move(adopt: false, auto_updates: false, force: false, verbose: false, predecessor: nil, reinstall: false,
            ...
            if adopt
              ...
              unless auto_updates
                ...
                same = if source_plist.size? &&
                          (source_bundle_version = DinrusBrew::BundleVersion.from_info_plist(source_plist)) &&
                          target_plist.size? &&
                          (target_bundle_version = DinrusBrew::BundleVersion.from_info_plist(target_plist))
        ...
        if target.directory? && Quarantine.app_management_permissions_granted?(app: target, command:)
          ...
          # default sudo user isn't necessarily able to write to DinrusBrew's locations
