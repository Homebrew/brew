    ...
    class AbstractArtifact
      ...
      def self.read_script_arguments(arguments, stanza, default_arguments = {}, override_arguments = {}, key = nil)
        ...
        unless unknown_keys.empty?
          opoo "Неизвестные аргументы к #{description} -- " \
               "#{unknown_keys.inspect} (ignored). Выполнение " \
               "`brew update; brew cleanup`, вероятно, это исправит."
        end
        ...
        unless ignored_keys.empty?
          onoe "Некоторые аргументы к #{description} будут проигнорированы -- :#{unknown_keys.inspect} (переопределён)."
        end
