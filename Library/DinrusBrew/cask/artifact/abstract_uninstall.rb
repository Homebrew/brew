    ...
    class AbstractUninstall < AbstractArtifact
      ...
      def uninstall_launchctl(*services, command: nil, **_)
        ...
        all_services.each do |service|
          ohai "Удаление службы launchctl #{service}"
      ...
      def automation_access_instructions
        ...
        <<~EOS
          Активируйте доступ к Automation для "Terminal → System Events" в:
      ...
      def uninstall_quit(*bundle_ids, command: nil, **_)
        bundle_ids.each do |bundle_id|
          ...
          unless T.must(User.current).gui?
            opoo "Не логирован в ГИП; пропуск выхода из приложения ID '#{bundle_id}'."
            next
          end

          ohai "Выход из приложения '#{bundle_id}'..."
          ...
          begin
            Timeout.timeout(10) do
              Kernel.loop do
                ...
                puts "Приложение '#{bundle_id}' успешно закрыто."
                break
              end
            end
          rescue Timeout::Error
            opoo "Приложение '#{bundle_id}' не закрыто. #{automation_access_instructions}"
          end
        end
      end
      ...
      def uninstall_signal(*signals, command: nil, **_)
        signals.each do |pair|
          raise CaskInvalidError.new(cask, "Каждый #{stanza} :signal должен состоять из 2 элементов.") if pair.size != 2
      ...
      def uninstall_script(directives, directive_name: :script, force: false, command: nil, **_)
        ...
        ohai "Выпролняется сценарий деинсталяции #{executable}"
        raise CaskInvalidError.new(cask, "#{stanza} :#{directive_name} без :executable.") if executable.nil?
        ...
        if (executable_path.absolute? && !executable_path.exist?) ||
          ...
          message = "сценария деинсталяции #{executable} не существует"
          raise CaskError, "#{message}." unless force

          opoo "#{message}; пропускается."
          return
        end
      ...
      def uninstall_pkgutil(*pkgs, command: nil, **_)
        ohai "Деинсталируются пакеты с sudo; может потребоваться пароль:"
        pkgs.each do |regex|
      ...
      def each_resolved_path(action, paths)
        ...
        paths.each do |path|
          ...
          if resolved_path.relative? || resolved_path.split.any? { |part| part.to_s == ".." }
            opoo "Пропускается #{Formatter.identifier(action)} для относительного пути '#{path}'."
            next
          end

          if MacOS.undeletable?(resolved_path)
            opoo "Пропускается #{Formatter.identifier(action)} для неудаляемого пути '#{path}'."
            next
          end
          ...
          begin
            ...
            odie "Не удаётся удалить некоторые файлы. Активируйте полный доступ к диску для вашего терминала в " \
      ...
      def uninstall_trash(*paths, **options)
        ...
        ohai "Файлы отправляются в корзину:", resolved_paths.map(&:first)
      ...
      def trash_paths(*paths, command: nil, **_)
        ...
        stdout, = system_command DINRUSBREW_LIBRARY_PATH/"cask/utils/trash.swift",
                                 args:         paths,
                                 print_stderr: DinrusBrew::EnvConfig.developer?
        ...
        trashed_with_permissions, untrashable = untrashable.partition do |path|
          Utils.gain_permissions(path, ["-R"], SystemCommand) do
            system_command! DINRUSBREW_LIBRARY_PATH/"cask/utils/trash.swift",
                            args:         [path],
                            print_stderr: DinrusBrew::EnvConfig.developer?
          end
        ...
        opoo "Следующие файлы нельзя удалить в корзину, выполните это вручную:"
      ...
      def uninstall_rmdir(*directories, **kwargs)
        ...
        ohai "Удаляются директории, если они пусты:"
