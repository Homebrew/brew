  ...
  module Artifact
    ...
    class Installer < AbstractArtifact
      ...
      def install_phase(command: nil, **_)
        if manual_install
          puts <<~EOS
            Каск #{cask} предоставляет только ручную установку. Чтобы выполнить её и завершить установку:
              откройте #{cask.staged_path.join(path).to_s.shellescape}
          EOS
        else
          ...
          command.run!(
            ...
            **args,
            env:       { "PATH" => PATH.new(
              DINRUSBREW_PREFIX/"bin", DINRUSBREW_PREFIX/"sbin", ENV.fetch("PATH")
            ) },
            reset_uid: !args[:sudo],
          )
        end
