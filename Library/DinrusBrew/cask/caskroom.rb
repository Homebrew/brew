...
module Cask
  ...
  module Caskroom
    ...
    def self.path
      @path ||= T.let(DINRUSBREW_PREFIX/"Caskroom", T.nilable(Pathname))
    end
    ...
    def self.ensure_caskroom_exists
      ...
      if sudo && !ENV.key?("SUDO_ASKPASS") && $stdout.tty?
        ohai "Создаётся директория Caskroom: #{path}",
             "Пермиссии будут установлены должным образом и в будущем sudo не потребуется."
      end
