...
module Cask
  ...
  class Installer
    ...
    def self.caveats(cask)
      ...
      DinrusBrew.messages.record_caveats(cask.token, caveats)
...
on_request: true)
      ...
      puts summary
      end_time = Time.now
      DinrusBrew.messages.package_installed(@cask.token, end_time - start_time)
    rescue
      restore_backup
      raise
    end
    ...
    def verify_has_sha
      ...
      raise CaskError, <<~EOS
        ...
        This means you have the #{Formatter.identifier("--require-sha")} option set, perhaps in your DINRUSBREW_CASK_OPTS.
      EOS
    end
    ...
    def extract_primary_container(to: @cask.staged_path)
      ...
      if (nested_container = @cask.container&.nested)
        Dir.mktmpdir("cask-installer", DINRUSBREW_TEMP) do |tmpdir|
    ...
    def satisfy_cask_and_formula_dependencies
      ...
      missing_formulae_and_casks.each do |cask_or_formula|
        if cask_or_formula.is_a?(Cask)
          ...
          DinrusBrew::Install.perform_preinstall_checks_once
    ...
    def forbidden_tap_check
      ...
      error_message = "The installation of #{@cask.full_name} has the tap #{cask_tap}\n" \
                      "but #{owner} "
      error_message << "has not allowed this tap in `DINRUSBREW_ALLOWED_TAPS`" unless cask_tap.allowed_by_env?
      error_message << " and\n" if !cask_tap.allowed_by_env? && cask_tap.forbidden_by_env?
      error_message << "has forbidden this tap in `DINRUSBREW_FORBIDDEN_TAPS`" if cask_tap.forbidden_by_env?
      error_message << ".#{owner_contact}"
    ...
    def forbidden_cask_and_formula_check
      ...
      forbidden_casks = Set.new(DinrusBrew::EnvConfig.forbidden_casks.to_s.split)
      ...
      unless skip_cask_deps?
        cask_and_formula_dependencies.each do |dep_cask_or_formula|
          dep_name, dep_type, variable = if dep_cask_or_formula.is_a?(Cask) && forbidden_casks.present?
            ...
            [dep_cask_name, "cask", "DINRUSBREW_FORBIDDEN_CASKS"]
            ...
            [formula_name, "formula", "DINRUSBREW_FORBIDDEN_FORMULAE"]
          end
      ...
      raise CaskCannotBeInstalledError.new(@cask, <<~EOS
        forbidden for installation by #{owner} in `DINRUSBREW_FORBIDDEN_CASKS`.#{owner_contact}
      EOS
      )
    end
    ...
    def load_cask_from_source_api!
      @cask = DinrusBrew::API::Cask.source_download(@cask)
    end
  end
end
