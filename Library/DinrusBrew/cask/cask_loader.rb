  ...
  module CaskLoader
    ...
    class FromPathLoader < AbstractContentLoader
      ...
      def self.try_new(ref, warn: false)
        ...
        return if DinrusBrew::EnvConfig.forbid_packages_from_paths? &&
                  !path.realpath.to_s.start_with?("#{Caskroom.path}/", "#{DINRUSBREW_LIBRARY}/Taps/")
      ...
      def initialize(path, token: T.unsafe(nil))
        ...
        @path = path
        @tap = Tap.from_path(path) || DinrusBrew::API.tap_from_source_download(path)
      end
    ...
    class FromAPILoader
      ...
      def self.try_new(ref, warn: false)
        ...
        return unless (token = ref[DINRUSBREW_DEFAULT_TAP_CASK_REGEX, :token])
        if !DinrusBrew::API::Cask.all_casks.key?(token) &&
           !DinrusBrew::API::Cask.all_renames.key?(token)
          return
        end
      ...
      def initialize(token, from_json: T.unsafe(nil), path: nil)
        ...
        @sourcefile_path = path || DinrusBrew::API::Cask.cached_json_file_path
        @path = path || CaskLoader.default_path(@token)
        @from_json = from_json
      end

      def load(config:)
        json_cask = from_json || DinrusBrew::API::Cask.all_casks.fetch(token)
        ...
        json_cask = DinrusBrew::API.merge_variations(json_cask).deep_symbolize_keys.freeze
      ...
      def from_h_string_gsubs(string, appdir)
        string.to_s
              ...
              .gsub(DINRUSBREW_PREFIX_PLACEHOLDER, DINRUSBREW_PREFIX)
              .gsub(DINRUSBREW_CELLAR_PLACEHOLDER, DINRUSBREW_CELLAR)
              .gsub(DINRUSBREW_CASK_APPDIR_PLACEHOLDER, appdir)
      end
    ...
    class FromNameLoader < FromTapLoader
      ...
      def self.try_new(ref, warn: false)
        return unless ref.is_a?(String)
        return unless ref.match?(/\A#{DINRUSBREW_TAP_CASK_TOKEN_REGEX}\Z/o)
