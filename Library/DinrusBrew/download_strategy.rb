...
class AbstractDownloadStrategy
  ...
  def initialize(url, name, version, **meta)
    ...
    @name = name
    @version = version
    @cache = meta.fetch(:cache, DINRUSBREW_CACHE)
    @meta = meta
    @quiet = false
  ...
  def chdir(&block)
    entries = Dir["*"]
    raise "Пустой архив" if entries.empty?
...
class VCSDownloadStrategy < AbstractDownloadStrategy
  ...
  def fetch(timeout: nil)
    ...
    raise <<~EOS
     Тэг #{@ref} должен быть #{@revision},
      но на самом деле #{current_revision}
    EOS
  end
...
class AbstractFileDownloadStrategy < AbstractDownloadStrategy
  ...
  def cached_location
    ...
    downloads = Pathname.glob(DINRUSBREW_CACHE/"downloads/#{url_sha256}--*")
                        .reject { |path| path.extname.end_with?(".incomplete") }

    @cached_location = if downloads.count == 1
      downloads.first
    else
      DINRUSBREW_CACHE/"downloads/#{url_sha256}--#{Utils.safe_filename(resolved_basename)}"
    end
  end
...
class CurlDownloadStrategy < AbstractFileDownloadStrategy
  ...
  def fetch(timeout: nil)
    ...
    begin
      ...
      begin
        ...
        if (domain = DinrusBrew::EnvConfig.artifact_domain)
          url = url.sub(%r{^https?://#{GitHubPackages::URL_DOMAIN}/}o, "#{domain.chomp("/")}/")
          urls = [] if DinrusBrew::EnvConfig.artifact_domain_no_fallback?
        end

        ohai "Загружается #{url}"
        ...
        if use_cached_location
          puts "Уже загружен: #{cached_location}"
        else
          begin
        ...
        puts "Проверка зеркала..."
        retry
      rescue Timeout::Error => e
        raise Timeout::Error, "Загрузка просрочена #{self.url}: #{e}"
      end
    ensure
  ...
  def _fetch(url:, resolved_url:, timeout:)
    ohai "Загрузка из #{resolved_url}" if url != resolved_url

    if DinrusBrew::EnvConfig.no_insecure_redirect? &&
       url.start_with?("https://") && !resolved_url.start_with?("https://")
      $stderr.puts "Обнаружен редирект из HTTPS в HTTP и установлена DINRUSBREW_NO_INSECURE_REDIRECT."
      raise CurlDownloadStrategyError, url
    end
...
class CurlGitHubPackagesDownloadStrategy < CurlDownloadStrategy
  ...
  def initialize(url, name, version, **meta)
    ...
    # DINRUSBREW_GITHUB_PACKAGES_AUTH set in brew.sh
    meta[:headers] << "Authorization: #{DINRUSBREW_GITHUB_PACKAGES_AUTH}"
    super
  end
...
class CurlApacheMirrorDownloadStrategy < CurlDownloadStrategy
  ...
  def apache_mirrors
    ...
    raise CurlDownloadStrategyError, "Зеркало не обнаружено, попробуйте ещё раз позднее."
  end
end
...
class GitDownloadStrategy < VCSDownloadStrategy
  ...
  def checkout(timeout: nil)
    ohai "Чекаутирование #{@ref_type} #{@ref}" if @ref_type && @ref
  ...
  # See https://github.com/DinrusBrew/homebrew-core/pull/1520 for an example.
  def fix_absolute_submodule_gitdir_references!
    ...
    submodule_dirs.lines.map(&:chomp).each do |submodule_dir|
      ...
      if git_dir.nil?
        onoe "Не удалось разобрать '#{dot_git}'." if DinrusBrew::EnvConfig.developer?
        next
      end
...
class MercurialDownloadStrategy < VCSDownloadStrategy
  ...
  def update(timeout: nil)
    ...
    update_args << if @ref_type && @ref
      ohai "Чекаутирование #{@ref_type} #{@ref}"
      @ref
    else
      "default"
    end
