...
class SoftwareSpec
  ...
  PREDEFINED_OPTIONS = {
    universal: Option.new("universal", "Построить универсальный бинарник."),
    cxx11:     Option.new("c++11",     "Построить в режиме C++11."),
  }.freeze
  ...
  def resource(name = T.unsafe(nil), klass = Resource, &block)
    if block
      raise ArgumentError, "У ресурса должно быть имя." if name.nil?
  ...
  def go_resource(name, &block)
    odisabled "`SoftwareSpec#go_resource`", "Модули Go"
  ...
  def option(name, description = "")
    opt = PREDEFINED_OPTIONS.fetch(name) do
      unless name.is_a?(String)
        raise ArgumentError, "имя опции должно быть текстом или символом; а получен #{name.class}: #{name}"
      end
      raise ArgumentError, "требуется имя опции" if name.empty?
      raise ArgumentError, "имя опции должно быть длинее одного символа: #{name}" if name.length <= 1
      raise ArgumentError, "имя опции не должно начинаться с тире: #{name}" if name.start_with?("-")
  ...
  def deprecated_option(hash)
    raise ArgumentError, "хэш у deprecated_option не должен быть пуст" if hash.empty?
  ...
  def add_dep_option(dep)
    dep.option_names.each do |name|
      if dep.optional? && !option_defined?("with-#{name}")
        options << Option.new("with-#{name}", "Построить с поддержкой #{name}.")
      elsif dep.recommended? && !option_defined?("without-#{name}")
        options << Option.new("without-#{name}", "Построить без поддержки #{name}.")
      end
    end
  end
end
...
class Bottle
  ...
  class Filename
    ...
    def initialize(name, version, tag, rebuild)
      ...
      raise ArgumentError, "Неполноценное имя бутыли" unless Utils.safe_filename?(@name)
      raise ArgumentError, "Неполноценная версия бутыли" unless Utils.safe_filename?(version.to_s)
  ...
  def fallback_on_error
    ...
    if @resource.url.start_with?(DinrusBrew::EnvConfig.bottle_domain) &&
       DinrusBrew::EnvConfig.bottle_domain != DINRUSBREW_BOTTLE_DEFAULT_DOMAIN
      opoo "Отсутствует бутыль, откат к дефолтному домену..."
...
class BottleSpecification
  ...
  def initialize
    @rebuild = 0
    @repository = DinrusBrew::DEFAULT_REPOSITORY
  ...
  def root_url(var = nil, specs = {})
    if var.nil?
      @root_url ||= if (github_packages_url = GitHubPackages.root_url_if_match(DinrusBrew::EnvConfig.bottle_domain))
        github_packages_url
      else
        DinrusBrew::EnvConfig.bottle_domain
      end
    else
  ...
  def compatible_locations?(tag: Utils::Bottles.tag)
    ...
    cellar_relocatable = cellar.size >= DINRUSBREW_CELLAR.to_s.size && ENV["DINRUSBREW_RELOCATE_BUILD_PREFIX"].present?
    prefix_relocatable = prefix.size >= DINRUSBREW_PREFIX.to_s.size && ENV["DINRUSBREW_RELOCATE_BUILD_PREFIX"].present?

    compatible_cellar = cellar == DINRUSBREW_CELLAR.to_s || cellar_relocatable
    compatible_prefix = prefix == DINRUSBREW_PREFIX.to_s || prefix_relocatable
