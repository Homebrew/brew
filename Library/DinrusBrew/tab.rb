...
class Tab < AbstractTab
  ...
  def self.from_file_content(content, path)
    ...
    tab.tap = "homebrew/core" if ["mxcl/master", "DinrusBrew/homebrew"].include?(tab.tap)
    ...
    # Tabs created with DinrusBrew 1.5.13 through 4.0.17 inclusive created empty string versions in some cases.
  ...
  def runtime_dependencies
    # DinrusBrew versions prior to 1.1.6 generated incorrect runtime dependency
    # lists.
  ...
  def to_s
    ...
    s << if poured_from_bottle
      ...
      "Построено из исходников"
    end

    s << "использован formulae.brew.sh API" if loaded_from_api
    s << Time.at(time).strftime("на дату %Y-%m-%d в %H:%M:%S") if time
