...
# A formula provides instructions and metadata for DinrusBrew to install a piece
# of software. Every DinrusBrew formula is a {Formula}.
...
# `ThisFormula`. DinrusBrew does enforce that the name of the file and the class
# correspond.
...
class Formula
  ...
  include DinrusBrew::Livecheck::Constants
  extend Forwardable
  extend Cachable
  extend Attrable
  extend APIHashable
  ...
  # Used for creating new DinrusBrew versions of software without new upstream
  # versions.
  # @see .revision=
  sig { returns(Integer) }
  ...
  def initialize(name, path, spec, alias_path: nil, tap: nil, force_bottle: false)
    ...
    raise "Не вызывайте `Formula.new' напрямую, без подкласса." unless self.class < Formula
  ...
  def active_spec=(spec_sym)
    spec = send(spec_sym)
    raise FormulaSpecificationError, "#{spec_sym} недоступно для #{full_name}" unless spec
  ...
  def determine_active_spec(requested)
    ...
    spec || raise(FormulaSpecificationError, "#{full_name}: формуле требуется как минимум URL")
  end
  ...
  def installed_alias_path
    ...
    return unless path&.match?(%r{#{DINRUSBREW_TAP_DIR_REGEX}/Aliases}o)
  ...
  def specified_path
    return DinrusBrew::API::Formula.cached_json_file_path if loaded_from_api?
  ...
  #        (see https://github.com/DinrusBrew/brew/issues/17203#issuecomment-2093654431).
  #
  # @api public
  sig {
  ...
  # @api internal
  def linked_keg
    linked_keg = possible_names.map { |name| DINRUSBREW_LINKED_KEGS/name }
                               .find(&:directory?)
    return linked_keg if linked_keg.present?

    DINRUSBREW_LINKED_KEGS/name
  end
  ...
  sig { returns(Pathname) }
  def rack
    DINRUSBREW_CELLAR/name
  end

  # All currently installed prefix directories.
  def installed_prefixes
    possible_names.map { |name| DINRUSBREW_CELLAR/name }
                  .select(&:directory?)
                  .flat_map(&:subdirs)
                  .sort_by(&:basename)
  end
  ...
  # This is symlinked into `DINRUSBREW_PREFIX` after installation or with
  ...
  # The directory where the formula's binaries should be installed.
  # This is not symlinked into `DINRUSBREW_PREFIX`.
  # It is commonly used to install files that we do not wish to be
  # symlinked into `DINRUSBREW_PREFIX` from one of the other directories and
  ...
  # This directory is not inside the `DINRUSBREW_CELLAR` so it persists
  # across upgrades.
  #
  # @api public
  sig { returns(Pathname) }
  def etc
    (DINRUSBREW_PREFIX/"etc").extend(InstallRenamed)
  end

  # A subdirectory of `etc` with the formula name suffixed.
  # e.g. `$DINRUSBREW_PREFIX/etc/openssl@1.1`
  ...
  sig { returns(Pathname) }
  def pkgetc
    (DINRUSBREW_PREFIX/"etc"/name).extend(InstallRenamed)
  end
  ...
  # installation so, despite not being in `DINRUSBREW_CELLAR`, they are installed
  ...
  def plist
    odisabled "`Formula#plist`", "`DinrusBrew::Service`"
    nil
  end
  ...
  def service
    @service ||= DinrusBrew::Service.new(self, &self.class.service)
  end
  ...
  def install_etc_var
    ...
    Find.find(*etc_var_dirs.select(&:directory?)) do |path|
      ...
      path.cp_path_sub(bottle_prefix, DINRUSBREW_PREFIX)
    end
  end
  ...
  def run_post_install
    ...
    begin
      ...
      new_env = {
        TMPDIR:        DINRUSBREW_TEMP,
        TEMP:          DINRUSBREW_TEMP,
        TMP:           DINRUSBREW_TEMP,
        _JAVA_OPTIONS: "-Djava.io.tmpdir=#{DINRUSBREW_TEMP}",
        DINRUSBREW_PATH: nil,
  ...
  # Warn the user about any DinrusBrew-specific issues or quirks for this package.
  ...
  def link_overwrite?(path)
    # Don't overwrite files not created by DinrusBrew.
    return false if path.stat.uid != DINRUSBREW_ORIGINAL_BREW_FILE.stat.uid
    ...
    to_check = path.relative_path_from(DINRUSBREW_PREFIX).to_s
  ...
  def lock
    ...
    oldnames.each do |oldname|
      next unless (oldname_rack = DINRUSBREW_CELLAR/oldname).exist?
  ...
  def oldnames_to_migrate
    oldnames.select do |oldname|
      old_rack = DINRUSBREW_CELLAR/oldname
  ...
  def std_cmake_args(install_prefix: prefix, install_libdir: "lib", find_framework: "LAST")
    %W[
      ...
      -DCMAKE_PROJECT_TOP_LEVEL_INCLUDES=#{DINRUSBREW_LIBRARY_PATH}/cmake/trap_fetchcontent_provider.cmake
      -Wno-dev
      -DBUILD_TESTING=OFF
    ]
  end
  ...
  def rpath(source: bin, target: lib)
    unless target.to_s.start_with?(DINRUSBREW_PREFIX)
      raise "rpath `target` should only be used for paths inside DINRUSBREW_PREFIX!"
    end
  ...
  def generate_completions_from_executable(*commands,
    ...
    shells.each do |shell|
      ...
      popen_read_options = {}
      popen_read_options[:err] = :err unless ENV["DINRUSBREW_STDERR"]
  ...
  def self.all(eval_all: false)
    if !eval_all && !DinrusBrew::EnvConfig.eval_all?
      raise ArgumentError, "Formula#all without `--eval-all` or DINRUSBREW_EVAL_ALL"
    end
  ...
  def self.racks
    Formula.cache[:racks] ||= if DINRUSBREW_CELLAR.directory?
      DINRUSBREW_CELLAR.subdirs.reject do |rack|
  ...
  # True if this formula is provided by DinrusBrew itself
  sig { returns(T::Boolean) }
  def core_formula?
    !!tap&.core_tap?
  end
  ...
  def to_hash_with_variations(hash_method: :to_hash)
    ...
    if loaded_from_api? && !DinrusBrew::EnvConfig.no_install_from_api?
      json_formula = DinrusBrew::API::Formula.all_formulae.fetch(name).dup
    ...
    if path.exist? && on_system_blocks_exist?
      ...
      OnSystem::ALL_OS_ARCH_COMBINATIONS.each do |os, arch|
        ...
        DinrusBrew::SimulateSystem.with(os:, arch:) do
  ...
  def caveats_with_placeholders
    caveats&.gsub(DINRUSBREW_PREFIX, DINRUSBREW_PREFIX_PLACEHOLDER)
           &.gsub(DINRUSBREW_CELLAR, DINRUSBREW_CELLAR_PLACEHOLDER)
  end
  ...
  def run_test(keep_tmp: false)
    ...
    test_env = {
      ...
      TMP:           DINRUSBREW_TEMP,
      TERM:          "dumb",
      PATH:          PATH.new(ENV.fetch("PATH"), DINRUSBREW_PREFIX/"bin"),
      DINRUSBREW_PATH: nil,
    }.merge(common_stage_test_env)
    test_env[:_JAVA_OPTIONS] += " -Djava.io.tmpdir=#{DINRUSBREW_TEMP}"
  ...
  def test_fixtures(file)
    DINRUSBREW_LIBRARY_PATH/"test/support/fixtures"/file
  end
  ...
  # hash-checked and then DinrusBrew changes into a temporary directory where the
  ...
  # defined by the formula, as only `DINRUSBREW_PREFIX` is available
  ...
  #   s.gsub! "/usr/local", DINRUSBREW_PREFIX.to_s
  # end
  # ```
  #
  ...
  def system(cmd, *args)
    verbose_using_dots = DinrusBrew::EnvConfig.verbose_using_dots?
    ...
    File.open(logfn, "w") do |log|
      ...
      unless $CHILD_STATUS.success?
        log_lines = DinrusBrew::EnvConfig.fail_log_lines
  ...
  # Runs `xcodebuild` without DinrusBrew's compiler environment variables set.
  ...
  def exec_cmd(cmd, args, out, logfn)
    ENV["DINRUSBREW_CC_LOG_PATH"] = logfn
  ...
  def common_stage_test_env
    {
      _JAVA_OPTIONS:           "-Duser.home=#{DINRUSBREW_CACHE}/java_cache",
      GOCACHE:                 "#{DINRUSBREW_CACHE}/go_cache",
      GOPATH:                  "#{DINRUSBREW_CACHE}/go_mod_cache",
      CARGO_HOME:              "#{DINRUSBREW_CACHE}/cargo_cache",
      PIP_CACHE_DIR:           "#{DINRUSBREW_CACHE}/pip_cache",
  ...
  class << self
    ...
    # The reason for why this software is not linked (by default) to {::DINRUSBREW_PREFIX}.
    attr_reader :keg_only_reason

    # A one-line description of the software. Used by users to get an overview
    # of the software and DinrusBrew maintainers.
    ...
    #
    # @!attribute [w] license
    # @see https://docs.brew.sh/License-Guidelines DinrusBrew License Guidelines
    ...
    def network_access_allowed?(phase)
      ...
      env_var = DinrusBrew::EnvConfig.send(:"formula_#{phase}_network")
      env_var.nil? ? @network_access_allowed[phase] : env_var == "allow"
    end

    # The homepage for the software. Used by users to get more information
    # about the software and DinrusBrew maintainers as a point of contact for
    ...
    # Used for creating new DinrusBrew version schemes. For example, if we want
    ...
    # This provides a pre-built binary package built by the DinrusBrew maintainers for you.
    ...
    # You can ignore this block entirely if submitting to DinrusBrew/homebrew-core.
    ...
    #   # Optionally specify the DINRUSBREW_CELLAR in which the bottles were built.
    ...
    # ```
    #
    # DinrusBrew maintainers aim to bottle all formulae.
    #
    # @api public
    ...
    # formulae. DinrusBrew provides some `:special` {Requirement}s for stuff
    ...
    # only live in its Cellar. Other formulae can depend on it and DinrusBrew
    ...
    # if you build your own software outside of DinrusBrew. This way, we
    ...
    # Marks the {Formula} as needing a certain standard, so DinrusBrew
    ...
    # Defines whether the {Formula}'s bottle can be used on the given DinrusBrew
    # installation.
    #
    # ### Examples
    #
    ...
    def pour_bottle?(only_if: nil, &block)
      ...
      when :default_prefix
        lambda do |_|
          T.bind(self, PourBottleCheck)
          reason(<<~EOS)
            The bottle (and many others) needs to be installed into #{DinrusBrew::DEFAULT_PREFIX}.
          EOS
          satisfy { DINRUSBREW_PREFIX.to_s == DinrusBrew::DEFAULT_PREFIX }
        end
      else
