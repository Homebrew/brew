  ...
  module Help
    ...
    def self.help(cmd = nil, empty_argv: false, usage_error: nil, remaining_args: [])
      if cmd.nil?
        ...
        if empty_argv
          $stderr.puts DINRUSBREW_HELP_MESSAGE
          exit 1
        end

        # Handle `brew (-h|--help|--usage|-?|help)` (no other arguments).
        puts DINRUSBREW_HELP_MESSAGE
        exit 0
      end
      ...
      if usage_error
        $stderr.puts path ? command_help(cmd, path, remaining_args:) : DINRUSBREW_HELP_MESSAGE
        $stderr.puts
        onoe usage_error
        exit 1
      end
    ...
    def self.command_help(cmd, path, remaining_args:)
      ...
      output ||= if output.blank?
        opoo "No help text in: #{path}" if DinrusBrew::EnvConfig.developer?
