...
module OS
  ...
  def self.mac?
    return false if ENV["DINRUSBREW_TEST_GENERIC_OS"]
  ...
  ::OS_VERSION = T.let(ENV.fetch("DINRUSBREW_OS_VERSION").freeze, String)
  ...
  if OS.mac?
    ...
    if !OS::Mac.version.prerelease? &&
       ...
       (DINRUSBREW_PREFIX.to_s == DINRUSBREW_DEFAULT_PREFIX ||
       (DINRUSBREW_PREFIX.to_s == DINRUSBREW_MACOS_ARM_DEFAULT_PREFIX && Hardware::CPU.arm?))
