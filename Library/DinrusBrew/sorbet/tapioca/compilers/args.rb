  ...
  module Compilers
    class Args < Tapioca::Dsl::Compiler
      GLOBAL_OPTIONS = T.let(
        DinrusBrew::CLI::Parser.global_options.map do |short_option, long_option, _|
          [short_option, long_option].map { "#{DinrusBrew::CLI::Parser.option_to_name(_1)}?" }
        end.flatten.freeze, T::Array[String]
      )

      Parsable = T.type_alias { T.any(T.class_of(DinrusBrew::CLI::Args), T.class_of(DinrusBrew::AbstractCommand)) }
      ...
      def self.gather_constants
        ...
        DinrusBrew::AbstractCommand.subclasses
      end

      sig { override.void }
      def decorate
        cmd = T.cast(constant, T.class_of(DinrusBrew::AbstractCommand))
        # This is a dummy class to make the `brew` command parsable
        return if cmd == DinrusBrew::Cmd::Brew

        args_class_name = T.must(T.must(cmd.args_class).name)
        root.create_class(args_class_name, superclass_name: "DinrusBrew::CLI::Args") do |klass|
      ...
      sig { params(parser: DinrusBrew::CLI::Parser).returns(T::Array[Symbol]) }
      ...
      sig { params(klass: RBI::Scope, parser: DinrusBrew::CLI::Parser).void }
