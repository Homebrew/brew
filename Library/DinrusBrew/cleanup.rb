...
module DinrusBrew
  # Helper class for cleaning up the DinrusBrew cache.
  class Cleanup
    CLEANUP_DEFAULT_DAYS = DinrusBrew::EnvConfig.cleanup_periodic_full_days.to_i.freeze
    ...
    class << self
      ...
      def stale_formula?(pathname, scrub)
        return false unless DINRUSBREW_CELLAR.directory?

        version = if DINRUSBREW_BOTTLES_EXTNAME_REGEX.match?(to_s)
          begin
        ...
        formula = begin
          Formulary.from_rack(DINRUSBREW_CELLAR/formula_name)
    ...
    PERIODIC_CLEAN_FILE = (DINRUSBREW_CACHE/".cleaned").freeze

    attr_predicate :dry_run?, :scrub?, :prune?
    attr_reader :args, :days, :cache, :disk_cleanup_size

    def initialize(*args, dry_run: false, scrub: false, days: nil, cache: DINRUSBREW_CACHE)
      @disk_cleanup_size = 0
      @args = args
      @dry_run = dry_run
      @scrub = scrub
      @prune = days.present?
      @days = days || DinrusBrew::EnvConfig.cleanup_max_age_days.to_i
      @cache = cache
      @cleaned_up_paths = Set.new
    end

    def self.install_formula_clean!(formula, dry_run: false)
      return if DinrusBrew::EnvConfig.no_install_cleanup?
      return unless formula.latest_version_installed?
      return if skip_clean_formula?(formula)

      if dry_run
        ohai "Был бы выполнен `brew cleanup #{formula}`"
      else
        ohai "Выполняется `brew cleanup #{formula}`..."
      end
    ...
    def self.puts_no_install_cleanup_disable_message
      return if DinrusBrew::EnvConfig.no_env_hints?
      return if DinrusBrew::EnvConfig.no_install_cleanup?

      puts "Отключите это поведение, установив DINRUSBREW_NO_INSTALL_CLEANUP."
      puts "Скрыть эти подсказки можно посредством DINRUSBREW_NO_ENV_HINTS (см. `man brew`)."
    end
    ...
    def self.skip_clean_formula?(formula)
      no_cleanup_formula = DinrusBrew::EnvConfig.no_cleanup_formulae
    ...
    def self.periodic_clean_due?
      return false if DinrusBrew::EnvConfig.no_install_cleanup?
    ...
    def self.periodic_clean!(dry_run: false)
      ...
      if dry_run
        ...
        oh1 "`brew cleanup` не выполнялся последние #{CLEANUP_DEFAULT_DAYS} дней, выполняется сейчас..."
      end
    ...
    def clean!(quiet: false, periodic: false)
      if args.empty?
        ...
        if ENV["DINRUSBREW_AUTOREMOVE"].present?
          opoo "DINRUSBREW_AUTOREMOVE теперь no-op, так как это его дефолтное поведение. " \
               "Чтобы отключить его, установите DINRUSBREW_NO_AUTOREMOVE=1."
        end
        Cleanup.autoremove(dry_run: dry_run?) unless DinrusBrew::EnvConfig.no_autoremove?
        ...
        args.each do |arg|
          ...
          if formula && Cleanup.skip_clean_formula?(formula)
            onoe "Отказ чистить #{formula}, так как она в списках у " \
                 "#{Tty.bold}DINRUSBREW_NO_CLEANUP_FORMULAE#{Tty.reset}!"
          elsif formula
    ...
    def cleanup_logs
      return unless DINRUSBREW_LOGS.directory?

      logs_days = [days, CLEANUP_DEFAULT_DAYS].min

      DINRUSBREW_LOGS.subdirs.each do |dir|
    ...
    def cleanup_path(path)
      ...
      if dry_run?
        puts "Был бы удалён: #{path} (#{path.abv})"
      else
        puts "Удаляется: #{path}... (#{path.abv})"
        yield
      end
    end
    ...
    def cleanup_lockfiles(*lockfiles)
      ...
      lockfiles = DINRUSBREW_LOCKS.children.select(&:file?) if lockfiles.empty? && DINRUSBREW_LOCKS.directory?
    ...
    def cleanup_portable_ruby
      ...
      bundler_paths.each do |bundler_path|
        if dry_run?
          puts Utils.popen_read("git", "-C", DINRUSBREW_REPOSITORY, "clean", "-nx", bundler_path).chomp
        else
          puts Utils.popen_read("git", "-C", DINRUSBREW_REPOSITORY, "clean", "-ffqx", bundler_path).chomp
        end
      end
    ...
    def cleanup_bootsnap
      ...
      bootsnap.each_child do |subdir|
        cleanup_path(subdir) { FileUtils.rm_r(subdir) } if subdir.basename.to_s != DinrusBrew.bootsnap_key
      end
    end
    ...
    def cleanup_python_site_packages
      ...
      DINRUSBREW_PREFIX.glob("lib/python*/site-packages").each do |site_packages|
    ...
    def prune_prefix_symlinks_and_directories
        ...
        dir.find do |path|
          ...
          if path.symlink?
            unless path.resolved_path_exists?
              ...
              if dry_run?
                puts "Был бы удалён (сломанная ссылка): #{path}"
      ...
      dirs.reverse_each do |d|
        if !dry_run?
          ...
          puts "Был бы удалён (пустая директория): #{d}"
      ...
      print "Отпрюнено #{n} символьных ссылок "
      print "и #{d} директорий " if d.positive?
      puts "из #{DINRUSBREW_PREFIX}"
    end
    ...
    def self.autoremove(dry_run: false)
      ...
      # Remove formulae listed in DINRUSBREW_NO_CLEANUP_FORMULAE and their dependencies.
      if DinrusBrew::EnvConfig.no_cleanup_formulae.present?
      ...
      verb = dry_run ? "Был бы автоудалён" : "Автоудаление"
