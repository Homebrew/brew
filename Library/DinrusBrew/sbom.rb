...
class SBOM
  ...
  SCHEMA_FILE = (DINRUSBREW_LIBRARY_PATH/"data/schemas/sbom.json").freeze
  ...
  def self.create(formula, tab)
    ...
    attributes = {
      name:                 formula.name,
      homebrew_version:     DINRUSBREW_VERSION,
  ...
  def schema_validation_errors(bottling: false)
    unless require? "json_schemer"
      error_message = "Требуется json_schemer для оценки SBOM, выполните `brew install-bundler-gems --add-groups=bottle`!"
      odie error_message if ENV["DINRUSBREW_ENFORCE_SBOM"]
      return []
    end
  ...
  def valid?(bottling: false)
    ...
    opoo "Ошибки валидации SBOM:"
    validation_errors.each(&:puts)

    odie "Не удалось валидировать SBOM относительно схемы JSON!" if ENV["DINRUSBREW_ENFORCE_SBOM"]
  ...
  def write(validate: true, bottling: false)
    ...
    if validate && !valid?(bottling:)
      opoo "SBOM неполноценен, на диск не записывается!"
      return
    end
  ...
  def to_spdx_sbom(bottling:)
    ...
    if bottling
      ...
      creators = ["Tool: https://github.com/DinrusBrew/brew"]
    else
      created = Time.at(time).utc.iso8601
      creators = ["Tool: https://github.com/DinrusBrew/brew@#{homebrew_version}"]
    end
