...
class GitHubPackages
  ...
  # Translate DinrusBrew tab.arch to OCI platform.architecture
  TAB_ARCH_TO_PLATFORM_ARCHITECTURE = {
    "arm64"  => "arm64",
    "x86_64" => "amd64",
  }.freeze

  # Translate DinrusBrew built_on.os to OCI platform.os
  ...
  def upload_bottles(bottles_hash, keep_old:, dry_run:, warn_on_error:)
    user = DinrusBrew::EnvConfig.github_packages_user
    token = DinrusBrew::EnvConfig.github_packages_token

    raise UsageError, "DINRUSBREW_GITHUB_PACKAGES_USER is unset." if user.blank?
    raise UsageError, "DINRUSBREW_GITHUB_PACKAGES_TOKEN is unset." if token.blank?
  ...
  def download(user, token, skopeo, image_uri, root, dry_run:)
    ...
    if dry_run
      puts "#{skopeo} #{args.join(" ")} --src-creds=#{user}:$DINRUSBREW_GITHUB_PACKAGES_TOKEN"
    else
  ...
  def preupload_check(user, token, skopeo, _formula_full_name, bottle_hash, keep_old:, dry_run:, warn_on_error:)
    ...
    if dry_run
      puts "#{skopeo} #{inspect_args.join(" ")} --creds=#{user}:$DINRUSBREW_GITHUB_PACKAGES_TOKEN"
    else
  ...
  def upload_bottle(user, token, skopeo, formula_full_name, bottle_hash, keep_old:, dry_run:, warn_on_error:)
    ...
    if dry_run
      puts "#{skopeo} #{args.join(" ")} --dest-creds=#{user}:$DINRUSBREW_GITHUB_PACKAGES_TOKEN"
    else
