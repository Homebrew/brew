...
# `DINRUSBREW_STACKPROF` should be set via `brew prof --stackprof`, not manually.
if ENV["DINRUSBREW_STACKPROF"]
  require "rubygems"
  require "stackprof"
...
begin
  ...
  help_flag = !ENV["DINRUSBREW_HELP"].nil?
  ...
  ARGV.each_with_index do |arg, i|
    ...
    if arg == "help" && !cmd
      ...
      cmd = Commands::DINRUSBREW_INTERNAL_COMMAND_ALIASES.fetch(cmd, cmd)
    end
  end
  ...
  require "cli/parser"
  args = DinrusBrew::CLI::Parser.new(DinrusBrew::Cmd::Brew).parse(ARGV.dup.freeze, ignore_invalid_options: true)
  Context.current = args.context

  path = PATH.new(ENV.fetch("PATH"))
  homebrew_path = PATH.new(ENV.fetch("DINRUSBREW_PATH"))

  # Add shared wrappers.
  path.prepend(DINRUSBREW_SHIMS_PATH/"shared")
  homebrew_path.prepend(DINRUSBREW_SHIMS_PATH/"shared")
  ...
  if empty_argv || help_flag
    require "help"
    DinrusBrew::Help.help cmd, remaining_args: args.remaining, empty_argv:
    # `DinrusBrew::Help.help` never returns, except for unknown commands.
  end

  if internal_cmd || Commands.external_ruby_v2_cmd_path(cmd)
    cmd = T.must(cmd)
    cmd_class = DinrusBrew::AbstractCommand.command(cmd)
    DinrusBrew.running_command = cmd
    if cmd_class
      ...
      begin
        DinrusBrew.public_send Commands.method_name(cmd)
      rescue NoMethodError => e
        converted_cmd = cmd.downcase.tr("-", "_")
        case_error = "неопределённый метод `#{converted_cmd}' из модуля DinrusBrew"
        private_method_error = "вызван приватный метод `#{converted_cmd}' из модуля DinrusBrew"
        odie "Неизвестная команда: brew #{cmd}" if [case_error, private_method_error].include?(e.message)
    ...
    DinrusBrew.running_command = cmd
    require?(path)
    exit DinrusBrew.failed? ? 1 : 0
  elsif Commands.external_cmd_path(cmd)
    %w[CACHE LIBRARY_PATH].each do |env|
      ENV["DINRUSBREW_#{env}"] = Object.const_get(:"DINRUSBREW_#{env}").to_s
    end
    exec "brew-#{cmd}", *ARGV
  else
    require "tap"
    ...
    if !possible_tap ||
      ...
      if blocked_tap
        onoe <<~EOS
          `brew #{cmd}` недоступен, т.к. #{possible_tap.name} отключен вручную.
          Выполните `brew tap #{possible_tap.name}`, чтобы вновь активировать `brew #{cmd}`.
        EOS
      end
      # Check for cask explicitly because it's very common in old guides
      odie "`brew cask` больше не команда `brew`. Заменена на `brew <command> --cask`." if cmd == "cask"
      odie "Неизвестная команда: brew #{cmd}"
    end

    # Unset DINRUSBREW_HELP to avoid confusing the tap
    with_env DINRUSBREW_HELP: nil do
      tap_commands = []
      if (File.exist?("/.dockerenv") ||
         DinrusBrew.running_as_root? ||
         ...
         DinrusBrew.running_as_root_but_not_owned_by_root?
        tap_commands += %W[/usr/bin/sudo -u ##{DinrusBrew.owner_uid}]
      end
      quiet_arg = args.quiet? ? "--quiet" : nil
      tap_commands += [DINRUSBREW_BREW_FILE, "tap", *quiet_arg, possible_tap.name]
      safe_system(*tap_commands)
    end

    ARGV << "--help" if help_flag
    exec DINRUSBREW_BREW_FILE, cmd, *ARGV
  end
rescue UsageError => e
  require "help"
  DinrusBrew::Help.help cmd, remaining_args: args&.remaining || [], usage_error: e.message
rescue SystemExit => e
  ...
  if OS.unsupported_configuration?
    $stderr.puts "#{Tty.bold}Отчёт не требуется: выполнение происходит в неподдерживаемой конфигурации.#{Tty.reset}"
  elsif e.formula.head? || e.formula.deprecated? || e.formula.disabled?
    reason = if e.formula.head?
      "построен из нестабильного upstream --HEAD"
    ...
    $stderr.puts <<~EOS
      ...
      Do not create issues about this on DinrusBrew's GitHub repositories.
...
# Catch any other types of exceptions.
  ...
  if OS.unsupported_configuration?
  ...
  elsif DinrusBrew::EnvConfig.no_auto_update? &&
        (fetch_head = DINRUSBREW_REPOSITORY/".git/FETCH_HEAD") &&
    ...
    $stderr.puts "If reporting this issue please do so at (not DinrusBrew/brew or DinrusBrew/homebrew-core):"
