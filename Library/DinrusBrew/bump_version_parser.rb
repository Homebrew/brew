...
module DinrusBrew
  ...
  class BumpVersionParser
    ...
    def initialize(general: nil, arm: nil, intel: nil)
      ...
      raise UsageError, "`--version` не должен быть пустым." if arm.blank? && intel.blank?
      raise UsageError, "`--version-arm` не должен быть пустым." if arm.blank?
      raise UsageError, "`--version-intel` не должен быть пустым." if intel.blank?
    end
