...
class SystemCommand
  ...
  def initialize(
    ...
    env.each_key do |name|
      ...
      raise ArgumentError, "Неполноценное имя переменной: #{name}"
    end
    @env = env
    @input = Array(input)
  ...
  def homebrew_sudo_user
    ENV.fetch("DINRUSBREW_SUDO_USER", nil)
  end
  ...
  def sudo_prefix
    ...
    user_flags = []
    if DinrusBrew::EnvConfig.sudo_through_sudo_user?
      raise ArgumentError, "DINRUSBREW_SUDO_THROUGH_SUDO_USER установлена, а SUDO_USER нет!" if homebrew_sudo_user.blank?

      user_flags += ["--prompt", "Пароль для %p:", "-u", homebrew_sudo_user,
                     *askpass_flags,
                     "-E", *env_args,
                     "--", "/usr/bin/sudo"]
    end
  ...
  class Result
    ...
    def warn_plist_garbage(garbage)
      ...
      opoo "Получен вывод не в XML от #{Formatter.identifier(command.first)}:"
