    ...
    class << self
      ...
      def disallowed_reason(name)
        ...
        when "gem", /^rubygems?$/ then <<~EOS
          macOS предоставляет gem как часть Ruby. Чтобы установить более новую версию:
            brew install ruby
        EOS
        when "pip" then <<~EOS
          pip является частью формулы python:
            brew install python
        EOS
        when "pil" then <<~EOS
          Вместо PIL, рассмотрите pillow:
            brew install pillow
        EOS
        when "macruby" then <<~EOS
          MacRuby более не разрабатывается. Рассмотрите RubyMotion:
            brew install --cask rubymotion
        EOS
        when /(lib)?lzma/ then <<~EOS
          lzma теперь входит в формулу xz:
            brew install xz
        EOS
        when "gsutil" then <<~EOS
          gsutil доступен через pip:
            pip3 install gsutil
        EOS
        when "gfortran" then <<~EOS
          GNU Fortran входит в формулу GCC:
            brew install gcc
        EOS
        when "play" then <<~EOS
          Play 2.3 заменяет команду play c активатором:
            brew install typesafe-activator

          Подробнее об этих изменениях написано в:
        ...
        when "haskell-platform" then <<~EOS
          Компоненты Haskell Platform доступны отдельно.

          Glasgow Haskell Compiler:
            brew install ghc

          Система построения Cabal:
