...
class Migrator
  ...
  class MigratorNoOldpathError < RuntimeError
    def initialize(oldname)
      super "#{DINRUSBREW_CELLAR/oldname} не существует."
    end
  end
  ...
  def initialize(formula, oldname, force: false)
    ...
    @formula = formula
    @old_cellar = DINRUSBREW_CELLAR/oldname
    ...
    @new_cellar = DINRUSBREW_CELLAR/formula.name
    ...
    unless old_linked_kegs.empty?
      @new_linked_keg_record = DINRUSBREW_CELLAR/"#{newname}/#{File.basename(old_linked_kegs.first)}"
    end

    @old_pin_record = DINRUSBREW_PINNED_KEGS/oldname
    @new_pin_record = DINRUSBREW_PINNED_KEGS/newname
  ...
  def from_same_tap_user?
    ...
    if formula_tap_user == old_tap_user
      true
    # DinrusBrew didn't use to update tabs while performing tap-migrations,
