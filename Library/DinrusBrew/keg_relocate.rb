...
class Keg
  PREFIX_PLACEHOLDER = "@@DINRUSBREW_PREFIX@@"
  CELLAR_PLACEHOLDER = "@@DINRUSBREW_CELLAR@@"
  REPOSITORY_PLACEHOLDER = "@@DINRUSBREW_REPOSITORY@@"
  LIBRARY_PLACEHOLDER = "@@DINRUSBREW_LIBRARY@@"
  PERL_PLACEHOLDER = "@@DINRUSBREW_PERL@@"
  JAVA_PLACEHOLDER = "@@DINRUSBREW_JAVA@@"
  NULL_BYTE = "\x00"
  ...
  def fix_dynamic_linkage
    symlink_files.each do |file|
      ...
      link_starts_cellar = link.to_s.start_with?(DINRUSBREW_CELLAR.to_s)
      link_starts_prefix = link.to_s.start_with?(DINRUSBREW_PREFIX.to_s)
  ...
  JAVA_REGEX = %r{#{DINRUSBREW_PREFIX}/opt/openjdk(@\d+(\.\d+)*)?/libexec(/openjdk\.jdk/Contents/Home)?}

  def prepare_relocation_to_placeholders
    relocation = Relocation.new
    relocation.add_replacement_pair(:prefix, DINRUSBREW_PREFIX.to_s, PREFIX_PLACEHOLDER, path: true)
    relocation.add_replacement_pair(:cellar, DINRUSBREW_CELLAR.to_s, CELLAR_PLACEHOLDER, path: true)
    # when DINRUSBREW_PREFIX == DINRUSBREW_REPOSITORY we should use DINRUSBREW_PREFIX for all relocations to avoid
    # being unable to differentiate between them.
    if DINRUSBREW_PREFIX != DINRUSBREW_REPOSITORY
      relocation.add_replacement_pair(:repository, DINRUSBREW_REPOSITORY.to_s, REPOSITORY_PLACEHOLDER, path: true)
    end
    relocation.add_replacement_pair(:library, DINRUSBREW_LIBRARY.to_s, LIBRARY_PLACEHOLDER, path: true)
    relocation.add_replacement_pair(:perl,
                                    %r{\A#![ \t]*(?:/usr/bin/perl\d\.\d+|#{DINRUSBREW_PREFIX}/opt/perl/bin/perl)( |$)}o,
                                    "#!#{PERL_PLACEHOLDER}\\1")
  ...
  def prepare_relocation_to_locations
    relocation = Relocation.new
    relocation.add_replacement_pair(:prefix, PREFIX_PLACEHOLDER, DINRUSBREW_PREFIX.to_s)
    relocation.add_replacement_pair(:cellar, CELLAR_PLACEHOLDER, DINRUSBREW_CELLAR.to_s)
    relocation.add_replacement_pair(:repository, REPOSITORY_PLACEHOLDER, DINRUSBREW_REPOSITORY.to_s)
    relocation.add_replacement_pair(:library, LIBRARY_PLACEHOLDER, DINRUSBREW_LIBRARY.to_s)
    relocation.add_replacement_pair(:perl, PERL_PLACEHOLDER, "#{DINRUSBREW_PREFIX}/opt/perl/bin/perl")
    if (openjdk = openjdk_dep_name_if_applicable)
      relocation.add_replacement_pair(:java, JAVA_PLACEHOLDER, "#{DINRUSBREW_PREFIX}/opt/#{openjdk}/libexec")
    end
