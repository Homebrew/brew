...
module Commands
  DINRUSBREW_CMD_PATH = (DINRUSBREW_LIBRARY_PATH/"cmd").freeze
  DINRUSBREW_DEV_CMD_PATH = (DINRUSBREW_LIBRARY_PATH/"dev-cmd").freeze
  # If you are going to change anything in below hash,
  # be sure to also update appropriate case statement in brew.sh
  DINRUSBREW_INTERNAL_COMMAND_ALIASES = {
    "ls"           => "list",
    "homepage"     => "home",
    "-S"           => "search",
    "up"           => "update",
  ...
  def self.internal_cmd_path(cmd)
    [
      DINRUSBREW_CMD_PATH/"#{cmd}.rb",
      DINRUSBREW_CMD_PATH/"#{cmd}.sh",
    ].find(&:exist?)
  end

  def self.internal_dev_cmd_path(cmd)
    [
      DINRUSBREW_DEV_CMD_PATH/"#{cmd}.rb",
      DINRUSBREW_DEV_CMD_PATH/"#{cmd}.sh",
    ].find(&:exist?)
  end
  ...
  def self.path(cmd)
    internal_cmd = DINRUSBREW_INTERNAL_COMMAND_ALIASES.fetch(cmd, cmd)
  ...
  def self.tap_cmd_directories
    Pathname.glob DINRUSBREW_TAP_DIRECTORY/"*/*/cmd"
  end

  def self.internal_commands_paths
    find_commands DINRUSBREW_CMD_PATH
  end

  def self.internal_developer_commands_paths
    find_commands DINRUSBREW_DEV_CMD_PATH
  end
  ...
  def self.internal_commands
    find_internal_commands(DINRUSBREW_CMD_PATH).map(&:to_s)
  end

  def self.internal_developer_commands
    find_internal_commands(DINRUSBREW_DEV_CMD_PATH).map(&:to_s)
  end

  def self.internal_commands_aliases
    DINRUSBREW_INTERNAL_COMMAND_ALIASES.keys
  end
  ...
  def self.rebuild_internal_commands_completion_list
    ...
    cmds.reject! { |cmd| DinrusBrew::Completions::COMPLETIONS_EXCLUSION_LIST.include? cmd }

    file = DINRUSBREW_REPOSITORY/"completions/internal_commands_list.txt"
  ...
  def self.rebuild_commands_completion_list
    ...
    cmds = commands(aliases: true) - DinrusBrew::Completions::COMPLETIONS_EXCLUSION_LIST

    all_commands_file = DINRUSBREW_CACHE/"all_commands_list.txt"
    external_commands_file = DINRUSBREW_CACHE/"external_commands_list.txt"
  ...
  def self.command_options(command)
    ...
    if (cmd_parser = DinrusBrew::CLI::Parser.from_cmd_path(path))
  ...
  def self.named_args_type(command)
    ...
    cmd_parser = DinrusBrew::CLI::Parser.from_cmd_path(path)
