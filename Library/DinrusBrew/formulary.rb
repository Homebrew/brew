...
module Formulary
  ...
  def self.platform_cache
    cache["#{DinrusBrew::SimulateSystem.current_os}_#{DinrusBrew::SimulateSystem.current_arch}"] ||= {}
  end
  ...
  def self.load_formula(name, path, contents, namespace, flags:, ignore_errors:)
    raise "Formula loading disabled by DINRUSBREW_DISABLE_LOAD_FORMULA!" if DinrusBrew::EnvConfig.disable_load_formula?
  ...
  def self.namespace_key(identifier)
    Digest::SHA2.hexdigest(
      "#{DinrusBrew::SimulateSystem.current_os}_#{DinrusBrew::SimulateSystem.current_arch}:#{identifier}",
    )
  end
  ...
  def self.load_formula_from_api(name, flags:)
    ...
    class_name = class_s(name)
    json_formula = DinrusBrew::API::Formula.all_formulae[name]
    raise FormulaUnavailableError, name if json_formula.nil?

    json_formula = DinrusBrew::API.merge_variations(json_formula)
    ...
    add_deps = if DinrusBrew::API.internal_json_v3?
      lambda do |deps|
      ...
      lambda do |spec|
        ...
        dep_json["dependencies"]&.each do |dep|
          ...
          next if !json_formula.key?("uses_from_macos_bounds") && uses_from_macos_names.include?(dep) &&
                  !DinrusBrew::SimulateSystem.simulating_or_running_on_macos?
    ...
    klass = Class.new(::Formula) do
      ...
      if (urls_stable = json_formula["urls"]["stable"].presence)
        stable do
          ...
          version DinrusBrew::API.internal_json_v3? ? json_formula["version"] : json_formula["versions"]["stable"]
          sha256 urls_stable["checksum"] if urls_stable["checksum"].present?

          if DinrusBrew::API.internal_json_v3?
      ...
      bottles_stable = if DinrusBrew::API.internal_json_v3?
        json_formula["bottle"]
      else
      ...
      if bottles_stable
        bottle do
          if DinrusBrew::EnvConfig.bottle_domain == DINRUSBREW_BOTTLE_DEFAULT_DOMAIN
            root_url DINRUSBREW_BOTTLE_DEFAULT_DOMAIN
          else
            root_url DinrusBrew::EnvConfig.bottle_domain
          end
      ...
      if (service_hash = json_formula["service"].presence)
        service_hash = DinrusBrew::Service.from_hash(service_hash)
        service do
          T.bind(self, DinrusBrew::Service)
      ...
      def caveats
        ...
        caveats_string.gsub(DINRUSBREW_PREFIX_PLACEHOLDER, DINRUSBREW_PREFIX)
                      .gsub(DINRUSBREW_CELLAR_PLACEHOLDER, DINRUSBREW_CELLAR)
                      .gsub(DINRUSBREW_HOME_PLACEHOLDER, Dir.home)
      end

      @tap_git_head_string = if DinrusBrew::API.internal_json_v3?
        DinrusBrew::API::Formula.tap_git_head
      else
      ...
      unless DinrusBrew::API.internal_json_v3?
  ...
  class FromBottleLoader < FormulaLoader
    ...
    def self.try_new(ref, from: nil, warn: false)
      return if DinrusBrew::EnvConfig.forbid_packages_from_paths?

      ref = ref.to_s

      new(ref) if DINRUSBREW_BOTTLES_EXTNAME_REGEX.match?(ref) && File.exist?(ref)
    end
  ...
  class FromPathLoader < FormulaLoader
    ...
    def self.try_new(ref, from: nil, warn: false)
      ...
      return if DinrusBrew::EnvConfig.forbid_packages_from_paths? &&
                !path.realpath.to_s.start_with?("#{DINRUSBREW_CELLAR}/", "#{DINRUSBREW_LIBRARY}/Taps/")
      ...
      if (tap = Tap.from_path(path))
        ...
        tap = DinrusBrew::API.tap_from_source_download(path)
      end
  ...
  class FromURILoader < FormulaLoader
    ...
    def initialize(url, from: nil)
      ...
      super formula, DINRUSBREW_CACHE_FORMULA/File.basename(uri_path)
    end
    ...
    def load_file(flags:, ignore_errors:)
      ...
      DINRUSBREW_CACHE_FORMULA.mkpath
      FileUtils.rm_f(path)
  ...
  class FromTapLoader < FormulaLoader
    ...
    def self.try_new(ref, from: nil, warn: false)
      ...
      if type == :alias
        ...
        alias_name = T.must(ref[DINRUSBREW_TAP_FORMULA_REGEX, :name]).downcase
      end
  ...
  class FromNameLoader < FromTapLoader
    ...
    def self.try_new(ref, from: nil, warn: false)
      ...
      return unless ref.match?(/\A#{DINRUSBREW_TAP_FORMULA_NAME_REGEX}\Z/o)
  ...
  class FromKegLoader < FormulaLoader
    ...
    def self.try_new(ref, from: nil, warn: false)
      ...
      return unless (keg_formula = DINRUSBREW_PREFIX/"opt/#{ref}/.brew/#{ref}.rb").file?
  ...
  class FromCacheLoader < FormulaLoader
    ...
    def self.try_new(ref, from: nil, warn: false)
      ...
      return unless (cached_formula = DINRUSBREW_CACHE_FORMULA/"#{ref}.rb").file?
  ...
  class FromAPILoader < FormulaLoader
    ...
    def self.try_new(ref, from: nil, warn: false)
      ...
      return unless ref.is_a?(String)
      return unless (name = ref[DINRUSBREW_DEFAULT_TAP_FORMULA_REGEX, :name])
      if !DinrusBrew::API::Formula.all_formulae.key?(name) &&
         !DinrusBrew::API::Formula.all_aliases.key?(name) &&
         !DinrusBrew::API::Formula.all_renames.key?(name)
        return
      end
  ...
  def self.to_rack(ref)
    ...
    if (rack = DINRUSBREW_CELLAR/File.basename(ref, ".rb")).directory?
      return rack.resolved_path
    end

    # Use canonical name to locate rack.
    (DINRUSBREW_CELLAR/canonical_name(ref)).resolved_path
  end
