...
module DinrusBrew
  ...
  module Manpages
    ...
    SOURCE_PATH = (DINRUSBREW_LIBRARY_PATH/"manpages").freeze
    TARGET_MAN_PATH = (DINRUSBREW_REPOSITORY/"manpages").freeze
    TARGET_DOC_PATH = (DINRUSBREW_REPOSITORY/"docs").freeze
    ...
    def self.build_man_page(quiet:)
      ...
      variables = Variables.new(
        ...
        lead:                       readme.read[/(DinrusBrew's \[Project Leader.*\.)/, 1]
                                      .gsub(/\[([^\]]+)\]\([^)]+\)/, '\1'),
        plc:                        readme.read[/(DinrusBrew's \[Project Leadership Committee.*\.)/, 1]
                                      .gsub(/\[([^\]]+)\]\([^)]+\)/, '\1'),
        tsc:                        readme.read[/(DinrusBrew's \[Technical Steering Committee.*\.)/, 1]
                                      .gsub(/\[([^\]]+)\]\([^)]+\)/, '\1'),
        maintainers:                readme.read[/(DinrusBrew's maintainers .*\.)/, 1]
                                      .gsub(/\[([^\]]+)\]\([^)]+\)/, '\1'),
    ...
    def self.generate_cmd_manpages(cmd_paths)
      ...
      cmd_paths.sort_by { sort_key_for_path(_1) }
               .each do |cmd_path|
        cmd_man_page_lines = if (cmd_parser = DinrusBrew::CLI::Parser.from_cmd_path(cmd_path))
    ...
    def self.cmd_parser_manpage_lines(cmd_parser)
      ...
      lines += cmd_parser.processed_options.filter_map do |short, long, desc, hidden|
        ...
        if long.present?
          next if DinrusBrew::CLI::Parser.global_options.include?([short, long, desc])
          next if DinrusBrew::CLI::Parser.global_cask_options.any? do |_, option, kwargs|
    ...
    def self.global_cask_options_manpage
      ...
      lines += DinrusBrew::CLI::Parser.global_cask_options.map do |_, long, kwargs|
    ...
    def self.global_options_manpage
      ...
      lines += DinrusBrew::CLI::Parser.global_options.map do |short, long, desc|
        generate_option_doc(short, long, desc)
      end
      lines.join("\n")
    end
    ...
    def self.env_vars_manpage
      lines = DinrusBrew::EnvConfig::ENVS.flat_map do |env, hash|
        entry = "`#{env}`\n\n: #{hash[:description]}\n"
