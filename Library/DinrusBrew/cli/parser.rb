  ...
  module CLI
    class Parser
      ...
      def self.from_cmd_path(cmd_path)
        ...
        begin
          if require?(cmd_path)
            cmd = DinrusBrew::AbstractCommand.command(cmd_name)
            if cmd
              cmd.parser
            else
              # FIXME: remove once commands are all subclasses of `AbstractCommand`:
              DinrusBrew.send(cmd_args_method_name)
            end
          end
      ...
      sig {
        params(cmd: T.nilable(T.class_of(DinrusBrew::AbstractCommand)), block: T.nilable(T.proc.bind(Parser).void)).void
      }
      def initialize(cmd = nil, &block)
        ...
        if cmd
          ...
          @is_dev_cmd = T.let(T.must(cmd_location.absolute_path).start_with?(Commands::DINRUSBREW_DEV_CMD_PATH),
                              T::Boolean)
          odeprecated(
            "`brew #{@command_name}'. This command needs to be refactored, as it is written in a style that",
            "inherits from `DinrusBrew::AbstractCommand' ( see https://docs.brew.sh/External-Commands )",
      ...
      def generate_usage_banner
        ...
        command_names += Commands::DINRUSBREW_INTERNAL_COMMAND_ALIASES.filter_map do |command_alias, command|
      ...
      def formulae(argv)
        ...
        named_args.filter_map do |arg|
          next if arg.match?(DINRUSBREW_CASK_TAP_CASK_REGEX)
      ...
      def value_for_env(env)
        ...
        method_name = :"#{env}?"
        if DinrusBrew::EnvConfig.respond_to?(method_name)
          DinrusBrew::EnvConfig.public_send(method_name)
        else
          ENV.fetch("DINRUSBREW_#{env.upcase}", nil)
        end
      end
    end
  end
