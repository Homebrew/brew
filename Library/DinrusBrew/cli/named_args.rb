  ...
  module CLI
    ...
    class NamedArgs < Array
      ...
      def to_paths(only: parent.only_formula_or_cask, recurse_tap: false)
        ...
        @to_paths[only] ||= DinrusBrew.with_no_api_env_if_needed(@without_api) do
          downcased_unique_named.flat_map do |name|
            ...
            if only.nil? && name.match?(LOCAL_PATH_REGEX) && path.exist?
              ...
              if formula_path.exist? ||
                 ...
                 DinrusBrew::API::Formula.all_formulae.key?(path.basename.to_s))
                paths << formula_path
              end
              if cask_path.exist? ||
                 (!DinrusBrew::EnvConfig.no_install_from_api? &&
                 !CoreCaskTap.instance.installed? &&
                 DinrusBrew::API::Cask.all_casks.key?(path.basename.to_s))
                paths << cask_path
              end
      ...
      def homebrew_tap_cask_names
        downcased_unique_named.grep(DINRUSBREW_CASK_TAP_CASK_REGEX)
      end
      ...
      def load_formula_or_cask(name, only: nil, method: nil, warn: nil)
        DinrusBrew.with_no_api_env_if_needed(@without_api) do
      ...
      def resolve_default_keg(name)
        ...
        linked_keg_ref = DINRUSBREW_LINKED_KEGS/rack.basename
        opt_prefix = DINRUSBREW_PREFIX/"opt/#{rack.basename}"
