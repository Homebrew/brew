...
class Sandbox
  ...
  def allow_write_temp_and_cache
    ...
    allow_write_path DINRUSBREW_TEMP
    allow_write_path DINRUSBREW_CACHE
  end
  ...
  def deny_write_homebrew_repository
    deny_write path: DINRUSBREW_ORIGINAL_BREW_FILE
    if DINRUSBREW_PREFIX.to_s == DINRUSBREW_REPOSITORY.to_s
      deny_write_path DINRUSBREW_LIBRARY
      deny_write_path DINRUSBREW_REPOSITORY/".git"
    else
      deny_write_path DINRUSBREW_REPOSITORY
    end
  end
  ...
  def run(*args)
    Dir.mktmpdir("homebrew-sandbox", DINRUSBREW_TEMP) do |tmpdir|
      ...
      begin
        ...
        unless logs.empty?
          ...
          if @failed && DinrusBrew::EnvConfig.verbose?
