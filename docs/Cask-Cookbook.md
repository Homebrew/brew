...
| `app_image`                                                                            |              yes              | Relative path to an AppImage that should be linked into the configured AppImage directory on installation. If the AppImage bundles desktop metadata, Homebrew also adds it to the user's XDG application menu and makes it launch the linked AppImage. An optional `binary` parameter also links it into Homebrew's binary directory under the provided name.                                                                                 |
