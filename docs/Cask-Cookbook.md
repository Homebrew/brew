...
| `container type:`                          | no                            | Symbol to override container-type autodetect. May be one of: `:bzip2`, `:dmg`, `:generic_unar`, `:gzip`, `:otf`, `:pkg`, `:seven_zip`, `:tar`, `:ttf`, `:zip`, `:naked`. The deprecated `:cab`, `:rar`, `:sit` and `:xar` types should use `:generic_unar`; deprecated `:air` containers should migrate to an `app` or `pkg` artifact. (Example: [parse.rb](https://github.com/Homebrew/homebrew-cask/blob/aa461148bbb5119af26b82cccf5003e2b4e50d95/Casks/p/parse.rb#L10)) |
...
Some operations cannot run inside the sandbox at all. Loading a launch agent is one: launchd refuses job submission from any sandboxed process, so a `run` step calling `launchctl bootstrap` fails with `Bootstrap failed: 5: Input/output error` whatever paths the sandbox permits, and the domain target needs a numeric UID that no install-time token provides. A steps block can still write the agent's property list with `write_file` and `base: :home`. Loading it requires an [`installer script:`](#installer-script), which runs outside the sandbox and can resolve the UID itself, or the installed software must load its own agent when it next runs.
